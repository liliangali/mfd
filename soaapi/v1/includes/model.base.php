...
class BaseModel
{
    ...
    function BaseModel($params, $db)
    {
        ...
        if ($this->true_table_name)
        {
            $this->table = $this->true_table_name;
        }
        else
        {
    ...
    function _getJoinString($relation_info)
    {
        switch ($relation_info['type'])
        {
            case HAS_ONE:
                ...
                /* 联合限制 */
                $ext_limit = '';
                !empty($relation_info['ext_limit']) && $ext_limit = ' AND ' . $this->_getExtLimit($relation_info['ext_limit'], $model->alias);//须加上当前被关联表的别名，因为有可能存在多个JOIN，并且可能存在同名字段。
