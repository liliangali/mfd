...
// print_r($file['type']);exit;
						...
						//创建一个目录
						$sub_dir = date('Ym');
// echo $this->_upload_dir.$sub_dir;exit;
						if(!is_dir($this->_upload_dir.$sub_dir)) {
							mkdir($this->_upload_dir.$sub_dir, 0777, true);
						}
// print_exit($file['tmp_name']."__".$this->_upload_dir.$sub_dir.'/'.$new_name);
// var_dump(move_uploaded_file($file['tmp_name'],$this->_upload_dir.$sub_dir.'/'.$new_name));exit;
// 						file_put_contents($this->_upload_dir.$sub_dir.'/'.$new_name, file_get_contents($file['tmp_name']));
						if(file_put_contents($this->_upload_dir.$sub_dir.'/'.$new_name, file_get_contents($file['tmp_name']))) {
							//上传文件成功 并且文件被放在了$this->_upload_dir.$new_name下边
// echo $sub_dir.'/'.$new_name;exit;
        ...
        public function uploadImage1($file) {
            //判断文件上传成功了没有
// echo $file['error'];exit;
//exit();
            if($file['error'] == 0) {
