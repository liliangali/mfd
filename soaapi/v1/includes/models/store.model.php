...
class StoreModel extends BaseModel
{
    ...
    function get_info($store_id)
    {
        ...
        $info["rate"] = $this->recount_praise_rate($store_id);
        return $info;
    }
    ...
    function recount_credit_value($store_id)
    {
        ...
        $info = $model_ordergoods->get(array(
            ...
            'conditions'    => "store_id={$store_id} AND evaluation_status=1 AND is_valid = 1",
    ...
    function recount_praise_rate($store_id)
    {
        ...
        $info  = $model_ordergoods->get(array(
            'join'          => 'belongs_to_order',
            'conditions'    => "store_id={$store_id} AND is_valid=1",
        ...
        $info = $model_ordergoods->get(array(
            'join'          => 'belongs_to_order',
            'conditions'    => "store_id={$store_id} AND evaluation_status=1 AND is_valid=1 AND evaluation=3",
...
//        }
    ...
    */
    ...
    function get_new_store(){
    	
    	$sql = "SELECT * FROM {$this->table} ORDER BY add_time DESC LIMIT 3";
