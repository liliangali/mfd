...
//     dump($data['data']);
    		$data['img_path'] = SITE_URL.'/'.IMG_PATH.'/custom/';
    		...
    		$data['botouParent'] = Constants::$botouParent;						//驳头型特殊处理
//    dump($data);
    		//获取分类下的工艺分类
...
//     		print_r($nodes_found); 
    		...
    		foreach ($nodes_found as $v){
    			...
    			if(in_array($v['top_cate'], Constants::$graphParent)){//工艺
    				if (!in_array($v['cate_id'], Constants::$graphRuleoutParentCid)){//工艺下不纳入出图规则计算
...
//     			foreach ($item as $r){
    				$patinfo = array();
    				$spec = array();
    				$patinfo = $this->_parts_mod->get(array(
                        'conditions' => $item['pt_id'],
                        "fields" => "part_id,part_name,code,cost_price,price,part_number,part_small,sort_order,cate_id,zindex,vcompositionid,vcolorid,vflowerid"));
//     				$spec = $this->_format_spec_data($item['pt_id']);
    	...
    	if ($hiddendata){
    		foreach ($hiddendata as $r){
    				if (in_array($r['parent_cate'], Constants::$processParent)){
    					$process[] = array('price'=>$r['price'],'part_id'=>$r['part_id'],'part_name'=>$r['part_name']);
//     					$process[] = array($r['part_name']);
    				}
    		}
