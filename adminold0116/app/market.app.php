...
class MarketApp extends BackendApp
{
    ...
    function add()
    {
        if (!IS_POST)
        {
			...
			$this->assign('scategories', $scategory_mod->get_list());
			//echo "<pre>"; print_r($scategory_mod->get_list()); echo "</pre>";
        ...
        else
        {
            ...
            $data['mark_desc']  = $_POST['mark_desc'];
            ...
            $this->show_message('add_brand_successed',
                ...
                'continue_add', 'index.php?app=market&amp;act=add'
            );
        }
    }
    ...
    function edit()
    {
        ...
        else
        {
            ...
            $this->show_message('edit_brand_successed',
                'back_list',        'index.php?app=market',
                'edit_again',    'index.php?app=market&amp;act=edit&amp;id=' . $brand_id);
        }
    }
   ...
   function ajax_col()
   {
       ...
       if (in_array($column ,array('name', 'recommended', 'sort_order', 'tag')))
       {
           $data[$column] = $value;
           if($column == 'name')
           {
    ...
    function _upload_logo($brand_id)
    {
        ...
        {
            $this->show_warning($uploader->get_error() , 'go_back', 'index.php?app=market&amp;act=edit&amp;id=' . $brand_id);
            return false;
        }
        /* 指定保存位置的根目录 */
        ...
        /* 上传 */
        if ($file_path = $uploader->save('data/files/mall/market', $brand_id))   //保存到指定目录，并以指定文件名$brand_id存储
        {
            return $file_path;
        }
        else
    ...
    function pass()
    {
        ...
        $ids = explode(',', $id);
        $brands = $this->_brand_mod->find(db_create_in($ids, 'id') . " AND if_show = 0");
        $this->_brand_mod->edit(db_create_in(array_keys($brands), 'id'), array('if_show' => 1));
        if ($this->_brand_mod->has_error())
        {
            $this->show_warning($this->_brand_mod->get_error());
            exit;
        ...
        {
            $content = get_msg('toseller_brand_passed_notify', array('name' => $brand['name']));
            $ms->pm->send(MSG_SYSTEM, $brand['store_id'], '', $content);
        }
        $this->show_message('brand_passed',
            'back_list', 'index.php?app=market&wait_verify=1');
    }
    ...
    function refuse()
    {
        ...
        if (!IS_POST)
        {
            ...
            $this->display('market_refuse.html');
        }
        else
        {
            ...
            foreach ($brands as $brand)
            {
                $content = get_msg('toseller_brand_refused_notify', array('name' => $brand['name'], 'reason' => trim($_POST['content'])));
                $ms->pm->send(MSG_SYSTEM, $brand['store_id'], '', $content);
                if (is_file(ROOT_PATH . '/' . $brand['brand_logo']) && file_exists(ROOT_PATH . '/' . $brand['brand_logo']))
                {
                    unlink(ROOT_PATH . '/' . $brand['brand_logo']);
                }
                $this->_brand_mod->drop($brand['id']);
            }
