...
(function($){
    $.rc.user = {
        settings: {
            ...
            add_ok_html: '<span class="fo_u_ok" style="display:none;">已关注</span><a href="javascript:;" class="J_unfo_u green">取消</a>',
            add_all_html: '<a href="javascript:;" class="J_unfo_u green">取消</a>'
        },
        init: function(options){
            options && $.extend($.rc.user.settings, options);
            var s = $.rc.user.settings;
            //名片
            $.rc.user.card();
            //名片上的关注
            $.rc.user.follow();
        ...
        follow: function(){
            var s = $.rc.user.settings;
            $(s.follow_bar).find('.J_fo_u').bind('click', function(){
                if(!$.rc.dialog.islogin()) return !1;
                var self = $(this),
                    uid = $(this).parent().attr('data-uid');
                $.getJSON('/index.php/club-ajax_follow.html', {uid:uid}, function(result){
                    if(result.status == 1){
                        result.data == 1 ? self.parent().html(s.add_ok_html) : self.parent().html(s.add_all_html);
                        $("body").data(uid, false);
                        $.rc.user.unfollow();
                        $.rc.tip({content:result.msg,  icon:'ok'});
                    }else{
        ...
        unfollow: function(){
            var s = $.rc.user.settings;
            $(s.follow_bar).find('.J_unfo_u').bind('click', function(){
                if(!$.rc.dialog.islogin()) return !1;
                if(!confirm(lang.confirm_unfollow)) return !1;
                var self = $(this),
                    uid = $(this).parent().attr('data-uid');
                $.getJSON('/index.php/club-ajax_unfollow.html', {uid:uid}, function(result){
                    if(result.status == 1){
                        self.parent().html(s.add_btn_html);
                        $(".J_fo_u fo_u_btn").data(uid, uid);
...
//                    }else{
                        n = setTimeout(function(){
                            $.getJSON('/index.php/club-ajax_cardinfo-'+uid+'.html', {uid:uid}, function(result){
                            	//alert(result);
                            	$("#J_card_info").html(result);
                                if(result.status == 1){
...
//                    }
        ...
        register_form: function(form){
            //协议
            $('#J_protocol_btn').bind('click', function(){
