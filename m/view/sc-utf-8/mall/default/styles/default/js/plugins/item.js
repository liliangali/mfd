...
//            joinalbum_btn: '.J_joinalbum',		//加入专辑忽略
            item_unit: '.J_item',
        ...
        init: function(options){
            ...
            //单个
            $(s.item_unit).bind('mouseover', function(){
        ...
        like: function(){
            ...
            $(s.like_btn).bind('click', function(){
                ...
                $.getJSON('/index.php/club-ajax_like-'+id+'-'+ty+'.html', {}, function(result){
                    if(result.status == 1){
                        //喜欢成功
                        nb.text(n+1).show();
                        $("#like_"+id).attr('class',"yxih");
                        $("#like_"+id).attr('data-ty', false);
        ...
        unlike: function(){
            var s = $.rc.item.settings;
            $(s.unlike_btn).bind('click', function(){
                if(!$.rc.dialog.islogin()) return !1;
                var self = $(this),
                    id = self.attr('data-id');
        ...
        del: function(){
            var s = $.rc.item.settings;
            $(s.del_btn).bind('click', function(){
                if(!$.rc.dialog.islogin()) return !1;
                var self = $(this),
                    id = self.attr('data-id'),
                    album_id = self.attr('data-aid');
                     ty = self.attr('data-ty'),
                	$.getJSON('/index.php/club-del_hwaterfall_ajax-'+id+'-'+ty+'.html', {}, function(result){
                    if(result.status == 1){
                        self.parents(s.item_unit).slideUp(500, function(){
                            $(this).parent().masonry('remove', $(this)).masonry('reload');
                        });
        ...
        select_album: function(){
        	return;
            var s = $.rc.item.settings;
            $(s.select_album).find('.J_qcreate_btn').bind('click', function(){
                $(this).hide();
                $(s.select_album).find('.J_qcreate').show();
            });
            //点击创建
            $(s.select_album).find('.J_qc_submit').bind('click', function(){
        ...
        joinalbum: function(id){
        	return;
            var s = $.rc.item.settings;
            $(s.joinalbum_btn).bind('click', function(){
                if(!$.rc.dialog.islogin()) return !1;
                ...
                $.getJSON(PINER.root + '/?m=album&a=join', {id:id}, function(result){
                    if(result.status == 1){
                        ...
                        $.rc.item.joinalbum_form($('#J_join_album'));
                    }else{
                        $.rc.tip({content:result.msg, icon:'error'});
                    }
                });
        ...
        share: function(){
        	return;
            var s = $.rc.item.settings;
            $(s.share_btn).bind('click', function(){
                if(!$.rc.dialog.islogin()) return !1;
                ...
                $.getJSON(PINER.root + '/?m=item&a=share_item', function(result){
                    if(result.status == 1){
                        ...
                        $.rc.item.share_form($('#J_share_item'), aid);
                    }else{
                        $.rc.tip({content:result.msg, icon:'error'});
                    }
                });
        ...
        share_form: function(form, aid){
            ...
            form.find('.J_si_btn').die('click').live('click', function(){
                ...
                if(!$.rc.util.isURl(si_url)) return $.rc.tip({content:lang.please_input+lang.correct_itemurl, icon:'alert'}), !1;
        ...
        fetch: function(result){
            ...
            $.rc.item.fetch_form($('#J_fetch_item'), result.item);
        },
