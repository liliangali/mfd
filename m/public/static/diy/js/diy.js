...
function Custom(options){
	...
	this.shareUrl=false;//app分享地址
	this.api='http://api.diy.cotte.cn/v2/';
	this.log=true;//log开关
	this.cache=this.checkDomain()==1?"":"";//控制ajax全局缓存
	this.csql=this.checkDomain()==1?"":"";
...
*=========================*/
	init:function(data){
		_diy=this;
		//来源标记
		location.search.indexOf('source=app')>=0?_diy.source='app':_diy.source='wap';
		//匹配接口地址
		switch(_diy.checkDomain()){
			case 1:
				_diy.api='http://api.diy.cotte.cn/v2/';//线上环境
			break;
			case 2:
				_diy.api='http://api.diy.test.cotte.cn/v2/';//测试环境
			break;
			case 3:
				_diy.api='http://api.diy.dev.cotte.cn:8080/v2/';//开发环境
		}
		// 头部显示
		if(_diy.source=='wap'){
			$('.topbar').show();
		...
		//截取参数
		if(window.location.href.indexOf('custom-diy2-')>=0){
			//截取clothing
			var id=window.location.href.split('custom-diy2-')[1].split('-')
			_diy.clothing=id[0];
...
//			var winHeight=$(window).height(),num=winHeight-296;
			var winHeight=$(window).height(),num=winHeight-396;
	...
	getInitData:function(){
		function setInitData(d){
			...
			function upData(g){
				...
				for(var n in data[g]){
					if(n!='nav'){
						smallNav.push('<li class="nav-'+n+data[g].nav.i+" nav-"+n+'" id="nav-'+n+'" data-id="'+data[g][n].i+'"><span><i class="nav-ico"></i>'+data[g][n].n+'</span></li>');
		...
		switch(name){
			case 'fabric':
				return '<div id="opt-fabric">\
							<div class="titBar"><span class="tit fl js-back">'+obj.n+'</span><div class="sousuo"><input type="text" name="so" placeholder="面料搜索"><button>搜索</button></div>\
                                                        </div>\
							<div class="scrollBox">\
								<div class="subCat">\
									<ul class="cat2-1"></ul>\
			...
			case 'embroidery':
				return '<div id="opt-embroidery">\
							<div class="titBar"><span class="tit fl js-back">'+obj.n+'</span><span class="save fr">保存</span><span class="del fr">取消</span></div>\
							<div class="scrollBox">\
								<div class="itemList">\
...
*=========================*/
	...
	selectCat2:function(obj){
		...
		$('.cstrBox').off('click').on('click','li',function(){
			...
			if(menu.attr('data-rt')&&!$(this).hasClass('null')){
	...
	getWeitiao:function(pid,mid,ids,box){
		$.ajax({
			...
			success: function(d){
				 ...
				 * 特殊处理/S
				 */
				if(_diy.param.clothing=='0003'&&pid==50){
					$.each(d.options,function(i,d){
						if(d.selected){
							_diy.upDataCstr(d.parentid,d.id);
						}	
...
*=========================*/
				...
				if(index==0){
					...
					_diy.getFabric(20,{
                        ...
                        $('.sousuo button').click(function(){
                            var fcode=$('.sousuo input').val();
                            if(fcode=='面料号')return;
				var index=0;
				$('#opt-fabric .cat2-1').find('li').eq(0).addClass('cur').siblings().removeClass('cur');
                                     $('#opt-fabric .itemList').html('<div class="loading">加载中...</div>');
                                     _diy.getSoFabric(20,{
                                            flowerID:'',
                                            colorID:'',
	...
	getFabric:function(n,options){
		...
		function addHtml(start,end){
			...
			for(var i=start;i<end;i++){
				...
				html.push('<li class="'+(checkFocus?"cur ":"")+(fabricStatus(data[i].d_s)?"null ":"")+'"'+hd_flag+' data-code="'+data[i].code+'"><img src="'+data[i].imgurl+'" ><p>'+data[i].tname+hd_price+'</p>'+hd_ico+(data[i].code=="SKP025A"||data[i].code=="SAK567A"?"<span class='ico-mt'></span>":"")+fabricStatus(data[i].d_s)+'</li>');//新增免烫面料标签						
	...
	//获取筛选面料数据
	getSoFabric:function(n,options){
		var data=null,page=0,obj=$('#opt-fabric .itemList'),
			color={
				blue:'圣诞、元旦、新年、情人',
				green:'特惠、活动',
		...
		$.ajax({
			url:_diy.api+'fabrics/all',
//			url:'http://api.diy.test.cotte.cn/v2/fabrics/all',
			data:{
				cache:1,
				csql:1,
				fabricid:$('#nav-fabric').attr('data-id'),
...
*=======================================*/
	order:function(){
		...
		$('#bottomSize .amount').click(function(){
			...
			_diy.underOrder(true)	
		});
	},
	underOrder:function(tomeasur){
		_diy.checkLogin(function(){
			var isTomeasur=tomeasur&&(_diy.source=='app');
			if(isTomeasur||tomeasur){
				$('#zhezhaoLayer').show();
			}else{
				_diy.loading.show(2);
			}
			...
			_diy.generatePic(function(n){
				...
				if(n ==_diy.categoryId.length-1){
					if(isTomeasur){//量体前置
						window.location.href="ios:tomeasur?"+decodeURI(JSON.stringify(_diy.param.strData));
						app.tomeasur(JSON.stringify(_diy.param.strData));	
						return	
					}else if(tomeasur){
						...
						//alert('跳转到量体页面');
						//console.log(JSON.stringify(_diy.param.strData))
						return	
					}
					function geturl(){
						switch(_diy.checkDomain()){
							case 1:
								return 'http://shop.diy.cotte.cn';
							break;
							case 2:
								return 'http://shop.diy.test.cotte.cn';
							break;
							case 3:
								return 'http://shop.diy.dev.cotte.cn:8080'
						}	
					}
					$.ajax({
						url:geturl()+'/cart-add.html?token='+_diy.pwd+'&source='+_diy.source,
						type:'POST',
						timeout:20000,
						dataType:"json",
						data:{
						...
						success:function(data){
							if(data.done){
								window.location.href=geturl()+'/cart.html?token='+_diy.pwd+'&source='+_diy.source
							}else{
					...
					success:function(data){
						...
						if(isWorks){
							...
							if(typeof obj=='function'){
								...
								switch(_diy.checkDomain()){
									case 1:
										domain='http://wap.cotte.cn/';
									break;
									case 2:
										domain='http://wap.test.cotte.cn/';
									break;
									default:
										domain='http://wap.dev.cotte.cn:8080/';
								}
								obj(domain+data.url);
							}
						}
...
*=========================*/
	share:function(){//ios
		if(_diy.shareUrl){
			...
			return _diy.shareUrl+'|||'+img[0].replace('.png','_178.png');
		}else{
//			luck.open({
//				content:'请先点击保存在进行分享',
//				btn:['保存','取消'],
...
//					luck.close();
					...
					_diy.save(function(str){
						_diy.shareUrl=str;
						window.location.href='ios:goShare'
//						luck.open({
//							content:'现在可以点击分享了！',
//							yes:function(){
//								luck.close();
...
//			});
	...
	shareAndroid:function(){//Android
		...
		if(_diy.shareUrl){
			app.share(true,_diy.shareUrl+'|||'+img[0].replace('.png','_178.png'));
		}else{
			_diy.loading.show(1);
			_diy.save(function(str){
				_diy.shareUrl=str;
				app.share(true,str+'|||'+img[0].replace('.png','_178.png'));//安卓用
			});
		}
	},
...
*=========================*/
	...
	checkLogin:function(callback){
		if(_diy.source=='app'){
			...
			if(_diy.pwd=='cotte'&&$utk=='cotte'){
				$('#sizeTable,#bottomSize').removeClass('show');
				var obj=$('#loginPop');
				obj.addClass('show');
	...
	prompt:function(options){
		var ran=Math.round(Math.random()*100),
			...
			options.text?html.push('<input type="text" placeholder="'+options.text.tip+'" value="'+(_diy.works.name?_diy.works.name:'')+'">'):'';
			options.textarea?html.push('<textarea placeholder="'+options.textarea.tip+'">'+(_diy.works.desc?_diy.works.desc:'')+'</textarea>'):'';	
			html.push('<div class="layerbtn"><span class="no">取消</span><span class="ok">确定</span></div></div>');	
