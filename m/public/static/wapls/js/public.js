...
var cottewap={
	...
	pageLogin:function(){//登录页面
		...
		$('#getCode').click(function(){
			var $this=$(this),n=60,delay=null,phone=$('.username').val(),reg=/^1[3|5|8|7]\d{9}$/;
			if(!reg.test(phone)){
		...
		$('#loginBtn').click(function(){
			...
			var reg=/^1[3|5|8|7]\d{9}$/,phone=$('.username').val(),validateCode=$('.validateCode').val();
...
//		        if(curKey == 13){
		    		...
		    		$.ajax({
		    			...
		    			data:{user_name:username,uname:validateCode},
		    			datatype:'json',
		    			success: function(e){
		    				r=eval("("+e+")");
		    				if(r.done == 1) {
		...
		if($('#shadowLayer').length<=0){
		 ...
		 $(".save.fr").bind('click',function(){
            ...
            for(var i=0;i<li_length;i++){
                ...
                var info = $('.scrollBox div:eq('+i+') li.cur').html()
                if(typeof(info) !='undefined') {
                    dis_arr[key] = info;
                }else{
                    var name =$('#tab_'+i).html() ;
                    cottewap.popTip('请选择'+name,1500);
                    return false;
                }
            }
...
//						setTimeout(function(){window.location.href="/member-login.html"},1600);
		...
		$('#sizeTable .amount').click(function(){
			...
			var suitid=$('#listid').val();
			var dis =JSON.stringify(dis_arr) ;
			window.location.href='/appoint-meaInfo.html?&id=2&suitid='+suitid;
			//addCart(dis,sid)
		});
