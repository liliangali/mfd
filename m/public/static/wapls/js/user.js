...
var cotteFn={
    ...
    error:function(obj,error){
        ...
        if(errorBox.length>0){
            errorBox.html(t+error+'<img src="http://r.cotte.cn/cotte/pc/images/sjbz.png">').show();
        }else{
            obj.parent().append('<p class="error">'+t+error+'<img src="http://r.cotte.cn/cotte/pc/images/sjbz.png"></p>')
        }
...
//		})
	...
	kuka:function(){
        $('#kuka_submit').click(function(){
			var sn = $('#kuka').val();
			$.post('/kuka-kuka.html',{sn:sn,oid:1},function(res){
				var res = eval("("+res+")");
		//        console.log(res);
				if(res.done){
					alert(res.retval.msg)
					$("#kuka_table").append(res.retval.html)
				}else{
					alert(res.msg)
				}
			});
