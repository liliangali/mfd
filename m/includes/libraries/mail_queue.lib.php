 ...
 * ============================================================================
 * $Id: mail_queue.lib.php 4291 2015-05-30 02:41:45Z gaofei $
 */
...
class MailQueue
{
    ...
    function add($mail_to, $mail_encoding, $mail_subject, $mail_body, $priority)
    {
        ...
        $sql = "INSERT INTO `rc_mail_queue` (mail_to, mail_encoding, mail_subject, mail_body, priority, add_time) ".
    ...
    function clear()
    {
        $sql = "DELETE FROM `rc_mail_queue` WHERE err_num > $this->_errnum OR add_time < " . (gmtime()-$this->_expire);
    ...
    function send($limit = 5)
    {
        ...
        if ($mail_list = ($this->db->getAll($sql)))
        {
            ...
            for ($i=0; $i < $mail_count; $i++)
            {
             ...
             //锁定
            $sql = "UPDATE `rc_mail_queue` SET err_num = err_num + 1, lock_expiry = $lock_expiry WHERE queue_id " . db_create_in($mail_ids);
            $this->db->query($sql);
            ...
            for ($i=0; $i < $mail_count; $i ++)
            {
                ...
                if ($res)
                {
                    $this->db->query("DELETE FROM `rc_mail_queue` WHERE queue_id='" . $mail_list[$i]['queue_id'] . '\'');
                }
                else
                {
                    $error_count ++;
