...
$(function()
{
    ...
    $('img[ectype="flex"]').click(function(){
        ...
        if(status == 'open')
        {
            ...
            $.get("./index.php?app=parttype&act=ajax_cate", {id: id}, function(data){
                if(data)
                {
                    var str = "";
                    var res = eval('('+data+')');
                    for(var i = 0; i < res.length; i++)
                    {
                       ...
                       /* if(res[i].if_show == '1')
                        ...
                        if_show = res[i].c_name;
                        //构造每一个tr组成的字符串，标语添加
                        str+="<tr class='row"+id+"'><td class='align_center w30'><input type='checkbox' class='checkitem' value='"+res[i].cate_id+"' /></td>"+
                        "<td class='node' width='50%'><img class='preimg' src='templates/style/images/treetable/vertline.gif'/>"+src+"<span class='node_name '  fieldname='cate_name' fieldid='"+res[i].cate_id+"' required='1'>"+res[i].cate_name+"</span></td>"+
            "<td class='align_center'><span class=''  fieldname='sort_order' fieldid='"+res[i].cate_id+"' datatype='number'>"+res[i].sort_order+"</span></td>"+
            "<td class='align_center'>"+if_show+"</td>"+
            "<td class='handler'><span><a href='index.php?app=parttype&amp;act=listattr&amp;id="+res[i].cate_id+"'>"+res[i].attr_list+"</a>  |<a href='index.php?app=parttype&amp;act=edittype&amp;id="+res[i].cate_id+"'>"+lang.edit+"</a>  | <a href='javascript:if(confirm(\""+lang.confirm_delete+"\"))window.location=\"index.php?app=parttype&act=droptype&id="+res[i].cate_id+"\";'>"+lang.drop+"</a> </span></td></tr>";
                    }
                    //将组成的字符串添加到点击对象后面
                    pr.after(str);
                    change_background();
            ...
            $.get(selfurl + "&act=ajax_cate", {id: id}, function(data){
                if(data)
                {
                    ...
                    for(var i = 0; i < res.length; i++)
                    {
                        ...
                        {
                           src =  "<img src='templates/style/images/treetable/tv-item.gif' fieldid='"+res[i].cate_id+"'><span class='node_name '  fieldname='cate_name' fieldid='"+res[i].cate_id+"' required='1'>"+res[i].cate_name+"</span>";
                        }
                        if(res[i].add_child)
                        {
                            add_child =  " | <a href='index.php?app=parttype&amp;act=addtype&amp;c_id="+res[i].c_id+"&amp;pid="+res[i].cate_id+"'>"+lang.add_child+"</a>";
                        }
