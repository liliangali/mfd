...
include (ROOT_PATH_mfd . '/uc_client/client.php');
...
//    }
    ...
    /* 注册 */
    function register($user_name, $password, $mobile, $email='', $local_data = array())
    {
        /* 到UCenter注册 */
        $user_id = outer_call('uc_user_register', array($user_name, $password, $mobile, $email));
        if ($user_id < 0)
        {
            switch ($user_id)
            {
...
//        $local_data['email']        = $email;
    ...
    function edit($user_id, $old_password, $items, $force = false)
    {
        ...
        /* 先到UCenter修改 */
        $result = outer_call('uc_user_edit', array($info['user_name'], $old_password, $new_pwd, $new_email,$items['real_name'], $force));
        if ($result != 1)
        {
            switch ($result)
            {
    ...
    /**
    * @deprecated 修改用户资料
    * @param string $username 用户UID
    * @param array $userInfo 要修改的用户资料
    * @return string
    * @see editInfo
    * @version 1.0.0 (2014-11-14)
    * @author gaofei
    */
    function editInfo($user_id, $items)
    {
        $new_pwd = $new_email = '';
        if (isset($items['password']))
        {
        ...
        $data = array( $info['user_name'], json_encode($items) );
        /* 先到UCenter修改 */
        $result = outer_call( 'uc_user_editInfo', $data );
    ...
    /**
    * @deprecated 修改用户密码
    * @param string 用户UID|用户登录名|用户邮箱|用户手机
    * @param array $userInfo 要修改的用户密码
    * @return string
    * @see editInfo
    * @version 1.0.0 (2014-11-14)
    * @author gaofei
    */
    function editpasswd($items)
    {
        /* 先到UCenter修改 */
        $result = outer_call( 'uc_user_editPasswd', $items );
    ...
    function get($flag, $is_name = false)
    {
        ...
        list($user_id, $user_name, $email, $regip,$real_name,$gender,$im_qq,$im_msn,$def_addr,$nickname,$mobile,$integral) = $user_info;

        /* 同步至本地 */
        $this->_local_sync($user_id, $user_name, $email, $regip,$real_name,$gender,$im_qq,$im_msn,$def_addr,$nickname,$mobile,$integral);
    ...
    {
        $result = outer_call('uc_user_checkmobile', array($mobile));
        if ($result < 0)
        {
            switch ($result)
            {
    ...
    //ns add 积分操作
    function add_user_integral($data){
        return outer_call('uc_add_user_integral',array($data));
    }

    //ns add 获取我量的量体数据
    function get_make_figure($user_id,$limit,$conditions=null){
        return outer_call('uc_get_user_make_figure', array($user_id,$limit,$conditions));
    }

    //ns add 删除我量的量体数据
    function drop_user_figure($data){
        return outer_call('uc_drop_user_make_figure', array($data));
    }
    //ns add 通过id获取量体信息
    function get_figure_info($data){
        return outer_call('uc_get_user_figure_info', array($data));
    }
    //ns add 添加我量的量体数据
    function add_user_figure($data){
        return outer_call('uc_add_user_make_figure', array($data));
    }
    //ns add 修改我量的量体数据
    function edit_user_figure($data){
        return outer_call('uc_edit_user_make_figure', array($data));
    }
    
    //Ruesin 获取量体数据列表
    function get_figure_list($user_id,$type){
        return outer_call('uc_get_figure_list', array($user_id,$type));
    }
    
    //ns add 获取订单
    function get_buyer_order($user_id,$limit)
    {
		return outer_call('uc_get_user_order', array($user_id,$limit));
    }
    
    //ns add 获取量体数据
    function get_user_body($user_id,$figure_type,$idfigure){
        return outer_call('uc_get_user_body', array($user_id,$figure_type,$idfigure));
    }

    /* ns add 获取用户地址信息信息 */
    function get_user_address($user_id,$addr_id=null)
    {
        /* 至UCenter取用户 */
        return outer_call('uc_get_user_address', array($user_id,$addr_id));
    }

    /*ns add 添加用户地址*/
    function add_user_address($data){
        return outer_call('uc_add_user_address',array($data));
    }

    /*ns add 修改用户地址*/
    function edit_user_address($data){
        return outer_call('uc_edit_user_address',array($data));
    }

    /* ns add 删除用户地址 */
    function drop_user_address($data){
        return outer_call('uc_drop_user_address',array($data));
    }

    /*ns add 获取收藏列表*/
    function get_user_collect($user_id,$type,$limit){
        return outer_call('uc_get_user_collect',array($user_id,$type,$limit));
    }

    //ns add 获取一个收藏品
    function get_user_collect_info($data){
        return outer_call('uc_get_user_collect_info',array($data));
    }
    //nsa dd 添加一个收藏品
    function add_user_collect($data){
        return outer_call('uc_add_user_collect',array($data));
    }
    
    /*ns add 删除收藏*/
    function drop_user_collect($data){
        return outer_call('uc_drop_user_collect',array($data));
    }
}
 ...
 */
class UcPassportOrder extends BasePassportOrder
{
    //创建订单
    ...
    //保存订单量体数据
    function add_order_figure($data){
        return outer_call('uc_add_order_figure',array($data));
    }
    //保存订单商品
    function add_order_goods($data){
        return outer_call('uc_add_order_goods',array($data));
    }
    
    //获取订单列表
    function getOrderList($data){
        return outer_call('uc_get_order_list',array($data));
    }
    
    //获取订单详情
    function getOrderDetail($sn,$id){
        return outer_call('uc_get_order_detail',array($sn,$id));
    }
    
    //编辑订单信息
    function edit_order_by_params($params,$data){
        return outer_call('uc_edit_order_by_params',array($params,$data));
    }
    ...
    function begin(){
        return outer_call('uc_order_begin');
    }
    function rollback(){
        return outer_call('uc_order_rollback');
    }
    function commit(){
        return outer_call('uc_order_commit');
    }
}
