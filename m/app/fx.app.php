...
class WorkApp extends MallbaseApp
{
    ...
    function fx()
    {
        ...
        $fx_info = $_order_serve_mod->get(array(
            ...
            'fields'=>'store_name,goods_name,address,sign,liangti_id,liangti_name,user_id,serve_id'
        ));
    ...
    function fx_add(){
        $order_id =$_REQUEST['order_id']?$_REQUEST['order_id']:'';
        $wl_sn = $_REQUEST['reason']?$_REQUEST['reason']:'';
        $to_time = $_REQUEST['to_time']?$_REQUEST['to_time']:'';
        $user_info = $this->visitor->get();
        ...
        $fx_info = $_order_serve_mod->get(array(
            'conditions'=>"type=1 and order_id={$order_id} ",
            'fields'=>'address,rec_id,sign,user_id'
        ));
       ...
       if($fx_info['user_id'] !=$user_info['user_id']){
        ...
        $ret = $_order_serve_mod->edit("order_id='{$order_id}'",array('wl_sn'=>$wl_sn,'reason'=>$reason,'to_time'=>$to_time,'status'=>1));


        $_order_serve_info_mod = & m('orderserveinfo');
        $ret = $_order_serve_info_mod->edit('order_id="'.$order_id.'"',array('info_status'=>1));
                    ...
                    if($k=="cx"){
                        foreach($v as $kk=>$vv){
                            foreach($cx_info as $kkk=>$vvv){
                                ...
                                if($vvv['name']==$kk){
                                    if($kk=="颜色"){
                                        $user_fx_info[$ok][$k][$kk] =$color_status[$vvv['list'][$vv]['name']];
                                    }else{
                                        $user_fx_info[$ok][$k][$kk] = $vvv['list'][$vv]['name'];
                                    }
                                }elseif($vvv['name']=='内容'){
                                    $content = explode(":",$v['content']);

                                    $user_fx_info[$ok][$k]['内容'] = $content[1];
                                }
                            }
                        }
                    }
    ...
    function fx_status_list(){
        ...
        foreach($fx_info as $k=>$v){
            ...
            //加入口
            $fx_order[$v['order_id']]['if_receive'] =$v['status']==9?1:0;
        ...
        foreach($order_info as $k=>$v){
            ...
            $fx_order[$v['order_id']]["price"] =0;

        }

        $this->assign("fx_info",array_values($fx_order));
    }
    ...
    function fx_receive(){
        ...
        if(!$fx_info){
            $this->show_warning("当前用户不能对此订单做返修完成操作！");
            return;
        }

        if($fx_info['status'] <7){
            $this->show_warning("还未发货，不能操作返修订单已完成！");
            return;
        }
        $ret = $_order_serve_info_mod->edit("order_id='{$order_id}' and type=1",array("info_status"=>8));
        $ret && $ret = $_order_serve_mod->edit("order_id='{$order_id}' and type=1",array('status'=>8));
