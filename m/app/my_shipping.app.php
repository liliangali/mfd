...
{
	var $default_shipping = array(
        			'logis' => array('shipping_name' => '物流配送','status' => 1),
        			'self'  => array('shipping_name' =>'上门自提', 'status' => 1),
        			'other' => array('shipping_name' =>"其它方式", 'status' => 1)
        		);
    function index()
    {
        /* 取得列表数据 */
        ...
        foreach($shippings as $key => $val){
        	$shipping_list[$val['code']] = $val;
        }
        

        
        foreach($this->default_shipping as $key => $val){
        	if(isset($shipping_list[$key])){
        		$this->default_shipping[$key] = $shipping_list[$key];
        	}
        }
        
       // print_R($this->default_shipping);
        
        $this->assign('shippings', $this->default_shipping);
    ...
    function add()
    {
        ...
        {
        	$code = trim($_GET['code']);
        	if(!isset($this->default_shipping[$code])){
        		    echo "非法操作!";  
        			return;
        	}
            ...
            $this->assign('shipping', $this->default_shipping[$code]);
