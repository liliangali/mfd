...
// 	    print_exit($user);
	    ...
	    //使用余额 麦富迪币 //收益
	    $pmt = array('coin','money');
	    //=====  验证支付密码  =====
	    if ($_POST['pmt'])
	    {
	        $pay_pass = $_POST['pay_password'];
	        if (!$pay_pass)
	        {
	            $this->json_error('请输入支付密码');
	            return;
	        }
	        if (md5($pay_pass) != $user['pay_password'])
	        {
	            $this->json_error('支付密码输入错误');
	            return;;
	        }
	    }
	
	    unset($_SESSION['_order']['pmt']);
	    foreach ((array)array_filter(explode(',',$_POST['pmt'])) as $row){
	        $cn = array();
	        $cn = explode(':',$row);
	        if(!in_array($cn[0], $pmt)){
	            $this->json_error('抵用使用失败!');
	            return;
	        }
	        if($user[$cn[0]] && $user[$cn[0]] >= floatval($cn[1])){
	            $_SESSION['_order']['pmt'][$cn[0]] = floatval($cn[1]);
	        }else{
	            $this->json_error('抵用使用失败!');
	            return;
	        }
	    ...
	    //校验库存
// 	    $fCart = $this->_cart_main_format($aCart);
...
// 	    print_exit($oInfo);
	    ...
	    if($_order['debit']['data']){
	        ...
	        $mDebit = &m('debit');
	        $mDebit->edit(db_create_in($dIds,'id'), array('is_used'=>'1'));
	    ...
	    $msg = new Notice();
	    $msg->send(array(
	        "content" => "{$_order['user']['user_name']}-新增订单（ID-{$oInfo['order_sn']}）<a href=\"/admin/index.php?app=order&act=view&id={$oInfo['order_id']}\">点击查看</a>",
	        'node'    => 'order',
	    ));
	    // dump($oInfo);
	    // 订单生成成功清理购物车
    ...
    function _clear()
    {
        $this->_mod_cart->drop(" user_id = '{$this->_user_id}' AND " . db_create_in($_SESSION['_cart']['check'],'ident'));
...
// dump($sn);
        ...
        if ($order['payment_code'] == 'wxpay') 
        {
            ...
            $input->SetGoods_tag("麦富迪");
            $input->SetNotify_url('http://'.$_SERVER['SERVER_NAME'].":".$_SERVER['SERVER_PORT']."/paynotifyw-index-{$order['order_sn']}.html");
