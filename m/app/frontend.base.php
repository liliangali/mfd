...
class FrontendApp extends ECBaseApp
{
    ...
    {
        parent::_config_view();
        $this->_view->template_dir  = ROOT_PATH . '/m/view/'.LANG.'/';
        $this->_view->compile_dir   = ROOT_PATH . '/m/temp/compiled/mall';
        $this->_view->res_base      = SITE_URL . '/m/view/'.LANG.'/';
...
class MallbaseApp extends FrontendApp
{
    ...
    function _config_view()
    {
        ...
        $this->_view->compile_dir  = ROOT_PATH . "/m/temp/compiled/mall/{$template_name}";
    ...
    {
        include_once(ROOT_PATH . '/m/includes/payment.base.php');
        include(ROOT_PATH . '/m/includes/payments/' . $code . '/' . $code . '.payment.php');
...
class ShoppingbaseApp extends FrontendApp
{
	...
	function __construct()
	{
		...
		//token做识别
		$this->_source_from = (isset($_REQUEST['token']) && !empty($_REQUEST['token'])) ? 'app' : 'wap';
	}
...
//         var_dump($this->visitor->has_login);exit();
    ...
    function _order_info(){
    	...
    	if(!$order['ships']){
    	    ...
    	    imports("orders.lib");
    	    $orderLib = new Orders();
    	    $shipInit = $orderLib->getShipInit(['ship_area_id'=>$order['shipping']['address']['region_id'],'weight'=>'']);
...
class MemberbaseApp extends MallbaseApp
{
    ...
    function _config_view()
    {
        ...
        $template_name = $this->_get_template_name();
        $style_name    = $this->_get_style_name();
        $this->_view->template_dir = ROOT_PATH . "/m/view/".LANG."/mall/{$template_name}/user";
        $this->_view->compile_dir  = ROOT_PATH . "/m/temp/compiled/mall/{$template_name}/user";
        $this->_view->res_base     = SITE_URL . "/m/view/".LANG."/mall/{$template_name}/styles/{$style_name}";
    }
