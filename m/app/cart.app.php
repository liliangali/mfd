...
//        print_exit($main);
    ...
    function clear(){
    	...
    	$droped_rows = $this->_mod_cart->drop(db_create_in($_SESSION['_cart']['check'] , 'ident') . " AND user_id = '".$this->_user_id."'");
    	if (!$droped_rows)
    	{
...
//     	$aCart = $this->_total($_order);
    	$aCart = $this->_class_carts->main(['source_from'=>$this->_source_from,'check'=> (array)$_SESSION['_cart']['check']]);
    	
    	if(!$aCart['object']){
    	    $view = &v(); 
    	    $url = $view->build_url(array("app" => "cart"));
    	    header('Location:'.$url.'?token='.$_REQUEST['token'].'&source='.$this->_source_from);
    		return;
    	}
...
//     	if ($aCart['weight']){
    	    imports("orders.lib");
    	    $orderLib = new Orders();
    	    $_SESSION['_order']['ships']['defship']['post_fee'] = $orderLib->freightCnt($aCart['weight'],$_SESSION['_order']['ships']['defship']);
    	    if ($aCart['free_shipping']){//免运费
    	        $_SESSION['_order']['ships']['defship']['post_fee'] = 0.00;
    	    }
...
//     	print_exit($aCart);
    	...
    	//商品不允许使用抵扣券
//     	if($aCart['has_activity'] == 'yes' || $aCart['has_special'] == 'yes'){
...
//     	$aCart['debit_fee']
    	...
    	foreach ($aCart['object'] as $row){
    	    ...
    	    if($row['list']){
    	        foreach ($row['list'] as $arr){
    	            foreach ((array)explode(',', $arr['cloth']) as $val){
    	                $cls[] = $val;
    	            }
    	        }
    	    }else{
    	...
    	$mDbt = &m('debit');
    	$time = gmtime();
    	$debits = $mDbt->find(" user_id = '".$this->_user_id."' AND is_used = '0' AND is_invalid = '0' AND expire_time >'{$time}' AND (debit_t_id ={$cls[0]} or cate in (".implode(',',$cls)."))");
    ...
    function addressSave(){
        ...
        $_SESSION['_order']['address'] = $mAddr->get($id);
    ...
    function debit(){
    	...
    	foreach ((array)$cart['object'] as $item){
    	    foreach ((array)$item['list'] as $row){
    	        foreach ((array)explode(',', $row['cloth']) as $v){
    	            $cls[] = $v;
    	        }
    	    }
    	}
