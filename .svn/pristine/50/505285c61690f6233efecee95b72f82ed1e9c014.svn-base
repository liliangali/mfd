...
//         echo "<br>\n";
    ...
    function edit($conditions, $edit_data)
    {
        ...
        //=====  写入更新文件 End  =====
		$this->operation_log($conditions,'edit');
     ...
     *
     */
    protected function operation_log($conditions,$type){
    	if (defined('IN_BACKEND'))
    	{
    		...
    		if (!$visitor->info['user_id']){
    			$sql = "SELECT * FROM {$this->_prefix}member $conditions";
    			$row = $this->getRow($sql);
    				...
    				foreach ($v as $key=>$r){
    				    ...
    				    if(is_array($r)){
    				    	foreach ($r as $kk=>$rr){
    				    	    if(!is_array($rr)){
    				    	        if (strstr($rr,APP)){
    				    	            $logdata['operate_key'] = $GLOBALS['__ECLANG__'][$kk];
    				    	            break;
    				    	        }
    				    	    }
    				    	}
    				        ...
    				        if (strstr($r,APP)){
    				            $logdata['operate_key'] = $GLOBALS['__ECLANG__'][$key];
    				            break;
    				        }
    				    }
...
//     					exit;
    		...
    		if ($logdata['operate_key'] == '后台登录') $typedata['edit'] = '登录';
    		...
    		$this->db->query("INSERT INTO {$this->_prefix}operatorlog_logs  {$insert_info['fields']} VALUES{$insert_info['values']}");
    	}
    ...
    function drop($conditions, $fields = '')
    {
        ...
        $this->operation_log($conditions,'drop');
        return $affectedRows;
    }
    /**
     *  硬性删除 谨用
