...
class DiyGoods extends BaseGoods
{
    ...
    function add($post = array())
    {
        ...
        foreach ($post['params'] as $key=>$row){
            ...
            if (isset($stk[$row['fabric']][$key])){
                $stk[$row['fabric']][$key]['quantity'] += $post['quantity'];
            }else{
        ...
        $data = array(
                ...
                'dis_ident'    => $dis_ident ? $dis_ident : $ident,
                'suit_id'      => 0,
                'goods_id'     => 0,
                'type'         => $this->_name,
                'quantity'     => $post['quantity'],
    ...
    function update($post){
        ...
        if ($this->_check_stock($stk) !== true){
            $this->_error($this->get_error());
            return false;
        }
        
    
        $where = " dis_ident = '".$post['ident']."' AND user_id = '".$post['user_id']."' AND first = '' ";
