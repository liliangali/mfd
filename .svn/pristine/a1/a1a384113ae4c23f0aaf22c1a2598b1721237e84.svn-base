 ...
 */
define('MAX_LAYER', 4);
class JpjzpartApp extends BackendApp
{
    var $_part_mod;
	var $part_type;
    ...
    function index() {
       ...
       foreach ($part as $key=>$val){
          /*  $part[$key]['img_url'] =  isset($val['img_url']) ?  SITE_URL.'/upload/jipaijizuo/'.$val['img_url'] : '';
           $part[$key]['img_url2'] = isset($val['img_url2']) ?  SITE_URL.'/upload/jipaijizuo/'.$val['img_url2'] : '';
           $part[$key]['img_url3'] = isset($val['img_url3']) ?  SITE_URL.'/upload/jipaijizuo/'.$val['img_url3'] : '';
           $part[$key]['img_url4'] = isset($val['img_url4']) ?  SITE_URL.'/upload/jipaijizuo/'.$val['img_url4'] : ''; */
           if($part[$key]['img_url'])
           {
                $part[$key]['img_url']=  SITE_URL.'/upload/jipaijizuo/'.$part[$key]['img_url'];
           }
           if($part[$key]['img_url2'])
           {
               $part[$key]['img_url2']=  SITE_URL.'/upload/jipaijizuo/'.$part[$key]['img_url2'];
           }
           if($part[$key]['img_url3'])
           {
               $part[$key]['img_url3']=  SITE_URL.'/upload/jipaijizuo/'.$part[$key]['img_url3'];
           }
           if($part[$key]['img_url4'])
           {
               $part[$key]['img_url4']=  SITE_URL.'/upload/jipaijizuo/'.$part[$key]['img_url4'];
           }
       }
    	...
    	$this->display(PART."jpjzpart.index.html");
    }
    ...
    function searchdata(){
        ...
        import("toMTM.lib");
        $mtm = new toMTM();
        $fabric = trim($_GET['fabric']);
    
        if(!$fabric){
            $this->json_error('非法操作');
            die();
        }
        $res = $mtm->fabric($fabric);
    ...
    function partlining() {
    	...
    	$this->display(PART."jpjzpartlining.index.html");
    }
    ...
    function partmat() {
    	...
    	$this->assign("page_info",$page);
    	$this->assign("store_list",$this->_part_mod->get_store());
    	$this->display(PART."jpjzpartmat.index.html");
    }
    ...
    function partstyle()
    {
    	...
    	$this->assign("parents",$this->_get_options());
    	$this->assign("page_info",$page);
    	$this->display(PART."jpjzpartstyle.index.html");
    }
    ...
    function add()
    {
    	...
    	if (!IS_POST)
    	{
    		...
    		$this->assign("parents",$this->_get_options());
    		$this->display(PART.'jpjzpart.form.html');
    	} 
    	else 
    	{
    	    // 图片上传类
    		...
    		foreach ($_FILES as $k=>$v){
    		    ...
    		    if($k=='url'||$k=='url2'||$k=='url3'||$k=='url4')
    		    {
    		        ...
    		        if($upfile[$k])
    		        {
    		            ...
    		            }elseif($k=='url2')
    		            {
    		                ...
    		                $upfile['img_url2']=$goods_thumb;
    		            }elseif($k=='url3')
    		            {
    		
    		                $upfile['img_url3']=$goods_thumb;
    		            }elseif($k=='url4'){
    		                
    		                $upfile['img_url4']=$goods_thumb;
    		            }
    		        }
    		    ...
    		    else
    		    {
    		        ...
    		        if($upfilegallery[$k])
    		        {
    		             $this->_part_mod->add(array('img_url'=>$upfilegallery[$k]));
    		        }
    		    }
    		}
    		
    		;
    		$id=$this->_part_mod->edit($part_id,array('img_url'=> $upfile['url'],'img_url2'=>$upfile['url2'],'img_url3'=> $upfile['url3'],'img_url4'=> $upfile['url4']));
    		//结束
    		
    		
    		
    		$this->show_message('add_zujian_successed',
    				'back_list', 'index.php?app=jpjzpart&amp;act=index'
    		);
    	}
    }
    ...
    function addlining()
    {
    	...
    	if (!IS_POST)
    	{
    		...
    		$this->display(PART.'jpjzpartlining.form.html');
    	}
    	else
    	{
    		...
    		$this->show_message('add_zujian_successed',
    				'back_list', 'index.php?app=jpjzpart&amp;act=jpjzpartlining&amp;cate_id='.$data['cate_id']
    		);
    	}
    }
    ...
    function addmat()
    {
    	if (!IS_POST)
    	{
    		...
    		$this->assign("parents",$this->_get_options());
    		$this->assign("zujiantype",$this->part_type);
    		$this->display(PART.'jpjzpartmat.form.html');
    	}
    	else
    	{
    		...
    		$this->show_message('add_zujian_successed',
    				'back_list', 'index.php?app=jpjzpart&amp;act=jpjzpartmat&amp;cate_id='.$data['cate_id']
    		);
    	}
    }
    ...
    function addstyle()
    {
    	...
    	if (!IS_POST)
    	{
    		...
    		$this->display(PART.'jpjzpartstyle.form.html');
    	}
    	else
    	{
    		$data['part_name'] = $_POST['goods_name'];
    		...
    		$this->show_message('add_zujian_successed',
    				'back_list', 'index.php?app=jpjzpart&amp;act=jpjzpartstyle&amp;cate_id='.$data['cate_id']
    		);
    	}
    }
    ...
    function edit(){
    	...
    	if (!IS_POST){
    		...
    		if (substr($part_info['part_img'],0,4) != "http"){
    		    ...
    		    if($part_info['part_img'])
    		    {
    		        $part_info['part_img'] = SITE_URL.'/'.$part_info['part_img'];
    		    }
    		}
    	    if(substr($part_info['img_url'], 0,4) !="http"){
    	        if($part_info['img_url'])
    	        {
                    $part_info['img_url'] = SITE_URL.'/upload/jipaijizuo/'.$part_info['img_url'];
    	        }
            }
            if(substr($part_info['img_url2'], 0,4) !="http"){
                if($part_info['img_url2'])
                {
                    $part_info['img_url2'] = SITE_URL.'/upload/jipaijizuo/'.$part_info['img_url2'];
                }
            }
            if(substr($part_info['img_url3'], 0,4) !="http"){
                if($part_info['img_url3'])
                {
                   $part_info['img_url3'] = SITE_URL.'/upload/jipaijizuo/'.$part_info['img_url3'];
                }
            }
           if(substr($part_info['img_url4'], 0,4) !="http"){
               if($part_info['img_url4'])
               {
                    $part_info['img_url4'] = SITE_URL.'/upload/jipaijizuo/'.$part_info['img_url4'];
               }
           }
    		...
    		$this->display(PART.'jpjzpart.edit.html');
    	}else{
            ...
            if($upfile['url']){
                $id=$this->_part_mod->edit($part_id,array('img_url'=> $upfile['url']));
            }
            if($upfile['url2']){
                $id=$this->_part_mod->edit($part_id,array('img_url2'=> $upfile['url2']));
            }
            if($upfile['url3']){
                $id=$this->_part_mod->edit($part_id,array('img_url3'=> $upfile['url3']));
            }
            if($upfile['url4']){
                 $id=$this->_part_mod->edit($part_id,array('img_url4'=> $upfile['url4']));
            }
    ...
    function ajax_customs_info(){
        ...
        $this->display(PART.'jpjzpart.fratc.html');
    }
    ...
    function editlining()
    {
    	if (!IS_POST)
    	{
    		...
    		$this->assign("goods_attr_html",$this->build_attr_html($part_info['type_id'],$part_info['part_id']));
    		$this->assign("zujiantype",$this->_get_options1($cate_id));
    		$this->display(PART.'jpjzpartlining.edit.html');
    	}
    	else
    	{
    		$data['part_name'] = $_POST['goods_name'];
    		...
    		$this->show_message('edit_zujian_successed',
    				'back_list', 'index.php?app=jpjzpart&act=jpjzpartlining&amp;cate_id='.$cate_id
    		);
    	}
    }
    ...
    function editmat()
    {
    	if (!IS_POST)
    	{
    		...
    		$this->assign("goods_attr_html",$this->build_attr_html($part_info['type_id'],$part_info['part_id']));
    		$this->assign("zujiantype",$this->_get_options1($cate_id));
    		$this->display(PART.'jpjzpartmat.edit.html');
    	}
    	else
    	{
    		...
    		$this->show_message('edit_zujian_successed',
    				'back_list', 'index.php?app=jpjzpart&act=jpjzpartmat'
    		);
    	}
    }
    ...
    function editstyle()
    {
    	...
    	if (!IS_POST)
    	{
    		...
    		$this->assign("goods_attr_html",$this->build_attr_html($part_info['type_id'],$part_info['part_id']));
    		$this->assign("zujiantype",$this->_get_options1($cate_id));
    		$this->display(PART.'jpjzpartstyle.edit.html');
    	}
    	else
    	{
    		$data['part_name'] = $_POST['goods_name'];
    		...
    		$this->show_message('edit_zujian_successed',
    				'back_list', 'index.php?app=jpjzpart&act=jpjzpartstyle&amp;cate_id='.$cate_id
    		);
    	}
    }
   ...
   function check_part_sn_name()
   {
    ...
    function drop()
    {
            ...
            $this->show_message('drop_ok',
                'back_list', 'index.php?app=jpjzpart&act='.$del_act.'&cate_id='.$cate_id);
...
//     	        if($key == 3)break;
    ...
    function _upload_logo($part_id,$file)
    {
    	...
    	{
    		$this->show_warning($uploader->get_error() , 'go_back', 'index.php?app=jpjzpart&amp;id=' . $part_id);
    		return false;
    	}
    	/* 指定保存位置的根目录 */
