...
/* 多级选择：地区选择，分类选择 , 宠物种类分类*/
class MlselectionApp extends MallbaseApp
{
    function index()
    {
        in_array($_GET['type'], array('region', 'gcategory','pettype')) or $this->json_error('invalid type');
        ...
        switch ($_GET['type'])
        {
            ...
            case 'gcategory':
                ...
                break;
			case 'pettype' :
				$mod_pettype = & m ( 'pettype' );
				$types = $mod_pettype->get_list ( $pid );
				foreach ( $types as $key => $type ) {
					$types [$key] ['type_name'] = htmlspecialchars ( $type ['type_name'] );
				}
				$this->json_result ( array_values ( $types ) );
				break;
        }
    }
