...
class Xinge {
	...
	public function  PushSingleDevice($deviceToken, $message, $environment=0)
	{
		...
		if (!($message instanceof MessageAndroid) && !($message instanceof MessageIOS)) return $ret;
		...
		if($message instanceof MessageAndroid) $params['multi_pkg'] = $message->getMultiPkg();
	...
	public function  PushSingleAccount($deviceType, $account, $message, $environment=0)
	{
		...
		{
			if($environment!=Xinge::IOSENV_DEV && $environment!=Xinge::IOSENV_PROD)
			{
