...
class SgradeApp extends BackendApp
{
    ...
    function index()
    {
        ...
        $sgrades = $this->_grade_mod->find(array(
            ...
            //ns up 获取默认的。现在没有品牌一说
            'conditions' => 'grade_id=1' . $conditions,
            'limit' => $page['limit'],
            'count' => true,
            'order' => 'sort_order',
        ));
    ...
    function _get_skins()
    {
        ...
        $layout_dir = ROOT_PATH . '/view/sc-utf-8/store/';
                            ...
                            if ($sdh = opendir($skin_dir))
                            {
                                while (($sfile = readdir($sdh)) !== false)
                                {
                                    if ($sfile[0] != '.' && filetype($skin_dir . $sfile) == 'dir')
                                    {
                                        $skins[] = array(
                                            'value'     => $lfile . '|' . $sfile,
                                            'preview'   => 'view/sc-utf-8/store/' . $lfile . '/styles/' . $sfile . '/preview.jpg',
                                            'screenshot'=> 'view/sc-utf-8/store/' . $lfile . '/styles/' . $sfile . '/screenshot.jpg',
                                        );
                                    }
                                }
                                closedir($sdh);
