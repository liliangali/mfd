	...
	pageReg:function(){//注册页面
		...
		$('#getCode').click(function(){
			...
			$.ajax({
					...
					data:"phone="+phone+"&type=reg&category=reg",
					datatype:'json',
					success:function(e){
						//alert(phone);
						r=eval("("+e+")");
		...
		$('#regBtn').click(function(){
			var reg=/^1[3|5|8|7]\d{9}$/,phone=$('.phone').val(),invite=$('#invite').val(),validateCode=$('.yanzm').val(),pwd1=$('.userpwd1').val(),pwd2=$('.userpwd2').val();
				...
				success: function(res){
					...
					if(!res.done){
						...
						return
							...
							//bd
							if(res.retval.content ==1){

								luck.open({
										title:['提示'],
										content: '注册成功,<br/>下载麦富迪APP',
										btn:['以后再说','前往下载'],
										yes:function(){
											luck.close()
										...
										no: function(){
											luck.close()
											window.location.href = 'http://m.mfd.p.day900.com/default-down.html';
										}
	...
	pageFindPwd:function(){//找回密码
		...
		$('#getCode').click(function(){
			...
			$.ajax({
					...
					data:{phone:phone,type:'findps',category:'findps',opt:'get'},
		...
		$('#okBtn').click(function(){
			...
			var reg=/^1[3|5|8|7]\d{9}$/,phone=$('.phone').val(),validateCode=$('.yanzm').val();
			if(!reg.test(phone)){
				cottewap.popTip('请输入正确的手机号码',2000);
				return
			}
			...
			$.ajax({
				...
				type: "POST",
				dataType: "json",
				data: {account_type:'phone',account:phone,opt:2,authcode:validateCode},
				success: function(res){
					if(!res.done){
						cottewap.popTip(res.msg,2000);
						return
