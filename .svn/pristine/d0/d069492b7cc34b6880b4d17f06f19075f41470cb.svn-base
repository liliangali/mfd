...
class MailQueue
{
    ...
    function add($mail_to, $mail_encoding, $mail_subject, $mail_body, $priority)
    {
        ...
        $sql = "INSERT INTO `cf_mail_queue` (mail_to, mail_encoding, mail_subject, mail_body, priority, add_time) ".
    ...
    function clear()
    {
        $sql = "DELETE FROM `cf_mail_queue` WHERE err_num > $this->_errnum OR add_time < " . (gmtime()-$this->_expire);
    ...
    function send($limit = 5)
    {
        ...
        if ($mail_list = ($this->db->getAll($sql)))
        {
            ...
            for ($i=0; $i < $mail_count; $i++)
            {
             ...
             //锁定
            $sql = "UPDATE `cf_mail_queue` SET err_num = err_num + 1, lock_expiry = $lock_expiry WHERE queue_id " . db_create_in($mail_ids);
            $this->db->query($sql);
            ...
            for ($i=0; $i < $mail_count; $i ++)
            {
                ...
                if ($res)
                {
                    $this->db->query("DELETE FROM `cf_mail_queue` WHERE queue_id='" . $mail_list[$i]['queue_id'] . '\'');
                }
                else
                {
                    $error_count ++;
