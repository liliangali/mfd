...
class My_favoriteApp extends MemberbaseApp
{
    ...
    function index()
    {
        ...
        $collect_custom = $model_collect->find(array(
            'conditions' => "user_id = " . $this->visitor->get('user_id') ." and type='goods' ",
            'count' => true,
		...
		//获取同分类下的相关商品
        $link_goods = $custom_mod->find(array(
        ...
        $this->_config_seo('title', '我的麦富迪 - 我的收藏');
    ...
    function dropCollect(){
        ...
        $collect_mod->drop("item_id=".$item_id." and user_id=".$this->visitor->get('user_id'));
