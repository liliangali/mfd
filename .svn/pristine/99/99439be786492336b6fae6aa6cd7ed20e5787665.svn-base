    ...
    $('img[ectype="flex"]').click(function(){
        ...
        if(status == 'open')
        {
          ...
          "<td class='align_center'>"+res[i].sort_order+"</td>"+
            ...
            "<td class='handler'><span><a href='index.php?app=jpjz_gcategory&amp;act=edit&amp;id="+res[i].cate_id+"'>"+lang.edit+"</a>  | <a href='javascript:if(confirm(\""+lang.confirm_delete+"\"))window.location=\"index.php?app=jpjz_gcategory&act=drop&id="+res[i].cate_id+"\";'>"+lang.drop+"</a> | <a href='index.php?app=jpjz_gcategory&amp;act=add&amp;pid="+res[i].cate_id+"'>"+lang.add_child+"</a></span></td></tr>";
                    }
                    //将组成的字符串添加到点击对象后面
                    pr.after(str);
                    change_background();
...
function secajax(ob)
{
    ...
    var src = $(ob).attr('src');
        if(status == 'open')
        {
            ...
            $.get(selfurl + "&act=ajax_cate", {id: id}, function(data){
                if(data)
                {
                    ...
                    for(var i = 0; i < res.length; i++)
                    {
                        ...
                        if(res[i].add_child)
                        {
                            add_child =  " | <a href='index.php?app=jpjz_gcategory&amp;act=add&amp;pid="+res[i].cate_id+"'>"+lang.add_child+"</a>";
                        }
           ...
           "<td class='align_center'>"+res[i].sort_order+"</td>"+
            "<td class='align_center'>"+if_show+"</td>"+
            "<td class='handler'><span><a href='index.php?app=jpjz_gcategory&amp;act=edit&amp;id="+res[i].cate_id+"'>"+lang.edit+"</a>  | <a href='javascript:if(confirm(\""+lang.confirm_delete+"\"))window.location =\"index.php?app=jpjz_gcategory&amp;act=drop&amp;id="+res[i].cate_id+"\";'>"+lang.drop+"</a>" + add_child + "</span></td></tr>";
                    }
                    pr.after(str);
                    change_background();
                    $('span[ectype="inline_edit"]').unbind('click');
