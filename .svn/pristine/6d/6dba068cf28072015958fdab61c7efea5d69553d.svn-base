...
class Seller_orderApp extends StoreadminbaseApp
{
    ...
    function index()
    {
        $this->assign('title','RCTAILOR-酷客中心-订单管理');
        $this->assign('title_info','订单管理');
        /* 获取订单列表 */
        $this->_get_orders();

        /* 当前位置 */
        $this->_curlocal(LANG::get('member_center'),    '/index.php/member.html',
                         LANG::get('order_manage'), '/index.php/seller_order.html',
                         LANG::get('order_list'));
    ...
    function shipped()
    {
        ...
        else
        {
			...
			$code = $this->getCode(10);//验证码
			$smsText = "您的订单：".$order_info['order_sn'].",卖家：".$order_info['seller_name']."已经发货，验证码【".$code."】请及时查收！";//内容
			$time = time();
    ...
    function cancel_order()
    {
        ...
        else
        {
            ...
            foreach ($ids as $val)
            {
                ...
                if($info['status'] == ORDER_ACCEPTED && $info['payment_code'] != 'cod'){
                	$log = array(
                			'dateline' => gmtime(),
                			'remark'   => "取消订单退款",
                	...
                	$field = "money=money+{$info['order_amount']}";
    ...
    function _get_member_submenu()
    {
        $array = array(
            array(
                'name' => 'all_orders',
                'url' => '/index.php/seller_order.html?type=all_orders',
            ),
            array(
                'name' => 'pending',
                'url' => '/index.php/seller_order.html?type=pending',
            ),
            array(
                'name' => 'submitted',
                'url' => '/index.php/seller_order.html?type=submitted',
            ),
            array(
                'name' => 'accepted',
                'url' => '/index.php/seller_order.html?type=accepted',
            ),
            array(
                'name' => 'shipped',
                'url' => '/index.php/seller_order.html?type=shipped',
            ),
            array(
                'name' => 'finished',
                'url' => '/index.php/seller_order.html?type=finished',
            ),
            array(
                'name' => 'canceled',
                'url' => '/index.php/seller_order.html?type=canceled',
        ),
        );
        return $array;
    }
