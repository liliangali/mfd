...
class Page {
	...
	function __construct($mTotalData, $mShowPageNum = 20 , $page = 1 ){
...
// 			$this->mShowHTML = $this->getEverypageModule();
	...
	function getPrevPageModule(){
		...
		$html = $this->getHtmlLink($page,$imgUrl,'fyzj');
	...
	function getNextPageModule(){
		...
		$html = $this->getHtmlLink($page,$imgUrl,'fyyj');
		return $html;
	}
	//
	...
	function getMiddlePageModule(){
		...
		for($i=$firstLoca;$i<=$lastLoca;$i++){
			if($i == $this->mCurrPage){
				$info = $i;
				$html .= " ".$this->getHtmlLink($i,$info,'on');
			}else{
				$info = $i;
