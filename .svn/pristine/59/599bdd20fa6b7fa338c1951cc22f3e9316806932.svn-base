var cotteFn={
	...
	amount10:function(id,type){
			...
			$(".impression").on('click','li',function(){
	...
	amount110:function(id,type){
		
				//评价图
				var gallery='';
				$("input[name='gallerys[]']").each(function(){
					 gallery += $(this).val()+",";
					});

				var star = $(" #star ul").attr('data-num');
			 	star = parseInt(star)+1;

				$(this).removeClass('ltbut');
				...
				if(info ==""){
		         ...
		         var impress =[]
		         $(".impression .pfcur").each(function(){
		       ...
		       if(impress == ""){
				...
				$.ajax({
					...
					data:{star:star,info:info,gallery:gallery,id:id,type:type,impress:impress.join(';')},
					datatype:'json',
					success: function(e){
						//var r = $.parseJSON(e);
						var r=eval("("+e+")");
						if(r.done == true) {
							alert('发表评论成功',1500);
							window.location.href = "/comment-index.html";
						}else{
							$(".coment_pinglun").addClass('ltbut');
							$(".coment_pinglun").css('display','');
							alert(r.msg);
						}
					}
				});
