...
//     public static $body_options = array('定制方式','has_measure_0'=>'标准号','has_measure_1'=>'量体');
    ...
    function index()
    {
		...
		$siteurl=SITE_URL."/order_wuliu.php";
				  ...
				  if($order_goods_list){
                       ...
                       $goods_diy=0;
					    foreach ($order_goods_list as $key => $value) 
                        {
						 ...
						 if($goods_pt > '0' && $goods_diy > '0'){
                           $orders[$value['order_id']]['tps'] = "ftwo";
                         }elseif($goods_pt != '0' && $goods_diy == '0'){
                           $orders[$value['order_id']]['tps'] = "fpu";
                         }elseif($goods_pt == '0' && $goods_diy != '0'){
                           $orders[$value['order_id']]['tps'] = "fdiy";
                         }
				  }
      ...
      /*  if ($order_ids_str) 
        {
            ...
            if ($order_goods_list) 
            {
                ...
                foreach ($order_goods_list as $key => $value) 
                {
				  ...
				  // var_dump($orders[$value['order_id']]['tps']);exit;
            }
        }*/
        //=====  liang.li End  =====
      if($orders){
	     ...
	     foreach ($orders as $key1=>$value1){
        	...
        	if(isset($value1['order_id'])){

                //获取时间
        		$mtm_time = $order_mtm_log->get(array(
        				'conditions'=>"order_id ='{$value1['order_id']}'",
        				'order'=>"delivery_date DESC",
        		));
                if($mtm_time){
                    $orders[$key1]['mtm_delivery_date'] = date("Y-m-d H:i",$mtm_time['delivery_date']);
                }
                elseif ($value1['ship_time'])
                {
                    $orders[$key1]['mtm_delivery_date'] = date("Y-m-d H:i",$value1['ship_time']);
                }
        	}
        }
       }
    ...
    function giveaway(){
        $conditions='';
        $query['quanx']    =isset($_GET['quanx']) ? trim($_GET['quanx']) : '';
        $query['quanq']    =isset($_GET['quanq']) ? trim($_GET['quanq']) : '';
        $query['gongx']    =isset($_GET['gongx']) ? trim($_GET['gongx']) : '';
        $query['h_money']    =isset($_GET['h_money']) ? trim($_GET['h_money']) : '';
        $query['l_money']    =isset($_GET['l_money']) ? trim($_GET['l_money']) : '';
        if($query['quanx']){
            $conditions .= " AND quanx_id LIKE '%{$query["quanx"]}%'";
        }
        if($query['quanq']){
            $conditions .= " AND quanq_id LIKE '%{$query["quanq"]}%'";
        }
        if($query['gongx']){
            $conditions .= " AND gongx_id LIKE '%{$query["gongx"]}%'";
        }
        if($query['h_money']){
            $conditions .= " AND h_money >='{$query["h_money"]}'";
        }
        if($query['l_money']){
            $conditions .= " AND l_money >='{$query["l_money"]}'";
        }
        ...
        $orders = $model_order->get(array(
            'conditions' => "order_id = '{$orderId}'",
            'fields' => 'is_giveaway,giveaway_price,json_giveaway,user_id'
        ));
        if (!$orders)
        {
            $this->show_warning('订单信息有误!');
        ...
        $this->assign('is_giveaway', $orders['is_giveaway']);
        //查询属性
        $fbcategoryList = $fbcategory_mod->find(array(
        ...
        //获取常规商品（赠品）
        $goods['goodsList'] = $goods_mod->find(array(
            'conditions'=>" is_giveaway=2 AND marketable=1".$conditions,
            'fields' => 'name,bn,mktprice,h_money,l_money',
            'order'=>'goods_id ASC',
        ));
        //获取非常规商品
        $goods['giveawayList'] = $goods_giveaway_mod->find(array(
            'conditions'=>"marketable=1".$conditions,
            'fields' => 'name,bn,mktprice,h_money,l_money',
            'order'=>'goods_id ASC',
        ));
        //礼包
        $goods['packageList'] = $goods_package_mod->find(array(
            'conditions'=>"marketable=1".$conditions,
            'fields' => 'name,price,h_money,l_money',
            'order'=>'id ASC',
        ));
        foreach($goods['packageList'] as $key=>$val){
            $goods['packageList'][$key]['title_list'] = $goods_package_mod->getPackageInfo($val['id']);
        }

        $json_giveaway = json_decode($orders['json_giveaway'],true);
        //判断是否选择
        $json_fList = explode(",",$json_giveaway['fList']);
        foreach($json_fList as $j_f){
            foreach($goods['giveawayList'] as $fk=>$f){
                if($j_f == $f['goods_id']){
                    $goods['giveawayList'][$fk]['is_info'] = 1;
                }
            }
        }

        $json_cList = explode(",",$json_giveaway['cList']);
        foreach($json_cList as $j_c){
            foreach($goods['goodsList'] as $ck=>$c){
                if($j_c == $c['goods_id']){
                    $goods['goodsList'][$ck]['is_info'] = 1;
                }
            }
        }
        $json_gList = explode(",",$json_giveaway['gList']);
        foreach($json_gList as $j_g){
            foreach($goods['packageList'] as $gk=>$g){
                if($j_g == $g['id']){
                    $goods['packageList'][$gk]['is_info'] = 1;
                }
            }
        }
        
        //配送赠品log日志
        $log_mod =& m('order_giveaway_log');
        $log_list = $log_mod->find(array(
            'conditions' => 'user_id='.$orders['user_id'],
        ));

        foreach($log_list as $log_k=>$log_val){
             $log_list[$log_k]['add_time'] = date('Y-m-d H:i:s',$log_val['add_time']);
             $log_list[$log_k]['up_time'] = date('Y-m-d H:i:s',$log_val['up_time']);
        }
        ...
        if($res['is_try'])//试吃产品
        {
            import("diys.lib");
            $_Diys = new \Diys();
            foreach ($res['order_goods'] as $index => $re)
            {
                if($re['type'] == 'custom')
                {
                    continue;
                ...
                $params = $re['params'];
                $price_item = "";
                foreach ($params as $index1 => $param1)
                {
                    foreach (explode(",",explode(":",$param1)[1])as $index2 => $item3)
                    {
                        $price_item  .= $item3.",";
                    }
                }

                $diyRes = $_Diys->calcDiyPrice(trim($price_item,','), []);   //价格计算

                $res['order_goods'][$index]['price'] = sprintf('%.2f',($diyRes['baseprice']));
                $res['order_goods'][$index]['subtotal']  =  sprintf('%.2f',($diyRes['baseprice']) * $re['quantity']);
       ...
       // print_exit($order_goods_list);
        ...
        $order_info['shipping_name']=$shiplist[$order_info['shipping_id']];
        //抵扣券
...
//echo '<pre>';print_r($order_info);exit;
            ...
            if($cList){
              foreach($cList as $cid){
                if($cid){
                  ...
                  $order_info['c_giveaway_list'][$cid]['bn'] = $goods['bn'];
                  $order_info['c_giveaway_list'][$cid]['name'] = $goods['name'];
                  $order_info['c_giveaway_list'][$cid]['mktprice'] = $goods['mktprice'];//成本价格
                  $order_info['c_giveaway_list'][$cid]['h_money'] = $goods['h_money'];//最高消费
                  $order_info['c_giveaway_list'][$cid]['l_money'] = $goods['l_money'];//最低消费
                }
              }
            }
            //非常规减库存
            ...
            if(!empty($fList)){
                  foreach($fList as $fid){
                    if($fid){
                      ...
                      $order_info['f_giveaway_list'][$fid]['bn'] = $giveaway['bn'];
                      $order_info['f_giveaway_list'][$fid]['name'] = $giveaway['name'];
                      $order_info['f_giveaway_list'][$fid]['mktprice'] = $giveaway['mktprice'];//成本价格
                      $order_info['f_giveaway_list'][$fid]['h_money'] = $giveaway['h_money'];//最高消费
                      $order_info['f_giveaway_list'][$fid]['l_money'] = $giveaway['l_money'];//最低消费
                    }
                  }
            }
            ...
            if($gList){
              foreach($gList as $gid){
                if($gid){
                  ...
                  $order_info['g_giveaway_list'][$gid]['name'] = $package['name'];
                  $order_info['g_giveaway_list'][$gid]['price'] = $package['price'];//价格
                  $order_info['g_giveaway_list'][$gid]['h_money'] = $package['h_money'];//最高消费
                  $order_info['g_giveaway_list'][$gid]['l_money'] = $package['l_money'];//最低消费
                  $order_info['g_giveaway_list'][$gid]['title_list'] = $goods_package_mod->getPackageInfo($gid);
                }
               }
            }
        }
        ...
        $this->display('order.giveaway.html');
    }
    ...
    function addgiveaway(){
        ...
        if(!$_GET['fList'] && !$_GET['cList'] && !$_GET['gList']){
            $this->json_error('赠品必须选择一项');
            return ;
        }
        ...
        $goods_giveaway_mod =& m('goods_giveaway');
        $goods_package_mod =& m('goods_package');
        $order = $order_mod->get($orderId);
        $goods_mod =& m('goods');
        // if($order['status'] != 20){
        //     $this->json_error('订单只能在已支付进行配送赠品');
        //     return ;
        // }
        $arr = array(
            'fList' => $_GET['fList'],
        ...
        $data = array(
            'is_giveaway' => 1,
            'json_giveaway' => json_encode($arr),
            'giveaway_price' => $_GET['price'],
            'giveaway_name' => $_GET['giveaway_name']
        );
        $res = $order_mod->edit($orderId,$data);
        //进行log的添加与修改
        $log_info = $log_mod->get(array(
            'conditions'=>"order_id=".$order['order_id'],
        ));
        $f_List = explode(",",$_GET['fList']);
        $c_List = explode(",",$_GET['cList']);
        $g_list = explode(",",$_GET['gList']);
        if($log_info){
            $log_mod->drop('order_id='.$order['order_id']);
        }
            if($c_List){
                foreach($c_List as $c_id){
                    if($c_id){
                        //获取常规商品（赠品）
                        $goods_info = $goods_mod->get(array(
                            'conditions'=>" goods_id=".$c_id,
                            'fields' => 'name,mktprice',
                        ));
                        $add_data = array(
                            ...
                            'giveaway_price' => $goods_info['mktprice'],
                            'giveaway_id' => $c_id,
                            'giveaway_name' => $goods_info['name'],
                            'type' => 1,
                            'price' => $_GET['price'],
                        ...
                        $log_mod->add($add_data);
                    }  
                }
            }
            if($f_List){
                foreach($f_List as $f_id){
                    if($f_id){
                        //获取常规商品（赠品）
                        $giveaway_info = $goods_giveaway_mod->get(array(
                            'conditions'=>" goods_id=".$f_id,
                            'fields' => 'name,mktprice',
                        ));
                        $add_data = array(
                            'order_id' => $order['order_id'],
                            ...
                            'user_name' => $order['user_name'],
                            'order_price' => $order['order_amount'],
                            'giveaway_price' => $giveaway_info['mktprice'],
                            'giveaway_id' => $c_id,
            ...
            if($g_list){
                foreach($g_list as $g_id){
                    if($g_id){
                        //获取常规商品（赠品）
                        $package_info = $goods_package_mod->get(array(
                            'conditions'=>" id=".$g_id,
                            'fields' => 'name,price',
                        ));
                        $add_data = array(
                            ...
                            'user_name' => $order['user_name'],
                            'order_price' => $order['order_amount'],
                            'giveaway_price' => $package_info['price'],
                            'giveaway_id' => $c_id,
                            'giveaway_name' => $package_info['name'],
                            'type' => 3,
                            'price' => $_GET['price'],
                            'admin_id' => $this->visitor->info['user_id'],
                            'admin_name' => $this->visitor->info['user_name'],
    ...
    public function view_img()
    {
		...
		/*if(IS_POST){*/
			...
			$status=$_POST['status']?$_POST['status']:'0';
						...
						//修改状态
						$statu_datas['img_status']=$status;
						$order_mod->edit($order_id,$statu_datas);

			 $this->show_message("审核修改成功！", '返回订单列表',$_SESSION['admin_info']['order_reurl'] ? $_SESSION['admin_info']['order_reurl'] : 'index.php?app=order');
...
/*	function verify()
	{
        ...
        else
        {
		   ...
		   $this->show_message("订单已进入生产环节,无法修改标签图");
            return;
        }
...
//                ),
        ...
        /* 量体信息查询 */
        if($get['status']){
            $s = db_create_in($get['status'],'status');
            $conditions .= " AND $s " ;
        }
        //ns add
        if($_GET['img_status'])
        {
            $_GET['img_status'] -= 1;
            $conditions .= " AND img_status =".$_GET['img_status'] ;
        }
        if($_GET['is_giveaway'])
        {
            $_GET['is_giveaway'] -= 1;
            $conditions .= " AND is_giveaway =".$_GET['is_giveaway'] ;
        }
        return $conditions;
    }
...
// 				}
    ...
    */
    ...
    public function uploadimg()
    {
        ...
        $fpic = isset($_POST['upimg']) ? $_POST['upimg'] : '';
        $order_goods_mod = m('ordergoods');
        $order_goods_info = $order_goods_mod->get_info($id);
        if(!$order_goods_info)
        {
        ...
        $dir = '/upload/images/diy/'.$t;
        $baseurl = SITE_URL.$dir;
        $goods = Types::createObj("fdiy");
        #TODO mes接口修改标签图
        $mesref = $goods->mesimg($id,$baseurl);
        $order_goods_data['style'] = $dir;
        $order_goods_mod->edit("rec_id=$id",$order_goods_data);//修改电商数据库图片字段
//        if(!$mesref)
//        {
 ...
 * */
    ...
    function export(){
        ...
        foreach ($gdTem as $row){
            ...
            $user_name = $orders[$row['order_id']]['user_name'];
            ...
            //试吃计算价格
            if($orders[$row['order_id']]['is_try'] && $row['type'] == 'fdiy')//试吃产品
            {
                ...
                if($params)
                {
                    ...
                    {   
                        $aa = explode(":",$param1)[1];
                        foreach (explode(",",$aa)as $index2 => $item3)
                        {
                            $price_item  .= $item3.",";
                        }
                    }
                    $diyRes = $_Diys->calcDiyPrice(trim($price_item,','), []);   //价格计算
                    $row['price'] = sprintf('%.2f',($diyRes['baseprice']));
                    $row['subtotal']  =  sprintf('%.2f',($diyRes['baseprice']) * $row['quantity']);
                }
            }
                ...
                if ($order_goods_list['items'])
                {
                    ...
                    if($fb_bz_info) 
                    {
                        foreach ((array)$fb_bz_info as $index4 => $item4)
                        {
                            ...
                            {
                                $baozhuang = $item4['cate_name'];
                            }

                            if($item4['parent_id'] == 11)
                            {
                                $kezhong   = $item4['cate_name'];
                            }


                            if(in_array($item4['parent_id'],$dog_son))
                            {
                                $quanzhong   = $item4['cate_name'];
                            }

                            if($item4['parent_id'] == 34)
                            {
                                $jieduan   = $item4['cate_name'];
                            }

                            if($item4['parent_id'] == 1)
                            {
                                $gongxiao  = $item4['cate_name']."|".$gongxiao;
                            }

                            if($item4['parent_id'] == 6)
                            {
                                $kouwei  = $item4['cate_name'];
                            }
                        }
                    }
...
//echo '<pre>';print_r($orders[$row['order_id']]);exit;
        ...
        $res = $order;
        $fields_name = array('日期','订单编码','用户名称','收货人名称','客户电话','送货地址', '销售组织','物料编码','Header备注',' 产品编码',
            '包装方式','重量单位','数量','个性化图片文件','宠物名称','宠物性别','宠物生日','主人寄语',
            '行项目备注','犬种','生长阶段','功效','口味');
//        $fields_name = array('订单号','物料编码','存货名称','包装','规格','数量','单价','金额','收款金额','折扣','订单日期','收货日期','收货人');
    ...
    function exportt(){
        ...
        foreach ($gdTem as $row){
            ...
            if($row['price'] != $row['nprice'])
            {
                $row['price'] = $row['nprice'];
                $row['subtotal'] = sprintf('%.2f',($row['price']) * $row['quantity']);
            }

            $is_try = "";
            //试吃计算价格
            if($orders[$row['order_id']]['is_try'] && $row['type'] == 'fdiy' && $row['price'] == 0.00)//试吃产品
            {
                $is_try = "是";
                $params = json_decode($row['params'],1);
                $price_item = "";
...
//echo '<pre>';print_r($orders[$row['order_id']]);exit;
//echo '<pre>';print_r($orders[$row['order_id']]);exit;
if($orders[$row['order_id']]['order_id'] == $row['order_id'])
{
    $orders[$row['order_id']]['count'] = $orders[$row['order_id']]['count'] + 1;
}
$final_price = $orders[$row['order_id']]['final_amount'];
if($orders[$row['order_id']]['count'] > 1)
{
    $final_price = 0;
}
            $order[] = array(
//                    'add_time'    => date('Y/m/d H:i',$add_time), //日期
                'order_sn'    => $order_sn,             //APP订单号
                'out_trade_sn'    => $orders[$row['order_id']]['out_trade_sn'],             //APP订单号
                'wuliaocode' => $code,//物料
                'goods_name' => $row['goods_name'],
                'baozhuang' =>$baozhuang,
                'guige' => $kezhong,
                'shuliang' => $row['quantity'],
                'price' => sprintf("%.2f", $row['price']),
                'subprice' => sprintf("%.2f", $row['subtotal']),
                'final_price' => sprintf("%.2f", $final_price),
                'zhekou' => $orders[$row['order_id']]['discount'],
                'huodong' => $is_try,
                'add_time' => date("Y-m-d H:i:s",$orders[$row['order_id']]['add_time']),
                'ship_time' => !empty($orders[$row['order_id']]['ship_time']) ? date("Y-m-d H:i:s",$orders[$row['order_id']]['ship_time']) : '',
                'ship_name' => $orders[$row['order_id']]['ship_name'],
//                    'user_name'   => $user_name,    //用户姓名
//                    'ship_name'   => $ship_name,    //收货人姓名
//                    'ship_tel'   => $ship_tel,    //收货人姓名
...
//            '行项目备注','犬种','生长阶段','功效','口味');
        ...
        $fields_name = array('订单号','外单号','物料编码','存货名称','包装','规格','数量','单价','金额','收款金额','折扣','活动','订单日期','收货日期','收货人');
        array_unshift($res,$fields_name);
        $this->export_to_csv($res, '订单跟进-'.date('YmdHis',time()), 'gbk');
    }
...
//         ));
    ...
    function edians(){
        ...
        $order=$order_mod->get("order_id='{$order_id}'");
        ...
        if($ins==0){
            //require dirname(realpath(dirname(_FILE_)))."/cron/edbdemo.php";
            require"./../cron/edbdemo1.php";
            $cl=new EdbDemo();
      
            if($order['ship_area_id']){
                $order_region=explode(',' , $order['ship_area_id']);
            }
            $region1=$region[$order_region[0]];
            $region2=$region[$order_region[1]];
            $region3=$region[$order_region[2]];

            $re=$cl->edbTradeAdd($order,$num,$order_goods,$region1,$region2,$region3); //添加订单
			 ...
			 if($re){
                $res=json_decode($re, true);	
					...
					if($row['items']['item'][0]['is_success']=='True'){
					  ...
					  $order_mod->edit("order_sn = '{$order['order_sn']}'" ,$edData);
                     echo '推送成功';exit;
					}else{
							$edData = array(
							'mes_status'   =>2,
					     ...
					     $order_mod->edit("order_sn = '{$order['order_sn']}'" ,$edData);
					  echo '推送失败，原因：'.$res['error_msg'];exit;
					}
