...
function Custom(options){
	...
	this.api=function(){
		switch(this.checkDomain()){
			case 1:
				return 'http://api.diy.cotte.cn/v2/';//线上环境
			break;
			case 2:
				return 'http://api.diy.test.cotte.cn/v2/';//测试环境
			break;
			case 3:
				return 'http://api.diy.dev.cotte.cn:8080/v2/';//开发环境
		}
	}
	this.log=true;//log开关
...
*=======================================*/
	...
	underOrder:function(tomeasur){
		var isTomeasur=tomeasur&&location.search.indexOf('a=new')>=0;
		...
		$('#saveWorks').click(function(){
			if($uid==0){//未登录操作，与app对接跳转登录页面
				/*if(window.localStorage){
					localStorage.diyData=JSON.stringify(_diy.param.strData)
				}*/
				window.location.href="ios:login";
				app.login()
				return
