...
class MailtemplateApp extends BackendApp
{
     ...
     */
    function sms(){
        //sms_new(18513587871, getSmsEmailDesc('business','stylist_deny',array('order_id'=>'20140915104356','name'=>'Ruesin')));
        ...
        $this->display('noticetemplate/smsList.html');
    ...
    function smsEdit(){
        ...
        if (!IS_POST){
            $this->assign('content',$list[$cate]['list'][$type]['msg']);
            $this->display('noticetemplate/smsEdit.html');
        }else{
            $list[$cate]['list'][$type]['msg'] = $_POST['content'];
            $data = "<?php\n\nreturn " . var_export($list, true) . ";\n\n?>";
            file_put_contents($config, $data, LOCK_EX);
            $this->show_message('保存成功',
                    '返回',     'index.php?app=mailtemplate' . '&act=sms',
                    '继续编辑',    'index.php?app=mailtemplate&amp;act=smsEdit&amp;cate=' . $cate . '&type='.$type);
        }
    }
