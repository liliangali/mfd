...
;(function($){
    $.rc.topic = {
        ...
        init: function(options){
            options && $.extend($.rc.topic.settings, options);
            var s = $.rc.topic.settings;
            $(s.topic_unit).live({
                mouseover: function(){
            ...
            $.rc.topic.forward();
            $.rc.topic.comment();
            $.rc.topic.comment_sub();
            $.rc.topic.comment_reply();
            $.rc.topic.comment_del();
            $.rc.topic.comment_me_reply();
            $.rc.topic.comment_me_reply_sub();
        },
        //点击缩略查看详细
        piczoom: function(){
            var s = $.rc.topic.settings;
                ...
                if(self.siblings('.J_src').html()){
                    ...
                    $.getJSON(PINER.root + '/?m=space&a=feed_src', {src_id:src_id, src_type:src_type}, function(result){
                        if(result.status == 1){
                            ...
                            $.rc.ui.decode_img(self.siblings('.J_src'));
                        }else{
            ...
            $(s.topic_unit).find('.J_fw').live('click', function(){
                ...
                $.getJSON(PINER.root + '/?m=topic&a=forward', {tid:tid}, function(result){
                    if(result.status == 1){
                        ...
                        $.rc.topic.forward_form($('#J_forward_form'));
                    }else{
                        $.rc.tip({content:result.msg, icon:'error'});
                    }
                });
        ...
        forward_form: function(form){
            form.ajaxForm({
                beforeSubmit: function(){
                    ...
                    if(content == ''){
                        $.rc.tip({content:lang.please_input+lang.forward_content, icon:'error'});
                        return !1;
                    }
                },
