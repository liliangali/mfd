...
class XingeApp {
	...
	public static function PushAccountAndroid($accessId, $secretKey, $title, $content, $account)
	{
		...
		$mess->setTitle($title);
		$mess->setContent($content);
		$mess->setType(Messages::TYPE_NOTIFICATION);
		$mess->setStyle(new Style(0, 1, 1, 1, 0));
		$action = new ClickAction();
		$action->setActionType(ClickAction::TYPE_ACTIVITY);
		$mess->setAction($action);
	 ...
	 * app信鸽--Android
	 *
	 * @param  string $token 当前用户token；string $title 标题；string  $content 发送内容； String  $access_id 信鸽应用id；String  $access_key 信鸽应用key；
	 *
	 * @return  string
	 */
	public static function toXingeAndroid($accessId, $secretKey, $title, $content, $account, $param = array() )
	{	
		$push = new XingeApp($accessId, $secretKey);
		//下发安卓
		$mess = new Messages();
		...
		$mess->setStyle($style);
		$mess->setAction($action);
		$mess->setCustom($param);//参数设置array('url_type'=>'demand', 'location_id'=>'183')
	 ...
	 * @return  string
	 */
	public static function toXingeIos($accessId, $secretKey, $title, $content, $account, $param = array() )
	{	
		$pushios = new XingeApp($accessId, $secretKey);
		...
		$messios->setCustom($param);//参数设置array('url_type'=>'demand', 'location_id'=>'183')
	...
	public function  PushSingleDevice($deviceToken, $message, $environment=0)
	{
		...
		if (!($message instanceof Messages) && !($message instanceof MessageIOS)) return $ret;
		...
		if($message instanceof Messages) $params['multi_pkg'] = $message->getMultiPkg();
	...
	public function  PushSingleAccount($deviceType, $account, $message, $environment=0)
	{
		...
		if (!($message instanceof Messages) && !($message instanceof MessageIOS))
		{
	 ...
	 * 安卓创建批量推送任务
	 */
	public static  function  CreateMultipushAndroid($accessId, $secretKey, $title, $content, $accounts, $param = array() )
	{
		$push = new XingeApp($accessId, $secretKey);
		//下发安卓
		$mess = new Messages();
		...
		$mess->setAction($action);
		$mess->setCustom($param);//参数设置array('url_type'=>'demand', 'location_id'=>'183')
		$retAndroid = $push->CreateMultipush($mess);//创建任务
	//print_r($retAndroid);exit;
		if (!($retAndroid['ret_code'] === 0)) {
			return $retAndroid;
		} else {
			$retAndroid = $push->PushAccountListMultiple($retAndroid['result']['push_id'], $accounts);
			return ($retAndroid);
		}
	 ...
	 * IOS创建批量推送任务
	 */
	public static  function  CreateMultipushIos($accessId, $secretKey, $title, $content, $accounts, $param = array() )
	{
		$pushios = new XingeApp($accessId, $secretKey);
		...
		$acceptTime1 = new TimeInterval(0, 0, 23, 59);
		$messios->addAcceptTime($acceptTime1);
		$retios = $pushios->CreateMultipush($messios, XingeApp::IOSENV_PROD);

		//执行批量发送
		if (!($retios['ret_code'] === 0)) {
			return $retios;
		} else {
			$retios = $pushios->PushAccountListMultiple($retios['result']['push_id'], $accounts);
			return ($retios);
		}
	}
