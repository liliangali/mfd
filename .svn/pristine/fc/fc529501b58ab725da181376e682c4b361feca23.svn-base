...
/* 商品分类 gcategory */
class Jpjz_gcategoryModel extends BaseModel
{
    var $table  = 'jpjz_gcategory';
    var $prikey = 'cate_id';
    var $alias  = 'g';
    var $_name  = 'jpjz_gcategory';
    var $_relation  = array(
        // 一个分类只能属于一个店铺
        'belongs_to_store' => array(
            ...
            'reverse'       => 'has_jpjz_gcategory',
        ),
        // 一个分类有多个子分类  //yusw
        'has_jpjz_gcategory' => array(
            'model'         => 'jpjz_gcategory',
            'type'          => HAS_MANY,
        ...
        'has_goods' => array(
            ...
            'foreign_key'   => 'cate_id',
            'reverse'       => 'belongs_to_jpjz_gcategory',
        ),
    );
...
/* 商品分类业务模型 */
class jpjz_gcategoryBModel extends jpjz_gcategoryModel
{
    var $_store_id = 0;
    ...
    function get_children($id, $cached = false)
    {
        ...
        {
            $sql = "SELECT cate_id, cate_name, intro FROM {$this->table} WHERE parent_id = '$id' AND store_id = '{$this->_store_id}'";
            $res = $this->getAll($sql);
        }
