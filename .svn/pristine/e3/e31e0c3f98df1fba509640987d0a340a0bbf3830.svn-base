...
//ns add 订单状态  //sin modi 2015-05-14 09:09:06
define('ORDER_PENDING', 11); //待付款
...
define('BOOKORDER_RETURN',   70); //申请退货
define('BOOKORDER_RETURNED',   80); //已退货
...
//=====  add by liang.li  =====
define('ORDER_TAX', 0);             //收益转余额需扣除的税(百分比)
...
defined('CHARSET') or define('CHARSET', substr(LANG, 3));
...
/* 创业者等级 */
define('LV_CLZD',   1); // 未处理
define('LV_KTDR',    2); // 通过
define('LV_KTJJ',    3); // 未通过
define('LV_KTLX',    4); // 未通过
...
EOT;
    ...
    function _assign_seo($type, $info)
    {
        ...
        /* add by Xiao5 注册 登录  头部统一提示 */
        if (isset($_seo_info['title'])){
            $rt = explode(" - ",$_seo_info['title']);
            if ($rt[0])
            {
                $this->assign('_rtitle', $rt[0]);
            }
        }
    ...
    function display($f)
    {
        ...
        /* 用户信息 */
        $this->assign('visitor', isset($this->visitor) ? $this->visitor->get() : array());
...
//echo  '//';
    ...
    function _format_page(&$page, $num = 7)
    {
        ...
        */
        ...
        for ($i = $from; $i <= $to; $i++)
        {
            $page['page_links'][$i] = mfdurl("{$url_format}&page={$i}");
        }
        if (($page['curr_page'] - $from) < ($page['curr_page'] -1) && $page['page_count'] > $num)
        {
            $page['first_link'] = mfdurl("{$url_format}&page=1");
        ...
        {
            $page['last_link'] = mfdurl("{$url_format}&page=" . $page['page_count']);
        ...
        $page['prev_link'] = $page['curr_page'] > $from ? mfdurl("{$url_format}&page=" . ($page['curr_page'] - 1)) : "";
        $page['next_link'] = $page['curr_page'] < $to ? mfdurl("{$url_format}&page=" . ($page['curr_page'] + 1)) : "";
    }
...
EOT;
    ...
    function _build_upload($params = array())
    {
        ...
        $include_js = '<script type="text/javascript" charset="utf-8" src="{lib file="swfupload/swfupload.js"}"></script>
<script type="text/javascript" charset="utf-8" src="{lib file="swfupload/js/handlers.js"}"></script>';
        ...
        $admin_info = base64_encode(json_encode($_SESSION['admin_info']));
        $str = <<<EOT
...
//         unset($_SESSION[$this->_info_key]);
        ...
        else
        {
            ...
            if(empty($_COOKIE['username']) && empty($_COOKIE['password']))
            {
                ...
                $user_id = $ms->user->auth($_COOKIE['username'], $_COOKIE['password'],true);//去取用户的个人资料
                if(empty($user_id) || $this->_info_key == 'admin_info')
                {
                    $this->info         = array(
                        'user_id'   => 0,
                        'user_name' => Lang::get('guest')
    ...
    function _get_detail()
    {
        ...
        /* 获取当前用户的详细信息，包括权限 */
        //var_dump($this->info['user_id']);
        $member_info = $model_member->findAll(array(
            'conditions'    => "member.user_id = '{$this->info['user_id']}'",
            //'join'          => 'has_store,has_serve',                 //关联查找看看是否有店铺
            'fields'        => 'state_info,is_ident,nickname,member_lv_id,birthday,member.region_name as m_rname,member.region_id as m_rid,memo,phone_mob,height,weight,member.email, point, coin, money, frozen,profit, password, member.real_name, logins, portrait',
//             'include'       => array(                       //找出所有该用户管理的店铺
//                 'manage_store'  =>  array(
//                     'fields'    =>  'user_priv.privs, store.store_name,store.level',
//                 ),
//             ),
        ));
        ...
        $lvids = array();
        $lvids['member'] = $detail['member_lv_id'];
//         if ($detail['s'][$detail['user_id'].'_'.$detail['store_id']])
//         {
//         	$lvids['supplier'] = $detail['s'][$detail['user_id'].'_'.$detail['store_id']]['level'];
//         }
       ...
       /*会员等级信息 */
        if ($lvids)
        {
            ...
            $lvs = $member_lv_mod->find(array(
                'conditions'    => db_create_in($lvids,'member_lv_id'),
            ...
            foreach ($lvs as $k=>$v)
            {
               if (isset($lvids[$v['lv_type']]))
               {
               	$detail[$v['lv_type'].'lv']['name'] = $v['name'];
               	$detail[$v['lv_type'].'lv']['lv_logo'] = site_url().$v['lv_logo'];
               }
            }
...
//         }
    ...
    function logout()
    {
        ...
        /* 清楚记录密码 */
//         if(!empty($_COOKIE['username']) || !empty($_COOKIE['password'])){
            setcookie("username", null, time()-3600*24*14);
            setcookie("password", null, time()-3600*24*14);
//         }
    }
    ...
    function check_do_action($privileges)
    {
        ...
        else
        {
            ...
            if(!in_array('default|all',$privs)){
                array_push($privs,'default|all');//后台登陆权限忘记添加
            }
            /* 后台弹窗组件 所有人都有权限 */
            array_push($privs,'inputobj|all');
            /* 后台相册上传组件 所有人都有权限 */
            array_push($privs,'swfupload|all');
            /* 后台图片上传组件 所有人都有权限 */
            array_push($privs,'images|all');
