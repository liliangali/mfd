 ...
 *    @usage    none
 */
class Manjian_promotionApp extends BackendApp
{
    ...
    function index()
    {
        ...
        $lists= $this->_newpromotion_mod->find(array(
            'conditions'=>"yhcase=6 or yhcase=7",
            'field'=>'*',
            'order'=>'add_time desc',
            'limit'=>$page['limit'],
            'count'=>true
        ));
        $page['item_count']=$this->_newpromotion_mod->getCount(array(
		 'conditions'=>"yhcase=6 or yhcase=7",
		));
        ...
        $this->display('mj/goods_promotion.index.html');
    }
     /**
     *    新增促销规则
     *
    ...
    function add()
    {
        if (!IS_POST)
        {
            ...
            $rules = $this->_newpromotion_mod->find();
            ...
            $this->display('mj/goods_promotion.form.html');
        ...
        { 
			...
			$casevalue2 = $_POST['casevalue2'];
                ...
                if(!$casevalue2 or !$casevalue){
                    $this->show_warning("请填写优惠方案值","back");
                    return false;
                }
            ...
            $r = $this->_newpromotion_mod->add($data);
            $id = $r;
            ...
            if($id){
                ...
                $this->_newpromotion_mod->edit($id,$cases);
                
            }
            
             if($id){
                $this->show_message('添加成功','back_list', 'index.php?app=manjian_promotion&act=index&amp;cate_id=0');
            }else{
                $this->show_message('添加失败','<<返回', 'index.php?app=manjian_promotion&act=add&amp;cate_id=0');
            }  
    ...
    function edit(){
        ...
        $id=$_REQUEST['id'];
        $rule= $this->_newpromotion_mod->get($id);
        ...
        if(!IS_POST){
            ...
            if(!in_array($yhcase, array(6,7))){
                ...
                if(!$casevalue2 or !$casevalue){
                    $this->show_warning("请填写优惠数值","back");
                    return false;
                }
            ...
            // 修改 规则
            $r = $this->_newpromotion_mod->edit($id,$data);
            ...
            if($r){
                $this->show_message('修改成功','back_list', 'index.php?app=manjian_promotion&act=index');
            }else{
                $this->show_message('修改失败','<<返回', 'index.php?app=manjian_promotion&act=edit');
            }  
    ...
    function drop(){
        ...
        $c_id = $this->_newpromotion_mod->drop($id);
        if($c_id){
            $this->_goodslink_mod->drop("rules_id='{$id}'");
        }
        
        
        $this->show_message('促销规则删除成功','<<返回','index.php?app=manjian_promotion&act=index&amp;cate_id=0');
    }
