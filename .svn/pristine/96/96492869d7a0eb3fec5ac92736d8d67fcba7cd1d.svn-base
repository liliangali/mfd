...
class DefaultPassportUser extends BasePassportUser
{
	...
	function auth_info($user_name, $password){
    ...
    function set_order_num($user_id){
        $model_member =& m('member');
        $info = $model_member->setInc(" user_id = $user_id " , 'order_num');
        if (!empty($info))
        {
    ...
    function set_final_amount_num($user_id,$final_amount){
        $model_member =& m('member');
        $final_amount_s = $this->get($user_id);
        $final_amount_num = $final_amount_s['final_amount_num'] + $final_amount;
        $info = $model_member->edit(" user_id = $user_id " , array('final_amount_num'=>$final_amount_num));
        if (!empty($info))
        {
            $this->_error('email_exists');
    ...
    /* 注册 */
    function register($user_name, $password, $email, $local_data = array(),$mark='')
    {
       ...
       // }
        ...
        /* 添加到用户系统 */
        $user_id = $this->_local_add($local_data,$mark);
    ...
    function edit($user_id, $old_password, $items, $force = false)
    {
     ...
     /* 编辑邮箱 */
    function editemail($user_id,$items,$force = false){
		if(!$force)
		{
    ...
    /* 获取用户信息 */
    function get($flag,$is_name = false,$source=false)
    {
        if ($is_name)
        {
            if($source){
                $conditions = " serve_type=1 and (user_name='{$flag}' or phone_mob='{$flag}' or email='{$flag}')";
            }else{
...
//        echo '<pre>';var_dump($conditions);die;
     ...
     *    @return    int    用户ID
     */
    function auth($user_name, $password,$source)
    {
        $info = $this->get($user_name,true,$source);
        if ($info['password'] != md5($password))
        {
            $this->_error('auth_failed');
    ...
    {
        $model_member =& m('member');
        $info = $model_member->get("serve_type=1 and (user_name='{$user_name}' or phone_mob = '$user_name' or email = '$user_name' )");
 ...
 *    ns add 模拟登陆
 */
// class DefaultPassportUserAdmin extends BasePassportUserAdmin
// {
...
//     {
//         $info = $this->get($user_name, true);
//         //var_dump($this->get($user_name,true));exit;
//         if ($info['password'] != md5($password))
//         {
//             $this->_error('auth_failed');
...
//     {
//         $model_member =& m('member_admin');
//         $info = $model_member->get("user_name='{$user_name}' or phone_mob = '$user_name' or email = '$user_name' ");
//         if (!empty($info))
//         {
//             $this->_error('user_exists');
