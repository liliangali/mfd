...
var cotteFn={
	orderInfo:function(opt){ 
		...
		var _orderS = {'figure':{},'ship':{},'tax':{},'pay':{},'dedu':{'debit':{'sn':'','mn':0,'_cate':0},'coin':0,'money':0,kuka:{'amount':0,'use':{}}},'amount':opt.amount,'diy_price':opt.diy_price,'custom_price':opt.custom_price,'also':opt.amount,'_also':opt.amount};
// console.log(_orderS);
		...
		function initDebit(){
			...
			_orderS['dedu']['debit'] = {'sn':'','mn':0,'_cate':0};
			$('.kuquanBox').find('.radioBox li').each(function(i){
				if($(this).data('sn') == _debit['sn']){
					//
					// var _cate = _debit['_cate'];
					// if(_cate == '1')//定制
					// {
					// 	if (_debit['mn'] > opt.diy_price)
					// 	{
					// 		_debit['mn'] = opt.diy_price;
					// 	}
					// }
					// if(_cate == '2')
					// {
					// 	if (_debit['mn'] > opt.custom_price)
					// 	{
					// 		_debit['mn'] = opt.custom_price;
					// 	}
					// }
					//
					// alert($(this).data('money'))
					// alert(_debit['mn']);
					_orderS['dedu']['debit'] = {'sn':$(this).data('sn'),'mn':_debit['mn'],'_cate':$(this).data('category')};
					return false;
				}
			})
		}
	   ...
	   if(opt.canCoin == '0'){
		...
		function setOrderSs(){
			...
			_orderS['also'] = _orderS['_also'] = opt.amount - _orderS['dedu']['money'] - _orderS['dedu']['coin'] - _orderS['dedu']['debit']['mn'] - _orderS['dedu']['kuka']['amount'];
			 // alert(_orderS['also']);
			// console.log(_orderS['dedu']['debit']['mn'])
...
// console.log(_orderS);
			...
			if(parseFloat(_dedu['debit']['mn']) > 0){
				//alert(_dedu['debit']['mn']);
				var _cate = _dedu['debit']['_cate'];
				// alert(_cate)
				// alert(_dedu['debit']['mn'])
				// console.log(_dedu['debit']);
				if(_cate == '1')//定制
				{
					if (_dedu['debit']['mn'] > opt.diy_price)
					{
// alert(_dedu['debit']['mn'])
						_dedu['debit']['mn'] = opt.diy_price;
						// alert(_dedu['debit']['mn'])
						// alert(_dedu['debit']['mn']);
					}
				}
				if(_cate == '2')
				{
					if (_dedu['debit']['mn'] > opt.custom_price)
					{
						_dedu['debit']['mn'] = opt.custom_price;
					}
				}
				$('.kuquanBox').find('.tit').addClass('show');
				$('.kuquanBox').find('.layer').show();
				$('.debit_'+_dedu['debit']['sn']).addClass('cur');
				$('.debit_'+_dedu['debit']['sn']).attr('data-money',_dedu['debit']['mn']);
 ...
 *======================*/
		...
		$('.kuquanBox li').click(function(){
			...
			var _cate = ($this.data('category'));
			// alert(_cate);
			if(_cate == '1')//定制
			{
				if (_mn > opt.diy_price)
				{
					_mn = opt.diy_price;
				}
			}
			if(_cate == '2')
			{
				if (_mn > opt.custom_price)
				{
					_mn = opt.custom_price;
				}
			}
			_orderS['dedu']['debit']['_cate'] = _cate;
