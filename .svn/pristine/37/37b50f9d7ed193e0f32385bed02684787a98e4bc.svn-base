...
$.cart = function(opt){
	...
	function _loadData(){
		...
		$('#JcartList').find('.cartList').each(function(i){
			var _list =$(this),_this = _list.find('input:checkbox'),_id=_list.data('id'),_price=_list.data('price'),_cate=_list.data('cate'),_num=_list.find('.jajia_box input').data('num');
		...
		$('.dzsp').find('span:eq(0)').html(_carts['num'])
