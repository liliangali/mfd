 ...
 * ============================================================================
 * $Id: mysql.php 12825 2015-12-19 08:19:50Z zhaoxr $
 */
...
class cls_mysql
{
    ...
    function query($sql, $type = '', $times=0)
    {
        /**
         * 本地调试sql
         * Warning: session_start() php.ini中的 session.auto_start = 0 为 session.auto_start = 1 
         */
//         if (DEBUG_SQL === 'show')
//         {
//             kint::dump($sql);
...
//         }
        ...
        if (!($query = mysql_query($sql, $this->_link_id)))
        {
            ...
            else
            {
                if ($type != 'SILENT')
                {
			        ...
			        {
			            $logfilename = $this->root_path . 'data/error/mysql_query_' . $this->_dbhash . '_' . date('Y_m_d') . '.log';
			            $str = $sql . "\n\n";
        ...
        if (defined('DEBUG_MODE') && (DEBUG_MODE & 8) == 8)
        {
            $logfilename = $this->root_path . 'data/log/mysql_query_' . $this->_dbhash . '_' . date('Y_m_d') . '.log';
            $str = $sql . "\n\n";
