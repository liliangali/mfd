...
$(function(){
    // 为登录用户初始化webchat
    if($.getCookie('wkey').length > 0){
        if(window.location.href != 'http://test.alicaifeng.com/view/sc-utf-8/mall/default/user/ajax_login.html'){
            $('body').append('<div id="talk"><script type="text/javascript" src="http://im.alicaifeng.com/init.js" charset="utf-8"></script></div>');
        }
    }
});
...
$(function(){
	...
	$('.fixed-nav .survey').click(function(){
			use('../../static/expand/layer/layer.min.js',function(){
				...
				html.push( "        <input type=\"button\" value=\"提交\" id='submitFk' class=\"ok\">")
		...
		if($('.fankui .editor').val()==""){
			...
			$.ajax({
				...
				success: function(){
					...
					$(document).on('click','#submitFk',fankui)
				}
			})
		}
	}
