/*
	...
	$(".container").hScrollPane({
		...
		handleMinLeft:15,//指定handle距离左边右边的位置 默认是为0 | 可选项
	...
	$.fn.hScrollPane=function(settings){
		...
		this.each(function(){
			...
			handle.stop().css({
				...
				width:settings.handleMinWidth,
				left:settings.handleMinLeft ? settings.handleMinLeft : 0
			});
			...
			*/
			handle.bind("mousedown",function(e){
				...
				$(document).bind("mousemove",function(e){
					if(settings.handleMinLeft){
						var lr = settings.handleMinLeft;
						var lr2 = (e.pageX-x+hx>=maxlen?maxlen:e.pageX-x+hx) - lr;
					 }else
					{
					    var lr = 0;
						var lr2 = (e.pageX-x+hx>=maxlen?maxlen:e.pageX-x+hx);
					}
					
					var left=e.pageX-x+hx< lr ? lr : lr2 ;
					...
					if(settings.easing){
						...
						if(settings.handleMinLeft){
						   var moverLeft = Math.floor(left/maxlen*(c-w) - settings.handleMinLeft+1);
						}else{
						   var moverLeft = left/maxlen*(c-w);
						}
						mover.css({left:-moverLeft});
					}
