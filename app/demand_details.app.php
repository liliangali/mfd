...
/**
 * 定制需求前台控制类
 * ns up 把之前的需求添加到用户中心里面
 */
class Demand_detailsApp extends MemberbaseApp
{
    ...
    function __construct(){
        $this->Demand_detailsApp();
    }
    
    function Demand_detailsApp(){
    ...
    function sue($type = 'normal'){
        ...
        /* 当前用户中心菜单 */
        $this->assign('_member_menu', $this->_get_member_menu('user_set'));
        $this->assign('_curitem', 'demand_details');
    ...
    function subsue(){
        ...
        if(count($_POST['adjunct']) >5){
            $this->json_error('图片最多只能上传5张哦!');
            return;
        }
