...
class StoreApp extends StorebaseApp
{
    function index()
    {
        ...
        //获取西服
        $custom_mod = &m("customs");
        $custom_list = $custom_mod->find(array(
            'conditions' => "is_active=1 AND cst_cate =3",
            'limit' => 4,
            'order' => "cst_id DESC"
        ));
    ...
    function story(){
        ...
        //ns add
        $this->assign("title",$store['store_name'].'-RCTAILOR-定制是一种生活态度(rctailor.com)');
        $this->assign('keywords','西服定制、衬衫定制、大衣定制、西裤定制、马甲定制、面料品牌');
        $this->assign('description',$store['store_name'].'-面料品牌推荐-RCTAILOR-定制是一种生活态度(rctailor.com)');

        $store['store_gcates'] = $this->_get_store_gcategory($id);
        $this->assign('store', $store);
        $this->assign('store_html', htmlspecialchars_decode($store['description']));
        $this->assign('story', 1);
        $this->display('store.story.html');
    }
    ...
    function search()
    {
        ...
        /* 取得劲爆商品 */
        //$this->assign('jtop_goods', $this->_get_jtop_goods($id));
        ...
        /* 取得销售排行信息 */
        //$this->assign('goods_if_list', $this->_get_ifList_goods($id));

        /* 当前位置 */
        $this->_curlocal(LANG::get('all_stores'), 'index.php/search-store.html',
            $store['store_name'], 'index.php/search-store-' . $store['store_id'].'.html',
            LANG::get('goods_list')
        );
	...
	*/
    function rn_goods()
	{
	   ...
	   $type = empty($_GET['type']) ? 0 : intval($_GET['type']);
		...
		/* 取得推荐商品 */
       $this->rn_goods_list($id,$type);
		
		$this->display('store.rn_goods_list.html');
	}
    ...
    function credit()
    {
        ...
        $goods_list = $order_goods_mod->find(array(
            ...
            'fields'     => 'user_id, buyer_name, anonymous, evaluation_time, goods_id, goods_name, specification, price, quantity, goods_image, evaluation, comment',
    ...
    /* 获得劲爆商品 */
    function _get_jtop_goods($id, $num = 6){
        $goods_mod =& bm('goods', array('_store_id' => $id));
        $goods_list = $goods_mod->find(array(
                'conditions' => "closed = 0 AND if_new = 1 AND recommended = 1",
                'fields'     => 'goods_id, goods_name, default_image, price, price_n',
                'limit'      => $num,
        ));
    ...
    /* 取得推荐商品 */
    function _get_recommended_goods($id, $num = 90)
    {
        $goods_mod =& bm('goods', array('_store_id' => $id));
        $part_mod =& m("part");

        $goods_list = $goods_mod->find(array(
            'conditions' => "closed = 0 AND if_show = 1 AND recommended = 1 AND cate_id != 5000",
            'fields'     => 'goods_name, default_image, price, price_n,fabric_number',
            'limit'      => $num,
            'count'      => true,
        ));
        foreach ($goods_list as $key => $goods)
        {
            empty($goods['default_image']) && $goods_list[$key]['default_image'] = Conf::get('default_goods_image'); 
            $goods_list[$key]['part_price'] = $part_mod->get(array('conditions'=>'goods_id='.$goods['goods_id'], 'fields'=>'price'));
        }
        return $goods_list;
    }
    ...
    function _assign_searched_goods($id)
    {
        ...
        $goods_list = $goods_mod->get_list(array(
            'conditions' => 'cate_id!=5000 AND closed = 0 AND if_show = 1' . $conditions,
            'count' => true,
     ...
     * 获取推荐商品
     */
	 function rn_goods_list($id,$type)
	 {
	    $goods_mod =& bm('goods', array('_store_id' => $id));
		if($type == 1)
		{
		  $this->assign('rn_goods_name', '推荐商品');
		  $conditions = 'AND g.recommended = 1';
		}
		else if($type == 2)
		{
		  $this->assign('rn_goods_name', '最新商品');
		  $conditions = 'AND if_new = 1';
		}
		$page = $this->_get_page(16);
        $goods_list = $goods_mod->get_list(array(
        ...
        $this->assign('recommended_goods', $goods_list);
        $page['item_count'] = $goods_mod->getCount();
        $this->_format_page($page);
        $this->assign('page_info', $page);
	 }
