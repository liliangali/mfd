...
define('THUMB_WIDTH', 325);
define('THUMB_HEIGHT', 425);
define('THUMB_QUALITY', 120);
...
class My_goodsApp extends StoreadminbaseApp
{
    ...
    function index()
    {
        ...
        /* 当前页面信息 */
        $this->_curlocal(LANG::get('member_center'), '/index.php/member.html',
                         LANG::get('my_goods'), '/index.php/my_goods.html',
                         LANG::get('goods_list'));
        ...
        $this->assign('title','RCTAILOR-酷客中心-面料管理');
        $this->assign('title_info','面料管理');
    ...
    function truncate()
    {
        ...
        $this->show_message(sprintf(Lang::get('truncate_ok'), $rows),
            'back_list', '/index.php/my_goods.html'
        );
    }
    ...
    function batch_edit()
    {
        ...
        else
        {
             ...
             if ($_POST['if_new'] >= 0)
             {
                 $data['if_new'] = $_POST['if_new'] ? 1 : 0;
             }
             if ($_POST['if_list'] >= 0)
             {
                 $data['if_list'] = $_POST['if_list'] ? 1 : 0;
             }
             if ($_POST['price_n'] >= 0)
             {
                 $data['price_n'] = $_POST['price_n'] ? $_POST['price_n'] : '0.00';
             }
     ...
     * 检查面料分类（必选，且是叶子结点）
     *
     * @param   int     $cate_id    面料分类id
     * @return  bool
     */
    function _check_mgcate($cate_id)
    {
		return true; //todo by liry
    ...
    function add()
    {
        $this->assign('title','RCTAILOR-酷客中心-添加面料');
        $this->assign('title_info','添加面料');
        import('image.func');
        import('uploader.lib');
        /* 检测支付方式、配送方式、面料数量等 */
        // if (!$this->_addible()) {
        ...
        if (!IS_POST)
        {
             ...
             $type = array("1"=>"面料","0"=>"配搭");
             $this->assign("is_mat",$type);
...
//              $mCst =& m('customs');
//              $cCates  = $mCst->getCate();
//              $mCates = $this->_get_mgcategory_options(0);
//              foreach ($mCates as  $key => $val){
// //              	$cates[$cCates[$key]['parts_type']['fabric']['id']] = $cCates[$key]['parts_type']['fabric']['name'];
//                  $cates[$cCates[$key]['parts_type']['fabric']['id']] = $val;
//              }
//              $this->assign('mgcategories', $cates);

             $mCates = array('8001'=>'西服', '8050'=>'大衣', '8030'=>'衬衣');//modify by Ruesin
             $this->assign('mgcategories', $mCates);
...
//                  'upload_url' => '/index.php/swfupload.html?instance=goods_image',
//                  'if_multirow' => 1,
//              )));
             ...
             $this->assign('editor_upload', $this->_build_upload(array(
                 ...
                 'upload_url' => '/index.php/swfupload.html?instance=desc_image',
                 'if_multirow' => 1,
//                  'ext_js' => false,
                 'ext_css' => false,
             )));
        ...
        else
        {
            ...
            $data['goods']['closed'] = 1 ;       //默认是未审核 ns add
            ...
            {
                $goods_url = SITE_URL . '/index.php/goods-' . $goods_info['goods_id'] .'.html';
                $feed_images = array();
                $feed_images[] = array(
            ...
            $part_data = $_POST;
            $part_data['default_image'] = SITE_URL . '/' . $goods_info['default_image'];
            //添加数据到part表
            $this->addPart($part_data,$this->_last_update_id);
            //生成二维码
            Qrcode('goods',$this->_last_update_id,SITE_URL.'/index.php/goods-'.$this->_last_update_id.'.html');


            $this->show_message('add_ok',
                'back_list', '/index.php/my_goods.html',
                'continue_add', '/index.php/my_goods-add.html'
            );
        }
    }
    ...
    function edit()
    {
        $this->assign('title','RCTAILOR-酷客中心-编辑面料');
        $this->assign('title_info','编辑面料');
        import('image.func');
        import('uploader.lib');
        $this->assign('act_info','edit');
        ...
        if (!IS_POST)
        {
            ...
            $default_goods_images = array(); // 默认面料图片
            $other_goods_images = array(); // 其他面料图片
            ...
            $this->assign('desc_images', $desc_images);
             $type = array("1"=>"面料","2"=>"配搭");
            $this->assign("is_mat",$type);
            $cate_id = $goods['cate_id'];
            //取得类型
            $type_list = $this->_get_options_type($cate_id);
			...
			{ */
				$part_mod = & m('part');
				$part_info = $part_mod->get("goods_id=".$goods_id);
				$part_id   = $part_info['part_id'] > 0 ? $part_info['part_id'] : 1;
				$this->assign("part_id",$part_id);
			//}
			if (!$part_id) 
			{
				//TODO $this->show_warning("面料正在审核中!!! 不允许修改");
			}
            $this->assign("goods_attr_html",$this->build_attr_html($goods['type'],$part_id));
        ...
        else
        {
            ...
            /* 检查数据 todo by lirp 先不检测数据 不然过不去。。。
          ...
          //$data['closed'] = 1;
            ...
            //生成二维码
            Qrcode('goods',$id,SITE_URL.'/index.php/goods-'.$id.'.html');
            
            $this->editPart($_POST, $id);

            $this->show_message('edit_ok',
                'back_list', '/index.php/my_goods.html',
                'edit_again', '/index.php/my_goods-edit.html?id=' . $id);
        }
    }
   ...
   function spec_edit()
   {
        ...
        else
        {
            ...
            $default_spec = array(); // 更新面料中默认规格的信息
   ...
   function ajax_col()
   {
    ...
    function import_taobao()
    {
        ...
        else
        {
            ...
            foreach ($records as $record)
            {
                ...
                if ($remain !== false) // 如果店铺等级有面料数量限制
                {
                    if ($remain <= 0)
                    {
                        if ($num_record == 0) // 还没有导入面料数就超过限制了
                        {
                ...
                {
                    if ($record['goods_image']) // 店铺等级无面料数量限制
                    {
                        $num_image += $record['image_count'];
                    }
                }
    ...
    function _get_mgcategory_options($parent_id = 0)
    {
        ...
        // }
        $res = array('3'=>'西服', '6000'=>'大衣', '3000'=>'衬衣');
        return $res;
    }
...
//         }
    ...
    {
        $default_goods_image = Conf::get('default_goods_image'); // 商城默认面料图片
        if ($id > 0)
        {
    ...
    function _get_post_data($id = 0)
    {
        $goods = array(
            ...
            'if_new'              => $_POST['if_new'],
            'if_list'             => $_POST['if_list'],
            'price_n'             => $_POST['price_n'],
            'price'               => $_POST['price'],
            'cost_price'               => $_POST['cost_price'],  //Modify By Ruesin
            'color'               => $_POST['color'] ,
            ...
            'fabric_number'       => $_POST['fabric_number'] ,
            'last_update'      => gmtime(),
            'recommended'      => $_POST['recommended'],
            'tags'             => trim($_POST['tags']),
        	'product_date'             => trim($_POST['product_date']),
        	'product_exp'             => trim($_POST['product_exp']),
        	'type'				=> intval($_POST['goods_type']),
        	'weight'			=> intval($_POST['weight']),
        	'shazhi'			=> intval($_POST['shazhi']),
    ...
    function _check_post_data($data, $id = 0)
    {
        ...
        /*验证名称是否在part表存在 如果存在就不允许添加  add by liang.li*/
        $goods_name = trim($data['goods']['goods_name']);
        $part_mod = m('part');
        $res = $part_mod->get(array(
        		'conditions' => "part_name='$goods_name' AND goods_id != $id",
        		'fields'  => 'part_name',
        		));
        if ($res)
        {
        	$this->_error('名称已经存在!不允许重复添加');
        	return false;
        }
...
//   dump($data);
     ...
     * @author liliang
     */
    function addPart($part_data,$goods_id)
    {

        $part_mod = & m("part");
        $part_attr_mod = & m("partattr");
        $data = array();
        $data['type_id'] = $part_data['goods_type'];
        /*荣平说供应商添加商品 默认都是面料  所以这里fabirc_id字段写死是1*/
        $data['fabric_id']  = $part_data['is_mat'];
        $data['part_name']  = $part_data['goods_name'];
        $data['goods_sn']   = $part_data['fabric_number'];
        $data['cate_id']   = $part_data['cate_id'];
        $data['cost_price'] = $part_data['price'];
        $data['weight'] = $part_data['weight'];
        $data['shazhi'] = $part_data['shazhi'];
        $data['goods_id'] = $goods_id;
        $data['part_img'] = $part_data['default_image'];
        ...
        {
            $part_id = $part_mod->add($data);
        }
    	//添加属性
    	if ((isset($part_data['attr_id_list']) && isset($part_data['attr_value_list'])) || (!empty($part_data['attr_id_list']) && !empty($part_data['attr_value_list'])))
    	{
    		foreach ($part_data['attr_value_list'] as $k=>$attr_value) {
    			$part_attr_mod->add(array(
    					'part_id' => $part_id,
    					'attr_id' => $part_data['attr_id_list'][$k],
     ...
     * @author liliang
     */
    function editPart($part,$goods_id)
    {
    	//根据goods_id查出part_id
    	$part_mod = & m("part");
    	$part_info = $part_mod->get("goods_id=".$goods_id);
    ...
    function get_type(){
    	...
    	foreach ($cCates as  $key => $row){
    	    if($row['parts_type']['fabric']['id'] == $cate_id){
    	        ...
    	        $type_list = $this->_get_options_type($key);
//     	        if($key == 3)break;
    	        break;
    ...
    function build_attr_html($cat_id, $goods_id = 0)
    {
    	...
    	{
    		$part_mod = & m("part");
    		$part_info = $part_mod->get_info($goods_id);
...
//     dump($attr);	 
    	...
    	foreach ($attr AS $key => $val)
    	{
    		...
    		$html .= "<h2>$val[attr_name]：</h2></td><td><input type='hidden' name='attr_id_list[]' value='$val[attr_id]' />";
    
    		if ($val['attr_input_type'] == 0)
    		{
    			$html .= '<input class="text width_normal" name="attr_value_list[]" type="text" value="' .htmlspecialchars($val['attr_value']). '" size="40" /> ';
    		}
    		else
    		{
    			$html .= '<select style="width:130px;padding:5px;" name="attr_value_list[]">';
    ...
    /* 取得可以作为上级的类型数据 */
    function _get_options_type($except = NULL)
    {
    	$parttype_mod = & m("parttype");
    	 
    	$gcategories = $parttype_mod->get_list_store(-1,$except);
//     	  dump($gcategories);
    	$tree =& $this->_tree_type($gcategories);
    ...
    /* 构造并返回树 */
    function &_tree_type($gcategories)
    {
    	import('tree.lib');
    	$tree = new Tree();
