...
function diy_url()
{
    ...
    if (stripos($config['SITE_URL'],"m.mfd.com"))
    {
        $url = "http://shop.diy.dev.mfd.cn:8080/";
    }
    elseif (stripos($config['SITE_URL'],"m.dev.mfd.cn"))
    {
        $url = "http://shop.diy.dev.mfd.cn:8080/";
    }
    elseif (stripos($config['SITE_URL'],"m.test.mfd.cn"))
    {
        $url = "http://shop.diy.test.mfd.cn/";
    }
    elseif (stripos($config['SITE_URL'],"m.mfd.cn"))
    {
        $url = "http://shop.diy.mfd.cn/";
    }
...
function reloadMember($user_id)
{
    ...
    if ($lv_id && $auth_log['status']==1 && !empty($invite_info))
    {
...
function &cache_server()
{
    ...
    {
        $config =  include ROOT_PATH.'/data/config.inc.php';
        if (stripos($config['SITE_URL'],"wap.mfd.com"))
        {
            $url = "http://mfd.mfd.com";
        }
        elseif (stripos($config['SITE_URL'],"wap.dev.mfd.cn"))
        {
            $url = "http://www.dev.mfd.cn/";
        }
        elseif (stripos($config['SITE_URL'],"wap.test.mfd.cn"))
        {
            $url = "http://www.test.mfd.cn/";
        }
        elseif (stripos($config['SITE_URL'],"wap.mfd.cn"))
        {
            $url = "http://www.mfd.cn/";
        }
...
function &gt($type, $params = array())
{
    ...
    {
        /* 加载订单类型基础类 */
        include_once(ROOT_PATH . '/h5/includes/goods.base.php');
        include(ROOT_PATH . '/h5/includes/goodstypes/' . $type . '.gtype.php');
...
function &ot($type, $params = array())
{
    ...
    {
        /* 加载订单类型基础类 */
        include_once(ROOT_PATH . '/h5/includes/order.base.php');
        include(ROOT_PATH . '/h5/includes/ordertypes/' . $type . '.otype.php');
 ...
 *    @author    Ruesin
 */
function &ots($type, $params = array())
{
    static $order_type = null;
    if ($order_type === null)
    {
        /* 加载订单类型基础类 */
        include_once(ROOT_PATH . '/h5/includes/orders.base.php');
        include(ROOT_PATH . '/h5/includes/orderstypes/' . $type . '.otype.php');
        $class_name = ucfirst($type) . 'Orders';
        $order_type = new $class_name($params);
    }
 ...
 * @return 没有返回值
 */
function Qrcode($type='goods',$id=1000,$text='http://rctailor.ec51.com.cn/'){
...
*     'location_url'   => 跳转地址
*     'price'          => 3000
*     'type            => 1.交易提醒, 2.晒单评论, 3.订单评论, 4.消费者发需求, 5.裁缝发作品, 6.裁缝发报价,7中标,[8,9,10]投诉处理
*  )
*/
