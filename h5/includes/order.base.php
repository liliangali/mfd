    ...
    function respond_notify($order_id, $notify_result)
    {
        ...
        switch ($notify_result['target'])
        {
            case ORDER_ACCEPTED:
                ...
                //$data['status'] = ORDER_ACCEPTED;       //已支付  //待审核

                //$mSet = &af('settings');
                //$ck = $mSet->getOne('order_if_review');
                //if($ck == 'no'){   //不用审核的付款之后直接加入到队列表并把状态改为生产中
                //if($ck != 'yes'){   //不用审核的付款之后直接加入到队列表并把状态改为生产中    //时不时的获取不到配置项  不明所以
//                     $res = $this->_mod_ordercron->_addToList($order_id);
//                     if(!$res){
...
//                     }
     ...
     * @author Ruesin
     */
    function _lessen_stock($order_id = 0){

        //$this->_mod_ordergoods->find();
     ...
     *    @return    string
     */
    function _gen_order_sn($city_id = 0)
    {
        /* 选择一个随机的方案 */
        ...
        $y = date('y', $timestamp);
        $z = date('z', $timestamp);
        $order_sn = (2000+$city_id).$y . str_pad($z, 3, '0', STR_PAD_LEFT) . str_pad(mt_rand(1, 99999), 5, '0', STR_PAD_LEFT);
...
// 		}
        ...
        return array(
            ...
            'source_lang'   => $order['source']['lang'],
            'source_brand'  => $order['source']['brand'],
            'source_city'   => $order['source']['city'],
            'source_store'  => $order['source']['store'],
        );
    }
    ...
    function _valid_consignee_info($consignee)
    {
        if (!$consignee['consignee'])
        {
            ...
            $this->_error('收货信息不完整，请去地址管理中完善收货地址！');
