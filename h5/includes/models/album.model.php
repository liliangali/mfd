...
class AlbumModel extends BaseModel
{
	...
	function delById($id){
		...
		$sql = "delete from rc_album where id = $id";
		$db->query($sql);
	...
	function getByUidCate($uid,$cate = ''){
		...
		$sql =  "select * from rc_album where $where";
	...
	function getRecommed($cate = '' ,$uid = '' , $desc = '' , $limit = 4){
		...
		$sql = "select * from rc_album where $where $desc $limit ";
// 		echo $sql;
		$rs = $db->getAll($sql);
		return $rs;
	}
	...
	function pageByCateUid($curr_oage = 1 , $page = 10,$cate = 0 ,$uid = 0 ,$desc = ''){
		...
		$sql = "select * from rc_album where $where $desc limit ".$page->mLimit[0]." , " .$page->mLimit[1];
		$rs = $db->getAll($sql);
		foreach($rs as $k=>$v){
	...
	function cntByCateUid($cate = 0 ,$uid = 0){
		...
		$sql = "select count(*) as total from rc_album where $where  ";
// 		echo $sql;
		$rs = $db->getRow($sql);
		return $rs['total'];
	}
