 ...
 * --
 *
 * @copyright Copyright (c) 2014 – 2015 www.alicaifeng.com
...
class check {
	...
	/**	 * 验证短信｜邮箱时效性	 *
	 * @param int          $fail_time      当前失效时间	 * @param string     $type              phone|email	 * @return boolean	 */	public static function isPrescription($fail_time,$type='phone') {
		/* 手机 验证失效延长至 3分钟，邮件5分钟 */
		$sdef = array('phone'=>240,'email'=>440);
		//echo intval($fail_time+$sdef[$type]) .'>'. time();
		if(intval($fail_time+$sdef[$type]) > time())
		{
			return true;
		}
		return false;	}
     ...
     * 验证手机短信 验证码
     *
     * $res = check::isMobileCode('reg', 'reg', 18510332021, 4685);
     *   if ($res)
     *   {
     *       echo $res;
     *   }else {
     ...
     *   }
     * @access public
     * @param string $category  大分类
     * @param string $type      小分类
     * @param integer $phone    手机号码
     * @param integer $code     短信验证码
     * @return NULL | string
     */
    public static function isMobileCode($category, $type, $phone,$code) {

        if (!$category || !$type || !$phone || !$code)
        {
            return '参数错误!';
        }
        $sms_reg_tmp_mod =& m('sms_reg_tmp');
        $conditions = array('conditions'=>" category = '$category' and type = '$type' and phone = '$phone' and code='$code' ");
        $sms_log = $sms_reg_tmp_mod->get($conditions);
        if (!$sms_log['id'])
        {
            return '请重新获取验证码!';
        }
        if(self::isPrescription($sms_log['fail_time']))
         {
         return '验证码已过期!';
         }
        return '';
