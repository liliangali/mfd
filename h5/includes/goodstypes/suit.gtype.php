...
//             var_dump($region);
        ...
        if ($post['figuretype'] == 2){
            ...
            ##门店
            $msv = m('serve');
            $server = $msv->get("region_id = '{$post['region']}' AND idserve = '{$post['server_id']}'");
            if(!$server){
                $this->_error('服务门店错误!');
                return false;
            }
    ...
    function add($post)
    {
    	...
    	if($_Cgoods && count($goods) == $_Cgoods){
        	$post['quantity'] += $_Cgoods['quantity'];
        	$post['ident']    = $_Cgoods['dis_ident'];
    ...
    function _base_info($goods_id){
        ...
        $data = $mSuit->get(array(
                'conditions' => " suit_list.id = '{$goods_id}' ", // AND suit_list.id = suit_relation.tz_id
