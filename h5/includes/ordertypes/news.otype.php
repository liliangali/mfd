    ...
    function submit($data){
    	...
    	foreach ((array)$goods as $key => $value)
    	{
    		...
    		$goods_items[] = array(
    		    ...
    		    'fav_name'      =>$favname,
    		    //'fav_price'      => $value['newPrice']['subtotal']-$value['subtotal'],
...
//    	echo '<pre>';print_r($goods_items);exit;
...
//        	}
//    	}
//    	echo '<pre>';print_r($_order['debit']['data']);exit;
        ...
        foreach ((array)$objs as $key=>$row){
            ...
            if($row['list']){
                ...
                foreach ($row['list'] as $arr){                    
                    ...
                    $arr['favprice']= $row['subtotal_m']-$row['subtotal'];
    ...
    function _handle_order_info($order, $cart)
    {
        ...
        $final_amount = $cart['final_amount']+$order['ships']['defship']['post_fee']-$order['discount'];
        if($final_amount < 0)
        {
            $final_amount = 0;
        }
        ...
        return array(
                ...
                'final_amount'   => $final_amount,//订单最后支付价格 订单价格+物流费用-优惠价格
                ...
                'ship_name'      => isset($order['shipping']['address']['consignee']) ? $order['shipping']['address']['consignee'] : '',
    ...
    function respond_notifyssss($order_id, $notify_result)
    {
        ...
        switch ($notify_result['target'])
        {
        	case ORDER_ACCEPTED:
        	    ...
        	    $data['status'] = ORDER_ACCEPTED;       //状态直接进入审核中
