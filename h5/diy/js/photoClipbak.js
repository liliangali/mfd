...
(function(root, factory) {
	...
	function photoClip(container, option) {
		...
		$file.on("change", function() {
			...
			// alert(files+"|||"+files.name +"|||"+ files.size +"|||"+ files.type)
outputType = files.type;
			if (!/image\/\w+/.test(files.type)  && (files.size <= 0)) {
		...
		function clipImg() {
			...
			var dataURL = canvas.toDataURL(outputType, 0.8);
