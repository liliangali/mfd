...
class RCTailor
{
    ...
    {
        /* 加载初始化文件 */
        require(ROOT_PATH.'h5/eccore/controller/app.base.php'); //基础控制器类
        require(ROOT_PATH.'h5/eccore/model/model.base.php');    //模型基础类
        ...
        $app    = isset($query_info['app']) ? preg_replace('/(\W+)/', '', $query_info['app']) : $default_app;
...
{
    include_once(ROOT_PATH.'h5/eccore/view/template.php');
    if ($is_new)
    {
...
function &bm($model_name, $params = array(), $is_new = false)
{
    ...
    if ($is_new || !isset($models[$model_hash]))
    {
        $model_file = ROOT_PATH.'h5/includes/models/' . $model_name . '.model.php';
        if (!is_file($model_file))
        {
            /* 不存在该文件，则无法获取模型 */
            return false;
...
function &f ($filename){
    ...
    $file = ROOT_PATH.'h5/includes/libraries/' . $filename . '.lib.php';
...
function &db()
{
    ...
    if ($db === null)
    {
        ...
        if ($cfg['scheme'] == 'mysql')
        {
            ...
            $db = new cls_mysql();
            $db->cache_dir = ROOT_PATH. 'h5/temp/query_caches/';
...
function clean_cache()
{
    /*清理缓存*/
    $cache_dirs = array(
        ROOT_PATH.'h5/temp/caches',
        ROOT_PATH.'h5/temp/compiled/mall/admin',
        ROOT_PATH.'h5/temp/compiled/mall/',
        ROOT_PATH.'h5/temp/compiled/store/admin',
        ROOT_PATH.'h5/temp/compiled/store',
        ROOT_PATH.'h5/temp/js',
        ROOT_PATH.'h5/temp/query_caches',
        ROOT_PATH.'h5/temp/tag_caches',
        ROOT_PATH.'h5/temp/style',
    );
    ...
    /*主分类缓存数据*/
    if (is_file(ROOT_PATH.'h5/temp/query_caches/cache_category.php'))
    {
        unlink(ROOT_PATH.'h5/temp/query_caches/cache_category.php');
    }

    /*清除一个周前图片缓存并回收多余目录*/
    $expiry_time = strtotime('-1 week');
    $path = ROOT_PATH.'h5/temp/thumb';
    $d = dir($path);
    if ($d)
    {
        while(false !== ($entry = $d->read()))
