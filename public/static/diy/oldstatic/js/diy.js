...
function Custom(options){
	...
	this.uf=null;//0:未完成首单体验款 1:是
	this.api=function(){
		var url=window.location.href;
		if(url.indexOf('test.cotte.cn')>=0){
			return 'http://api.diy.test.cotte.cn/v1/';//测试环境
		}else if(url.indexOf('m.cotte.cn')>=0){
			return 'http://api.diy.cotte.cn/v1/';//线上环境	
		}else{
			return 'http://api.diy.dev.cotte.cn:8080/v1/';//开发环境
		}
	}
	this.cartUrl='http://'+location.host;
...
Custom.prototype.setOptHtml=function(obj){
	var html={
		...
		embroidery:'<div id="opt-embroidery">\
					<div class="titBar"><span class="tit fl js-back">签名</span><span class="save fr">保存</span><span class="del fr">取消</span></div>\
					<div class="scrollBox">\
						<div class="itemList">\
...
Custom.prototype.checkDefault=function(obj){
	...
	function defaults(){
		...
		if((type=='lining'||type=='studs')&&data[type]==obj.pid+':'+obj.id){
			return ' class="cur"'
		}
		if(type=='button'){
			if(data[type].indexOf(obj.pid+':'+obj.id)>=0){
				return ' class="cur"'	
			}	
		}
		//数组格式的匹配
...
Custom.prototype.getFabricCat=function(){
	function addHtml(data){
		...
		if((len+1)%3!=0){
			for(var g=0;g<(3-(len+1)%3);g++){
				cat.push('<li class="null">&nbsp;</li>')	
			}	
		}

		//渲染面料一级分类并绑定事件
		$('#opt-fabric .cat2-1').html('<ul>'+cat.join('')+'</ul>').find('li').unbind('click').click(function(){
...
Custom.prototype.getFabric=function(n,options){
	...
	function addHtml(start,end){
		...
		for(var i=start;i<end;i++){
			...
			//活动面料（校园）
			if(data[i].activity==1 && data[i]['activity_price_'+_diy.param.clothing] > 0){
	...
	function btwHtml(data){
		...
		for(var i=0;i<len;i++){
			...
			if(data[i].selected){//更新str默认值
				(function(i){
					var pid=data[i].parentid,n,flag=null,id=data[i].id,arr=_diy.param.strData[_diy.param.clothing].styles,len=arr.length;
					for(var g=0;g<len;g++){
						(arr[g].indexOf(pid+':')>=0) && (flag=true,n=g)
					}
...
Custom.prototype.styleType={
	weitiao:[1119,35459,2133,2089,2067,2031],
	twocat:[176,2089]
}
//款式二级数据
		...
		for(var i=0;i<len;i++){
			...
			if(istwocat){//如果有二级分类
				...
				if(data[i].imgurl!=null){
					...
					html.push('<li'+cur+' data-id="'+data[i].id+'" data-pid="'+data[i].parentid+'"><img src="http://m.cotte.cn/app-diy/images/error.png"><p>'+data[i].name+'</p></li>');
				}
			}
		}
		if(istwocat){
		...
		for(var i=0;i<len;i++){
			...
			if(data[i].imgurl){
				if(data[i].name!='无'&&data[i].id!=36182&&data[i].id!=62240){//临时过滤
					html.push('<li'+(checkFocus?checkFocus:'')+' data-id="'+data[i].id+'" data-code="'+data[i].ecode+'" data-pid="'+data[i].parentid+'"><img src="'+data[i].imgurl+'"><p>'+data[i].name+'</p></li>');
				}
			}
		}
...
Custom.prototype.selectSyLine=function(){
	$('#opt-keyhole .itemList').off('click').on('click','li',function(){
		$(this).addClass('cur').siblings().removeClass('cur');
		//同步全局工艺变量
		var pid=this.getAttribute('data-pid'),id=this.getAttribute('data-id'),code=this.getAttribute('data-code'),mid=$('#nav-keyhole').attr('data-id'), str=pid+':'+id+'|||'+code;
...
Custom.prototype.selectBtnLine=function(){
	...
	function fn(){
		if(flag){
			...
			//同步全局工艺变量
			_diy.param.strData[_diy.param.clothing].studs[0]=pid+':'+id+'|||'+code;
			//获取数据
			_diy.getDiyData2({
				pid:pid,
				ids:id,
