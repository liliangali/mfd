...
function Custom(options){
	...
	this.api='http://api.diy.cotte.cn/v2/';//线上环境
	// 'http://api.diy.test.cotte.cn/v2/';//测试环境
	// 'http://api.diy.dev.cotte.cn:8080/v2/';//开发环境
	this.log=true;//log开关
	this.cache='';//控制ajax全局缓存
	this.csql='';
	this['access-token']='101-token';
	this.pwd=null;//token for cookie
	this.style=null;//style for url
	this.clothing=null;//定义初始的ID
	this.clothingCur=null;//当前的ID
	this.strData={}//diy工艺数据
}
...
*=========================*/
	init:function(data){
		...
		_diy.clothing=paramList[0];
		_diy.clothingCur=paramList[0];
		_diy.style=paramList[1];
		_diy.works.id=paramList[2]==0?false:paramList[2];
	...
	newUpData:function(url,rotate){
		...
		img.onload=function(){
			...
			if(!rotate){//如果不是旋转则更新数据，防止保存、下单生成侧身图片
				_diy.diyData[_diy.clothingCur]=url;
			}
		}	
	},
	/*收集工艺参数*/
	...
	getWeitiao:function(pid,mid,ids,box){
		$.ajax({
			...
			success: function(d){
				 ...
				 * 特殊处理/S
				 */
				if(_diy.clothingCur=='0003'&&pid==50){
					$.each(d.options,function(i,d){
						if(d.selected){
							_diy.upDataCstr(d.parentid,d.id);
						}	
			...
			for(var i=start;i<end;i++){
				...
				if(_diy.uf==0&&data[i].is_first==1){//设置限量（首单体验款面料）		
					if(_diy.clothing=='0003'||_diy.clothing=='0004'||_diy.clothing=='0006'){
						//hd_price="<br>￥"+data[i]['activity_price_'+_diy.clothingCur];
				...
				html.push('<li'+(checkFocus?checkFocus:'')+hd_flag+' data-code="'+data[i].code+'"><img src="'+data[i].imgurl+'" ><p>'+data[i].tname+hd_price+'</p>'+hd_ico+(data[i].code=="SKP025A"||data[i].code=="SAK567A"?"<span class='ico-mt'></span>":"")+'</li>');//新增免烫面料标签						
...
*================================*/
	...
	getSizeTable:function(){
		...
		$('#sizeTable .scrollBox').html(layer.join('')).on('click','li',function(){
			$(this).addClass('cur').siblings('.cur').removeClass('cur');
			//更新尺码
			_diy.strData[$('#sizeTable .tabBar .cur').attr('data-id')].sizes=$(this).attr('data-id');
		})	
		...
		function addHtml(data,n){
			...
			for(i=0;i<len;i++){
				...
				/*if(data[i].Dv){ //取消默认尺码
					_diy.strData[$('#sizeTable .tab li').eq(n).attr('data-id')].sizes=data[i].Id;
				}*/
			}
...
*=======================================*/
	...
	underOrder:function(){
		if(!app.checkState()){//未登录操作，与app对接跳转登录页面
			app.loginIn();
			return	
		}
		_diy.pwd=JSON.parse(app.checkState()).result.member.api_auth_code //调用app.js方法	
		_diy.loading.show(2);
		$('#zhezhaoLayer').unbind('click').css('z-index',110);
		_diy.generatePic(function(n){
			_diy.strData[_diy.categoryId[n]].image=_diy.diyData[_diy.categoryId[n]];
			if(n ==_diy.categoryId.length-1){
				$.ajax({
					url:'http://m.cotte.cn/cart-add.html?token='+_diy.pwd,
					type:'POST',
					timeout:5000,
					dataType:"json",
					data:{
					...
					success:function(data){
						if(data.done){
							//window.location.href='http://m.cotte.cn/cart.html?token='+_diy.pwd
							_diy.loading.hide();
							$('#zhezhaoLayer').removeAttr('style');

							var href = 'http://shop.diy.cotte.cn/cart.html?token='+_diy.pwd;
					
							//获得共用模板组
							var template = app.getTemplate('gotocart', '../template.html');;
							...
							headerWebview.show("slide-in-right", 240);
						}else{
					...
					error:function(){
						...
						console.log('http://m.cotte.cn/cart-add.html?token='+_diy.pwd)
					}
				});	
			}	
		})
...
//					_diy.strData=localStorage.diyData
					...
					success:function(data){
						...
						if(isWorks){
							...
							if(typeof obj=='function'){
								obj('http://wap.cotte.cn/'+data.url);
							}
						}
					},
					error:function(){
