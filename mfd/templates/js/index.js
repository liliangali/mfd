...
function initTopTab(){
    $.each(menu, function(k, v){
        var item = $('<li><a class="link" href="javascript:;" id="tab_' + k + '"><i></i><span>' + v.text + '</span><b></b></a></li>');
...
function addHistoryItem(tab, item){
    ...
    if($(id).length == 1){
        ...
        var lnk = $('<a href="javascript:;" id="' + tab + '-' + item + '">' + menu[tab]['children'][item]['text'] + '</a>').css({"color":"#333"});
        var close = $('<a href="javascript:;" class="close"><img src="templates/style/images/close.png" / ></a>');
        lnk.click(function(){
            openItem(item, tab);
        });
