...
class ArticleApp extends BackendApp
{
    ...
    function index()
    {
        ...
        'order'   => 'article.add_time DESC, article.sort_order ASC', //必须加别名
        'count'   => true   //允许统计
        ));    //找出所有的文章
    ...
    function add()
    {
        if (!IS_POST)
        {
            ...
            $article = array('cate_id' => $cate_id, 'sort_order' => 255,  'if_show' => 1);
...
/*             $this->assign('build_editor', $this->_build_editor(array(
        ...
        else
        {
            ...
            {
                return $this->show_warning('请选择所属分类');
            }
//            if(!$_POST['img']){
//                return $this->show_warning('麦富迪讲堂的文章要上传展示图片');
//            }
//            if(!$_POST['brief']){
//                return $this->show_warning('麦富迪讲堂的文章要填写简介');
//            }
            ...
            $this->show_message('add_article_successed',
                'back_list', 'index.php?app=article&amp;act=index&amp;title='.$_POST['title'].'&amp;cate_id='.$_POST['cate_id'],
...
/*     	if (!empty($_GET['ret_url']))
        ...
        {
        	/*原url带有分类  */
        	$ret_url =str_replace('&', '&amp;',basename($_SERVER['HTTP_REFERER']));
        	setcookie('ret_url',$ret_url);
            /* 当前文章的附件 */
            $files_belong_article = $this->_uploadedfile_mod->find(array(
...
/*             if($article['code']!=null)
        ...
        $this->show_message($message,'back_list',$ret_url);
    }
    ...
    /*检查该文章的分类是否是小讲堂*/
    function is_classroom($id){
        $config=new Config();
        $settings=$config->get_config();
        // $img=empty($_REQUEST['img'])?'':trim($_REQUEST['img']);
        // $brief=empty($_REQUEST['brief'])?'':trim($_REQUEST['brief']);
        $article=$this->_acategory_mod->get($id);
        // var_dump($settings['help_center_code']);
        // var_dump($article);
        if($article['code']!=$settings['help_center_code']){
            // return $this->json_result('通过');
            return false;
        }else{
            return true;
            // if($img && $img!='undefined' && $brief){
            //    return $this->json_result('通过');
                
            // }else if(!$brief){
            //     return $this->json_error('麦富迪讲堂的文章要填写简介','brief');
            // }else{
            //     return $this->json_error('麦富迪讲堂的文章要上传展示图片','img');
            // }
