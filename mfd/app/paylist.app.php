...
/* 收支明细控制器 */
class PaylistApp extends BackendApp
{

    var $order_cash_log_mod;
    function __construct()
    {
        $this->UserApp();
        $this->assign('status',array(0=>'审核中',1=>'审核成功',2=>'审核失败'));
        $this->assign('all',array(1=>'充值',2=>'订单',3=>'推荐',4=>'提现'));
        $this->assign('spending',array(1=>'订单',2=>'提现'));
        $this->assign('income',array(1=>'充值',2=>'订单',3=>'推荐'));
    ...
    {
        parent::__construct();
        $this->order_cash_log_mod = m('ordercashlog');
    }

    function index()
    {
        $type = array(0=>'收益',1=>'积分',2=>'麦富迪币',4=>'余额');
        ...
        $list = $this->order_cash_log_mod->find(array(
            'join' => 'belongs_to_member',
            'fields'	 => "id, user_name,nickname,order_cash_log.*",
       ...
       foreach ($list as $key => $value) 
       {
           ...
           $list[$key]['type_name'] = $type[$value['type']];
           $list[$key]['add_time'] = date('Y-m-d H:i:s',$value['add_time']);
       }
        $this->assign('list', $list);
        $page['item_count'] = $this->order_cash_log_mod->getCount();
        ...
        $this->display('paylist/paylist.all.html');
    ...
    function income()
    {
        ...
        if($_GET['type_name']){
            $conditions .= " AND type=".$_GET['type_name'];
        }
        $page = $this->_get_page(20);
        $money_list = $this->_money_mod->find(array(
            'join' => 'belongs_to_member',
            'fields'	 => "id, member.user_id,user_name,nickname,change_money as change_money,addtime,type",
            'conditions' => 'change_money > 0'.$conditions,
            'order'      => 'id DESC',
            'limit' => $page['limit'],
            'count' => true,
        ));

        foreach($money_list as $key=>$val) {//价格处理
            $money_list[$key]['change_money'] = number_format(floor($val['change_money']), 2);
        }
        $type_name = array(
            '1' => '充值',
            '2' => '订单',
        ...
        foreach($money_list as $key=>$val) {
            $money_list[$key]['type_name'] = $type_name[$val['type']];
        }
        //print_exit($money_list);
        $this->assign('money_list', $money_list);

        $page['item_count'] = $this->_money_mod->getCount();
        $this->_format_page($page);
        $this->assign('page_info', $page);
        /* 导入jQuery的表单验证插件 */
        $this->import_resource(array(
        ...
        $this->display('paylist/paylist.income.html');
    ...
    function spending()
    {
        ...
        if($_GET['type_name'] ){
            $m_conditions = " AND type=".$_GET['type_name'];
            $c_conditions = " AND 1!=1";
        }
        $page = $this->_get_page(20);
        $money_list = $this->_money_mod->find(array(
            'join' => 'belongs_to_member',
            'fields'	 => "id, member.user_id,user_name,nickname,change_money as change_money,addtime,type",
            'conditions' => 'change_money < 0'.$conditions.$m_conditions,
            'limit' => $page['limit'],
            'count' => true,
        ));
        $count = $this->_money_mod->getCount();
        $cash_list = $this->_cash_mod->find(array(
            'conditions' => "cash.status=1".$conditions.$c_conditions,
            'join'       => 'belongs_to_member',
            'fields'     => "id,member.user_id,user_name,nickname,cash_money as change_money,create_time as addtime,cash.status",
            'count'		 => "true",
            'limit'      => $page['limit'],
            'index_key'	 => '',
        ));
        ...
        //把支出记录和提现记录合并
        $count = $count + $this->_cash_mod->getCount();

        $money_list = array_merge($money_list,$cash_list);
        $time = array();
        foreach ($money_list as $val) {
            $time[] = $val['addtime'];
        }
        array_multisort($time, SORT_DESC, $money_list);
        $status = array(
            '1' => '提现成功',
            '2' => '提现失败',
        );
        //处理状态结果
        foreach($money_list as $key=>$val) {
            $money_list[$key]['status_name'] = $status[$val['status']];
        }
        ...
        $this->display('paylist/paylist.spending.html');
    ...
    function cash()
    {
        ...
        if($_GET['status']>-1){
            $conditions .= " AND cash.status=".$_GET['status'];
        }
        $cash_list = $this->_cash_mod->find(array(
            'conditions' => "1=1".$conditions,
            'join'       => 'belongs_to_member',
        ...
        //处理状态结果
        foreach($cash_list as $key=>$val) {
            $cash_list[$key]['status_name'] = $status[$val['status']];
        }


        foreach($cash_list as $key=>$val) {//价格处理
            $cash_list[$key]['change_money'] = number_format(floor($val['change_money']), 2);
        }
        $type_name = array(
            '1' => '充值',
            '2' => '订单',
        ...
        //处理状态结果
        foreach($cash_list as $key=>$val) {
            $cash_list[$key]['type_name'] = $type_name[$val['type']];
        }
//print_exit($cash_list);
        $this->assign('money_list', $cash_list);
        $page['item_count'] = $this->_cash_mod->getCount();
        $this->_format_page($page);
        $this->assign('page_info', $page);
        /* 导入jQuery的表单验证插件 */
        $this->import_resource(array(
        ...
        $this->display('paylist/paylist.cash.html');
