...
/*             if($acategory['code'])
   ...
   function ajax_col()
   {
    ...
    function drop()
    {
        ...
        $ids = explode(',', $id);
        $del_flag=0;
        $del_articles='请先删除该分类下的文章，再删除该分类，所有要删除的文章：';
        $message = 'drop_ok';
        foreach ($ids as $key=>$id){
            $acategory=$this->_acategory_mod->find("parent_id={$id}");
            if($acategory){
            	$acids=i_array_column($acategory, 'cate_id');
            	$acids[]=$id;
            }else{
            	$acids[]=$id;
            }
            $ids=array_values($acids);
/*             if($acategory['code']!=null)
            ...
            $articles=$this->_article_mod->find(array(
            		'conditions'=>"1=1 and ".db_create_in($acids,'cate_id')
            ));
            if($articles){
            	$del_flag=1;
            		foreach ($articles as $k=>$v){
            			$del_articles.="{文章编号：{$v['article_id']},文章标题：{$v['title']}},";
            		}
            }
        }
        if($del_flag){
        	$this->show_warning($del_articles);
		...
		$res=$this->_acategory_mod->drop($ids);
    ...
    function ajax_code_unique()
    {
    	...
    	if($code){
    		$acategory=$this->_acategory_mod->find(
    				array("conditions"=>"code='$code'")
    				);
    		if($id){
    			if($acategory && !$acategory[$id]){
    				echo ecm_json_encode(false);
    				return ;
    			}else{
    				echo ecm_json_encode(true);
