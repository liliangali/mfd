...
/* 主题分类 */
class Jpjz_discatApp extends DisApp
{
	var $_config;
    function __construct()
    {
    	$this->_config = ROOT_PATH . '/data/dissertation/jpjz_config.php';
        parent::__construct();
    }

    function index()
    {
 		$cats = $this->jpjz_disCats();
		...
		if(!IS_POST)
		{
			$this->assign('_tmp_list', $_tmp);
	 		$this->display("jpjz_discat.index.html");
		}
		else
		{
			$_data = array();
			...
			$this->show_message('保存成功!',
    					'back_list', 'index.php?app=jpjz_discat');
		}
    }
}
