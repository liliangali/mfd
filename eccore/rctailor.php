...
class RCTailor
{
	 ...
	 * 版本升级 5.4+
	 *
	 * preg_replace函数已经在php5.5.x版本中废除 可以把preg_replace函数替换成preg_replace_callback函数
	 */
	static function rpreg_replace($pattern, $replacement, $source){
		$php_version = phpversion();
		$wp_version='5.5';
		if(version_compare($php_version,$wp_version,'>'))
		{
			$source = preg_replace_callback($pattern, function($match){return $replacement;}, $source);
			return $source;
		}else{
    ...
    static function startup($config = array())
    {
        ...
        $pathinfo = base_request::get_path_info();
        
    

        $query_info = self::parse_query($pathinfo);
        
        $query_args = explode("-", $query_info["query"]);
        ...
        $app    = isset($query_info['app']) ? self::rpreg_replace('/(\W+)/', '', $query_info['app']) : $default_app;

        $act    = isset($action) ? trim($action) : $default_act;
    ...
    function parse_query($query){
    	...
    	if(count($matchs[0]))  return array('query'=>$query, 'app'=>$matchs[1][0]);
...
class Lang
{
    ...
    {
        /* 修复bug 出发错误数组格式  add by Xiao5 */
        if (is_array($key))
        {
            foreach ($key as $v)
            {
                if (Lang::_valid_key($v['msg']) == false)
                {
                    return $v['msg'];
                }
                $vkey = $v['msg'] ? strtokey("{$v['msg']}", '$GLOBALS[\'__ECLANG__\']') : '$GLOBALS[\'__ECLANG__\']';
...
class Conf
{
    ...
    {
        $old_conf = isset($GLOBALS['RCTailor_CONFIG']) ? $GLOBALS['RCTailor_CONFIG'] : array();
        if (is_string($conf))
        {
            $conf = include($conf);
        }
        ...
        {
            $GLOBALS['RCTailor_CONFIG'] = array_merge($old_conf, $conf);
        }
        else
        {
            $GLOBALS['RCTailor_CONFIG'] = $conf;
        }
    }
    /**
     *    获取配置项
    ...
    static function get($key = '')
    {
        $vkey = $key ? strtokey("{$key}", '$GLOBALS[\'RCTailor_CONFIG\']') : '$GLOBALS[\'RCTailor_CONFIG\']';
...
/**
* @deprecated 应用多语言配置:主要包括多语言的表示、数据、模板名称...
* @return array
* @see ecm_langs
* @version 1.0.0 (2014-11-12)
* @author Xiao5
*/
function ecm_langs()
{
    ...
    $return = include_once(ROOT_PATH . '/data/languages/config.php');
    return $return;
}
/**
 * 切换当前的数据库连接
 ...
 * @return Model
 */
function &db($linkNum='sc-utf-8',$config='',$force=false)
{
    /* 导入多语言配置  */
    $lang_settings = ecm_langs();
    include_once(ROOT_PATH . '/eccore/model/mysql.php');
    
    static $db = array();
    if('sc-utf-8' === $linkNum && isset($db[$linkNum]))
    {
        return $db[$linkNum];
    }

    $cfg = parse_url($lang_settings[LANG]['db']);
    ...
    if(!isset($db[$linkNum]) || $force ) {
        ...
        if ($cfg['scheme'] == 'mysql')
        {
            ...
            $db[$linkNum] = new cls_mysql();
            $db[$linkNum]->cache_dir = ROOT_PATH. '/temp/query_caches/';
            $db[$linkNum]->tname = $cfg['path'];
            $db[$linkNum]->connect($cfg['host']. ':' .$cfg['port'], $cfg['user'],
...
function site_url()
{
	...
	{
		return get_domain() . substr(PHP_SELF, 0, strrpos(PHP_SELF, 'index.php/'));
	}else{
		return get_domain() .'/';
	}
}
...
function is_mobile($str)
{
	...
	// 	$pattern = "/^13[0-9]{9}|15[0|1|2|3|5|6|7|8|9]d{8}|18[0|5|6|7|8|9]d{8}/";
...
{
   //迁移至includes/libraries/ipCity.class.php getGeoip
}
...
function ecm_define($source)
{
    ...
    foreach ($source as $key => $value)
    {
        ...
        {
            /* 定义语言项目切换 -- add by xiao5 2014-11-07 */
            if (ecm_getcookie('lang') && strtoupper($key) == 'LANG' && in_array(ecm_getcookie('lang'), array_keys($lang_settings)))
            {
