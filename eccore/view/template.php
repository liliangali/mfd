...
class ecsTemplate
{
    ...
    function fetch_str($source)
    {
        ...
        /* add by xiao5 升级5.4+ @preg_replace*/        if (version_compare(PHP_VERSION,'5.5','>'))        {        	return preg_replace_callback("/{([^\}\{\n]*)}/", function($r) { return $this->select($r[1]); }, $source);        }else{        	return preg_replace("/{([^\}\{\n]*)}/e", "\$this->select('\\1');", $source);        }
    }
    ...
    function select($tag)
    {
        ...
        else
        {
            ...
            switch ($tag_sel)
            {
                ...
                case 'link':
                	$t = $this->get_para(substr($tag, 4), 0);
                	return '<?php echo $this->build_url(' . $this->make_array($t) . '); ?>';
                	break;
                case 'ecjoin':
                ...
                case 'input_img':
                    ...
                    return '<?php echo $this->input_img(' . $this->make_array($t) . '); ?>';
                    break;
                case 'input_object':
                	$t = $this->get_para(substr($tag, 9), 0);
                	
                	return '<?php echo $this->input_object(' . $this->make_array($t) . '); ?>';
                	break;
                
            	case 'input_obj':
            	    $t = $this->get_para(substr($tag, 9), 0);
            	     
            	    return '<?php echo $this->input_obj(' . $this->make_array($t) . '); ?>';
            	    break;
            	 case 'avatar':
            	    $t = $this->get_para(substr($tag, 6), 0);
            	    return '<?php echo $this->get_avatar(' . $this->make_array($t) . '); ?>';
            	    break;
            	 case 'uname':
            	    $t = $this->get_para(substr($tag, 5), 0);
            	    return '<?php echo $this->get_uname(' . $this->make_array($t) . '); ?>';
            	    break;
            	 case 'nav_uname':
            	 	$rs = explode(" ", $tag);
            	 	$rs = $this->nav_uname($rs[1]);
            	 	echo $rs;
            	 	break;
            	 case 'userphoto':
            	    $t = $this->get_para(substr($tag, 9), 0);
            	    return '<?php echo $this->get_userphoto_img(' . $this->make_array($t) . '); ?>';
            	    break;
            	    case 'qrcode':
            	    	$t = $this->get_para(substr($tag, 6), 0);
            	    	return '<?php echo $this->get_qrcode_img(' . $this->make_array($t) . '); ?>';
            	    	break;
            	 case 'encrypt':
            	    $t = $this->get_para(substr($tag, 7), 0);
            	    return '<?php echo $this->encrypt(' . $this->make_array($t) . '); ?>';
            	    break;
                default:
                    return '{' . $tag . '}';
                    break;
     ...
     * 获取二维码
     */
    function get_qrcode_img($params){
    	$type = $params['type'];
    	$id = $params['id'];
    	$size = $params['size'];
    	$src = getQrcodeImage($type,$id,$size);
    	return '<img src='.$src.'>';
    }
    /**
     * 设计上传-用户的照片
     */
    function get_userphoto_img($params){
    	$file_name = $params['file_name'];
    	$size = $params['size'];
    	$cate = $params['cate'];
    	if ($cate == 1) {
    		return getDesignUrl($file_name,$size);
    	}elseif ($cate ==2){
    		return getCameraUrl($file_name,$size);
    	}
    }
    /**
     * 获取头像标签
    ...
    function get_avatar($params){
    	$info = getUinfoByUid($params['uid']);
    	return $info['avatar'];
//     	return getAvatarByFile($info['avatar'],$params['size']);
    }
    /**
     * 获取用户昵称
     * uid 	用户id
     * sub	截取长度
     */
    function get_uname($params){
    	$name = getUnameByUid($params['uid']);
    	return sub_str($name,$params['sub']) ;
    }
    /**
     * 字符串加密解密
     * @author 高飞
    ...
    function encrypt($params){
        return encrypt($params['string'], $params['operation'], $params['key']);
    }
    /**
     * 处理 input_obj  标签
     *
     ...
     * @return  str
     */
    function input_obj($params){
//         if(!$params || !isset($params['name'])) return;
        
        $text    = empty($params['text']) ? '请选择' : $params['text'];
        $name    = $params['name'];
        $class   = empty($params['class']) ? 'input_object' : $params['class'];
        $id      = empty($params['id']) ? 'input_object' : $params['id'];
        $type    = empty($params['type']) ? 'checkbox' : $params['type'];
        $post    = $params['post'];
        ...
        $str = "<lable><a href='javascript:;' class='{$class}' id='{$id}'>{$text}</a>
		<input type='hidden' id='input_{$id}' value='{$value}' name='{$name}'></lable>";
        
        if(!isset($model) || $model=='')return $str;
        
        $str.="<script>
				$(document).ready(function(){
					$('.{$class}').click(function(){
    					var _data = new Object();
            			_data.ids=$('#input_{$id}').val();
            			_data.type='{$type}';
            			_data.param='{$param}';
            			_data.post='{$post}';
						var ws = 'dialogHeight=500px;dialogWidth=710px;resizable=no;dialogLeft=500px;';
						var param ='&objmodel={$model}{$param}'
						var _back =window.showModalDialog('index.php?app=inputobj'+param,_data,ws);
						if(_back){
                            $('#input_{$id}').val(_back.ids);
                            if({$has_back}){
                                {$callback}(_back);
                            }else{
                                //if({$type}=='checkbox'){
                                    //$(this).parent().append('<div>Hello world!</div>');
                                //}
                            }
            			}
					})
     ...
     * @return  str
     */
    function input_img($arr){
		if(!$arr || !isset($arr['name'])) return;
		$img = !empty($arr["value"]) ? $arr["value"] : SITE_URL."/upload/upload-pic.png";
		if($arr["value"]){
			$str = '<div><a href="javascript:;" class="preview _upload_'.$arr["name"].'" attr_src="'.$arr["attr_src"].'" title="'.$arr["title"].'"><img src="'.$img.'" width="105" height="88"></a>
			<input type="hidden" value="'.$arr["value"].'" name="'.$arr["name"].'"></div>';
		}else{
			$str = '<div><a href="javascript:;" class="preview _upload_'.$arr["name"].'" title="单击上传图片"><img src="'.$img.'" width="105" height="88"></a>
		<input type="hidden" value="'.$arr["value"].'" name="'.$arr["name"].'"></div>';
		}
		
		$str.='<script>				$(document).ready(function(){					$("._upload_'.$arr['name'].'").click(function(){						var $ws = "dialogHeight=500px;dialogWidth=710px;resizable=no;dialogLeft=500px;";						var param ="&d='.$arr["dir"].'&w='.$arr['water'].'&callback='.$arr["name"].'";                    if(window.showModalDialog){                        window.showModalDialog("index.php?app=images"+param,"",$ws);                    }else{                        window.open("index.php?app=images"+param,"","width=710,height=500");                    }							})				})						function '.$arr['name'].'(src){
						$("._upload_'.$arr['name'].'").attr("attr_src",src)						$("._upload_'.$arr['name'].'").children("img").attr("src", src);						$("._upload_'.$arr['name'].'").parent("div").children("input").val(src);    			}				</script>';		return $str;
    }
    /**
     * 处理 input_object  标签
     * 
    ...
    function input_object($params){
    	$name = empty($params['name']) ? '请选择' : $params['name'];
    	$str = '<a href="javascript:void(0);" class="link">'.$name.'</a>
    	...
    	$(document).ready(function(){
    		$(".link").click(function(){
    			...
    			if($linkid){
    				var url = "index.php?app=common&act=ajax_customs_info";
    				params = {};
    				params.ids = $linkid;
    				$.post(
    						url,
    ...
    function get_val($val)
    {
        ...
        {
            /* add by zhaoxr 升级5.4+ @preg_replace*/
            if (version_compare(PHP_VERSION,'5.5','>'))
            {
    ...
    function java_script($args)
    {
        ...
        $idx = array_search("rctailor", $arr);
        if ($idx !== false)
        {
            unset($arr[$idx]);
            array_unshift($arr, 'rctailor');
        }
    ...
    */
    ...
    function nav_uname($uname){
    	if(!$uname)
    		return '游客';
    	
    	if(mb_strlen($uname,'utf-8') < 11)
    		return $uname;
    	
    	$str = mb_substr($uname, 0,10,'utf-8')."...";
