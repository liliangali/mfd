...
class BaseModel extends Object
{
    ...
    function BaseModel($params, $db)
    {
        ...
        $this->_prefix = !empty($this->_prefix) ? $this->_prefix : DB_PREFIX;
    ...
    function findAll($params = array())
    {
        ...
        if (!empty($include) && !empty($pri_data))
        {
            ...
            foreach ($include as $relation_name => $find_param)
            {
                ...
                /* 容错处理 has_error add by xiao5 */
                is_array($related_data) && !empty($related_data) && $pri_data = $this->assemble($relation_name, $related_data, $pri_data);
            }
        }
     ...
     *  @return mixed
     */
    function add($data, $compatible = false,$flag=1,$mark='')
    {
        ...
        if ($insert_id)
        {
            if ($insert_info['length'] > 1)
            {
                for ($i = $insert_id; $i < $insert_id + $insert_info['length']; $i++)
                {
                    ...
                    //=====  写入更新文件 Beg  =====
                    modeledit($this->table,$this->prikey."=".$i,'add');
                    //=====  写入更新文件 End  =====
                }
            }
            else
            {
                ...
                //=====  写入更新文件 Beg  =====
                modeledit($this->table,$this->prikey."=".$id,'add');
                //=====  写入更新文件 End  =====
            }
        }else{
        ...
        if($flag){
        	$this->operation_log($id,'add',$mark);
        }       
        return $id;
    }
     ...
     *  @return bool
     */
    function edit($conditions, $edit_data,$flag=1,$mark='')
    {
    	$tmp = array();
        if (empty($edit_data))
        {
       ...
       /*  add by v5 limit where条件位空 预警 start */
        if (!$conditions){
        	$trace  = debug_backtrace();
        	$this->plog('where is empty!', "UPDATE {$this->table} SET {$edit_fields}{$conditions}",$trace[0]['file'],$trace[0]['line']);
        	return false;
        }
        /*  add by v5 limit where条件位空 预警 end */
  		$sql="UPDATE {$this->table} SET {$edit_fields}{$conditions}";
        $res = $this->db->query($sql);

        //=====  写入更新文件 Beg  =====
        modeledit($this->table,$conditions,'edit');
        //=====  写入更新文件 End  =====

        if($flag){
        	$this->operation_log($conditions,'edit',$mark);
        }		
     ...
     * @param   string  $line
     */
    function plog($err, $msg, $file, $line)
    {
    	$filename = ROOT_PATH . "/temp/logs/update-" .date("Ym"). ".log";
     ...
     *
     */
    protected function operation_log($conditions,$type,$mark='',$drop_data=array()){
/*     	var_dump($conditions);
    	var_dump($type); */
    	if (defined('IN_BACKEND'))
    	{
    		...
    		$ext='';
    		if($type=='add'){
    			$ext.=addslashes(",对应id为<font style='red'>{$logdata['conditions']}</font>");
    		}else if($type=='drop'){
    			foreach ($drop_data as $k=>$v){
    				$ext.=",{$k}:{$v}";
    			}
    		}
...
/*     				    		var_dump($rr);
    				    		var_dump($_SESSION['app']);
    				    		var_dump(substr(str_replace($_SESSION['app'],'',$rr),0,1));
    				    		
    				    		var_dump($_SESSION['act']);
    				    		var_dump(substr(strstr($rr,$_SESSION['act'],true),-1)); */
    				    	    if(!is_array($rr)){
    				    	        if (substr(str_replace($_SESSION['operate_app'],'',$rr),0,1)=='|' && substr(strstr($rr,$_SESSION['operate_act'],true),-1)=='|'){
/*     				    	        	var_dump(2);
    				    	        	var_dump(substr(str_replace($_SESSION['app'],'',$rr),0,1)=='|' && substr(strstr($rr,$_SESSION['act'],true),-1)=='|');
    				    	        	var_dump(3); */
    				    	...
    				    	//var_dump($r);exit;
    				        if (substr(str_replace($_SESSION['operate_app'],'',$r),0,1)=='|' && substr(strstr($r,$_SESSION['operate_act'],true),-1)=='|'){
/*     				        	var_dump(4);
    				        	var_dump($rr);
    				        	var_dump($_SESSION['app']);
    				        	var_dump($_SESSION['act']);
...
//     					exit;
			...
			if($mark){
				...
				$logdata['memo']='操作人员：'.$logdata['username'].'在['.date('Y-m-d H:i:s',$logdata['dateline']).']对-'.$logdata['operate_key'].'-执行了'.$typedata[$type].'操作'.$ext;
			}  		
...
/*    		var_dump($res);
     ...
     *  @return int
     */
    function drop($conditions, $fields = '',$flag=1)
    {
        if (empty($conditions))
        {
            return;
        ...
        if (defined('IN_BACKEND')){
        	...
        	$this->_saveDroppedAllData("SELECT * FROM {$this->table}{$conditions}");
        	$droped_all_data=$this->getAllDroppedData();
        }
        ...
        {
            /* 删除依赖数据 */
            $res=$this->dropDependentData(array_keys($droped_data));
        }
        if (defined('IN_BACKEND')){
        	$tmp='';
        	foreach ($droped_all_data as $key=>$value){
        		$c_tmp='';
        		foreach ($value as $k=>$v){
        			if($v==='' || $v===null){
        				$v="&nbsp;";
        			}
        			$c_tmp.=addslashes("(<font style='green'>{$k}</font>:<font style='color:red'>{$v}</font>),");
        		}
        		$tmp.="{{$key}[".rtrim($c_tmp,',')."]}|";
        	}
	        if(gettype($res) !='boolean'){
	        	if(is_array($res)){
	        		$droped_data=array(
	        				'主表删除数据'=>rtrim($tmp,'|'),
	        				'从表表名'=>"{$res['relation_mod']}",
	        				'从表删除数据'=>addslashes("({$res['drop_condition']})"),
	        		);
	        	}      	
	        }else {
	        	$droped_data=array(
	        			'主表删除数据'=>rtrim($tmp,'|'),
	//         			'从表表名'=>$res['relation_mod'],
	//         			'从表删除数据'=>$res['drop_condition']
	        	);
	        }
	        if($flag){
	        	$this->operation_log($conditions,'drop','',$droped_data);
	        }
     ...
     *  @return bool
     */
    function delete_where($conditions){
        $this->db->query("DELETE FROM {$this->table} where {$conditions}");
    }
    /**
     *  删除依赖数据
     *
    ...
    function dropDependentData($keys)
    {
        ...
        foreach ($relation as $relation_name => $relation_info)
        {
            ...
            elseif ($relation_info['dependent'] && $relation_info['type'] !== BELONGS_TO)
            {
                ...
                return array('relation_mod'=>$relation_info['model'],'drop_condition'=>$conditions);
            }
        }
    }
     ...
     *  @return array
     */
    function getAllDroppedData()
    {
    	return $this->_all_dropped_data;
    }
     ...
     *  @return void
     */
    function _saveDroppedAllData($sql)
    {
    	$this->_all_dropped_data = $this->db->getAll($sql);
    }
