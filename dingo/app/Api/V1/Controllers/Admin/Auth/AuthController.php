...
namespace App\Api\V1\Controllers\Admin\Auth;
...
//            }
        ...
        $item->erweima_url = asset('storage/public/tqrcode/'.$new_url);
        $item->save();
    ...
    public function imgup(Request $request)
    {
        ...
        $img->save($dir.$string);
        $response = array(
            'imr_url' =>asset('storage/upload/'.$string)
        );
        return response()->json($response);
    ...
    public function register(Request $request) {
        ...
        //验证手机验证码
        $sms = LaravelSms::where("to",$request->get("user_name"))->where("fail_times",0)->orderBy("id","DESC")->first();
        $time = Carbon::now(config('app.timezone'))->timestamp;
        $smsdata = json_decode($sms->data,1);
        if(!$sms || !(($sms->sent_time + 300) >= $time)  || $smsdata['code'] != $request->get('phonecode'))
        {
            $response = array(
                'msg' => '验证码错误或者已过期',
                'status' => 1
