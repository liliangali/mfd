...
function get_invite1($sourceNumber, $num) {
	$i = pow ( 10, $num );
	return substr ( strval ( $sourceNumber + $i ), 1, $num );
}


function fahuo1($order_id)
{
    ...
    $res = smsAuthCode($phone,'deliver','deliver','get','pc','',['order_sn'=>$order_info['order_sn'],'express'=>$yundan]);
 ...
 */
function give_debit1($amount, $user_id) {
 ...
 *         @2015年7月6日
 */
function reloadMember1($user_id) {
	$mod = m ( 'member' );
 ...
 *         @2016年1月31日
 */
function editerweimaUrl1($user_id) {
	$m = &m ( 'member' );
 ...
 * @param string $dialog        	
 */
function sendSystem1($to_user_id, $type, $title_mes, $content_mes, $send_type = "all") {
 ...
 * @param string $dialog        	
 */
function sendMesXin1($to_user_id, $type, $title_mes, $content_mes, $send_type = "all") {
	$_message_mod = &m ( "usermessage" );
	$m_mod = m ( 'member' );
...
function _format_price_int1($price = 0.00) {
	return intval ( $price );
 ...
 */
function doCreatePush1($send_member_token, $send_member_ids, $type, $title_mes, $content_mes, $send_type = "all") {
	$_message_mod = &m ( "usermessage" );
	$m_mod = &m ( 'member' );
	// 发送信鸽推送
	include_once (ROOT_PATH . '/includes/xinge/xinggemeg.php');
 ...
 * @param string $dialog        	
 */
function sendMesXins1($to_user_id, $type, $title_mes, $content_mes, $send_type = "all") {
	$_message_mod = &m ( "usermessage" );
	$m_mod = m ( 'member' );
...
/* 短信发送 */
function SendSmss($phone, $msg, $type = 'bonus', $r = '111111', $fail_time = 60) {
	$err = array (
			'-1' => '传递参数错误',
			'-2' => '用户id或密码错误',
			'-3' => '通道id错误',
 ...
 * @author Xiao5
 */
function i_array_columns($input, $columnKey, $indexKey = null) {
