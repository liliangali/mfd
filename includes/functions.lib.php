...
if ($_SERVER ['HTTP_HOST'] == 'mfd.p.day900.com') {
	define ( 'LOCALHOST1', 'http://mfd.p.day900.com' );
	define ( 'DIYURL', 'http://m.cotte.cn/custom-diy-' );
	define ( 'mfdURL', 'http://api.mfd.cn/' );
	define ( 'MHOST', 'http://local.m.mfd.com' );
	define ( 'FIGUREURL', 'http://figure.mfd.cn' );
	define ( 'THESHAREURL', 'http://wap.mfd.cn/' );
} else if ($_SERVER ['HTTP_HOST'] == 'local.mfd.cn') {
	define ( 'LOCALHOST1', 'http://local.mfd.cn/' );
	define ( 'DIYURL', 'http://m.mfdb2c.cn/custom-diy-' );
	define ( 'COTTEURL', 'http://api.mfdb2c.com/' );
	define ( 'FIGUREURL', 'http://figure.mfdb2c.cn' );
} else if ($_SERVER ['HTTP_HOST'] == 'local.soaapi.mfd.com') {
	define ( 'LOCALHOST1', 'http://local.m.mfd.com' );
	define ( 'MHOST', 'http://local.m.mfd.com' );
	define ( 'DIYURL', 'http://m.test.mfd.cn/custom-diy2-' );
	define ( 'FIGUREURL', 'http://figure.mfd.cn/' );
} else if ($_SERVER ['HTTP_HOST'] == 'local.mfdform.com') {
	define ( 'LOCALHOST1', 'http://local.mfdform.com' );
	define ( 'MHOST', 'http://local.m.mfd.com' );
	define ( 'DIYURL', 'http://m.test.mfd.cn/custom-diy2-' );
	define ( 'FIGUREURL', 'http://figure.mfd.cn/' );
...
function get_invite($sourceNumber, $num) {
	$i = pow ( 10, $num );
	return substr ( strval ( $sourceNumber + $i ), 1, $num );
}
...
function fahuo($order_id,$express='',$type='0',$order_sn='')
{
    ...
    if(!$yundan)
    {
		...
		if($type=='1'){
			...
			$res = smsAuthCode($phone,'hebing','hebing','get','pc','',['order_sn'=>$order_sn,'express'=>$yundan,'shipping_name'=>$shipping_info['shipping_name']]);
		}else{
				
			$res = smsAuthCode($phone,'deliver','deliver','get','pc','',['order_sn'=>$order_info['order_sn'],'express'=>$yundan,'shipping_name'=>$shipping_info['shipping_name']]);
	    }
	//=====  如果是微信订单要发送消息  =====
...
function mpObj()
{
    ...
    $mpObj->authorizer_appid = $wx_info['authorizer_appid']['wxvalue'];
    $mpObj->authorizer_refresh_token = $wx_info['authorizer_refresh_token']['wxvalue'];
    $mpObj->authorizer_access_token =  $wx_info['authorizer_access_token']['wxvalue'];
    $mpObj->component_access_token = $wx_info['component_access_token']['wxvalue'];
    //查看authorizer_access_token 是否过期
    if(($wx_info['authorizer_access_token']['add_time'] + 7190) < time()) //过期
    {
        $mpObj->getAuthAccessToken();
        if($mpObj->authorizer_refresh_token && $mpObj->authorizer_access_token)
        {
            $wxconfig->edit($wx_info['authorizer_access_token']['id'],['wxvalue'=>$mpObj->authorizer_access_token,'add_time'=>time()]);
            $wxconfig->edit($wx_info['authorizer_refresh_token']['id'],['wxvalue'=>$mpObj->authorizer_refresh_token,'add_time'=>time()]);
        }
    }
    $mpObj->access_token = $mpObj->authorizer_access_token;
    return $mpObj;
}
...
function reloadMember($user_id) {
	...
	if ($lv_id) {
		$mod->edit ( $user_id, array (
				'member_lv_id' => $lv_id 
		) );
		
		// ===== 给会员发信鸽 =====
		sendSystem ( $user_id, 12, '恭喜，您已成为' . $lv_name . '！', '尊敬的麦富迪会员，您的积分已达到' . $point . ',恭喜您已升级为' . $lv_name . '。随着积分的累计,您的等级头衔将提升，从而享受更优惠的折扣，拥有更多的特权。请密切关注你的积分变化。' );
	}
...
function sendSystem($to_user_id, $type, $title_mes, $content_mes, $send_type = "all") {
	...
	$res = $_message_mod->add ( $arr, false, 0 );
	// 发送信鸽推送
	if ($res) {
		// 信鸽推送最多70字，限制其推送长度
		...
		$push = new XingeMeg ();
		$result = $push->tomfdXinApp ( $inviter_info ['user_token'], $title_mes, $content_mes, array (
				'url_type' => 'system',
				'location_id' => $res 
		), $send_type );
 ...
 * @param string $dialog        	
 */
function sendMesXin($to_user_id, $type, $title_mes, $content_mes, $send_type = "all") {
	$_message_mod = &m ( "usermessage" );
	$m_mod = m ( 'member' );
 ...
 * @param string $dialog        	
 * @param int $type
 *        	1.交易| 2.评论晒单| 3.评论订单|4.顾客发需求|5.实名认证|13订单发货|12用户等级提升|14红包相关|15抵用券相关|16提现|17返修
 */
function doCreatePush($send_member_token, $send_member_ids, $type, $title_mes, $content_mes, $send_type = "all") {
	...
	$result = $push->doCreateMultipush ( $send_member_token, $title_mes, $content_mes, array (
			'url_type' => 'system',
			'location_id' => 0 
	), $send_type );
 ...
 * @param string $dialog        	
 */
function sendMesXins($to_user_id, $type, $title_mes, $content_mes, $send_type = "all") {
	$_message_mod = &m ( "usermessage" );
	$m_mod = m ( 'member' );
...
/* 短信发送 */
function SendSms($phone, $msg, $type = 'bonus', $r = '111111', $fail_time = 60) {
	$err = array (
			'-1' => '传递参数错误',
			'-2' => '用户id或密码错误',
			'-3' => '通道id错误',
			...
			'-12' => '时间戳错误',
			'-13' => '编码异常',
			'-14' => '发送被限制', // 由于网关限制，同一个手机号不能反复发送过多短信，验证码一分钟只能下发一条一个小时三条
			'-15' => '语音验证码必须为4-8位的数字' 
	);
	...
	// 后台无配置地方，暂时写死--
	$user_id = '11664'; // sms9平台用户id
	$pass = 'mfd123'; // 用户密码
	$channelid = $_REQUEST ['ty'] == 'voice' ? 17099 : 12852;
	
	$msg = iconv ( "UTF-8", 'GBK', $msg );
	$url = "http://admin.sms9.net/houtai/sms.php?cpid={$user_id}&password={$pass}&channelid={$channelid}&tele={$phone}&msg={$msg}";
 ...
 *        
 */
function sendSafePhone($phone, $msg, $type = 'bonus', $r = '111111', $fail_time = 60) {
	$err = array (
			'-1' => '传递参数错误',
			'-2' => '用户id或密码错误',
			'-3' => '通道id错误',
	...
	$sms_mod = m ( 'smssafe' );
	$sms_mod->edit ( "type='$type' AND phone='$phone' ", array (
			'is_delete' => 1 
	) );
	$data ['type'] = $type;
	$data ['code'] = $r;
 ...
 * ns add 会员等级降级
 */
function reloadDowngradeMember($user_id) {
	$mod = m ( 'member' );
	$m_info = $mod->get_info ( $user_id );
	
	if (! $m_info || $m_info ['serve_type'] != 1 || $m_info ['member_lv_id'] <= 1 || $m_info ['member_lv_id'] > 4) {
		return false;
	}
	...
	if ($lv_id) {
		...
		// '抱歉，您已成为'.$lv_name.'！', '尊敬的麦富迪会员，您的积分已达到'.$point.',抱歉您已降级为'.$lv_name.'。') ;
	}
 ...
 * 调试打印
 * 
 * @param sting｜array $str        	
 */
function vdump($str) {
	global $debugbar;
	$debugbar ["messages"]->addMessage ( $str );
}
...
function emailcode($email, $category, $type, $way = 'ajax', $opt = 'get', $ps = '', $temp = array()) {
	...
	$conditions = array (
			'conditions' => " category = '$category' and type = '$type' and email = '$email' " 
	);
...
function getEmailcode($email, $category) {
	...
	$url .= "&email=$email&code=" . $code . "&account_type=email";
	return $url;
}
...
function getEmailDesc($category, $url, $temp) {
	...
	$arr = array (
			...
			'reg' => '欢迎您注册麦富迪，请点击以下链接完成邮箱验证:<a href=' . $url . '>' . $url . '</a>',
			'test' => '欢迎您注册麦富迪<{$test}>，请点击以下链接完成邮箱验证:<a href=' . $url . '>' . $url . '</a>',
			'editEmail' => '欢迎您注册麦富迪<{$test}>，请点击以下链接完成邮箱验证:<a href=' . $url . '>' . $url . '</a>' 
	);
	/* add by xiao5 替换模版变量 */
	if (! empty ( $temp )) {
		foreach ( $temp as $k => $v ) {
	...
	switch ($category) {
	    ...
	    case 'fukuan' :
		  ...
		  $code = 'SMS_47715016';
			 break;
		 case 'hebing' :
		  $params = "{'order_sn':'".$temp[order_sn]."','express':'" . $temp['express']. "','shipping_name':'" . $temp['shipping_name']. "'}";
		  $code = 'SMS_63925540';
		     break;
		case 'deliver':
			if($temp){
...
function getSmsEmailDesc($cate, $type, $temp = array()) {
	$arr = array (
			...
			'bingPhone' => array (
					'bingPhone' => '您的验证码为：<#code>',
					'reset' => '感谢您的绑定~您的绑定手机验证码为：<#code>~' . SMS_FAIL_TIME . '秒内有效，请及时注册...' 
			),
			'editPhone' => array (
					'editPhone' => '您的验证码为：<#code>',
					'reset' => '感谢您的绑定~您的绑定手机验证码为：<#code>~' . SMS_FAIL_TIME . '秒内有效，请及时注册...' 
			),
			'editPassword' => array (
					'editPassword' => '您的验证码为：<#code>',
					'reset' => '感谢您设置支付密码~您的手机验证码为：<#code>~' . SMS_FAIL_TIME . '秒内有效，请及时注册...' 
			) 
	);
	if ($arr [$cate] [$type]) {
...
//    echo '<pre>';print_r($content);exit;
		...
		} else if (in_array ( $parent_id, array (
				2,
				3 
		) )) { // 系统通知
			/*
...
//	echo '<pre>';print_r($send_members);exit;
...
//	echo '<pre>';print_r($send_member_token);exit;
 ...
 * @author Xiao5
 */
function getUser_feedback_imgUrl($path_name, $file_name) {
	/*
	 * if(!$file_name)
	 * return getDefAlbumPic($type);
	 */
	$src = get_domain () . "/upload" . $path_name . $file_name;
	return $src;
}
...
function get_instance_of($name, $method = '', $args = array()) {
	...
	if (! isset ( $_instance [$name] )) {
		if (class_exists ( $name )) {
			...
			stop ( "class not exists:" . $name, 1, 1 );
	}
	...
	if ($photo ['album_id']) {
		...
		if ($album) {
			if ($album ['top_url'] == $photo ['url']) {
				$m_album->edit ( $photo ['album_id'], array (
						'top_url' => '' 
				) );
			}
 ...
 * @author zxr
 */
function get_client_finance_expense() {
}
...
// @param $abstract 其他信息备注，默认为0,如收货人姓名:张三，余额消费:100，麦券100，现金：100（如果$minus=1,需要做这样的备注）
// 1用户在平台（APP和PC）购买实物商品的总额，统计节点：已发货
// 5购买商品包含：狗粮、猫粮等，统计节点：已付款
// @return void
// @version 1.0.0 (2015-11-13)
// @author zxr
function get_client_finance($user_id, $finance_sn, $minus, $trans_amount = 0, $abstract = array()) {
	$cfinance_mod = &m ( 'clientfinancedetail' );
	if (in_array ( $minus, array (
			FINANCE_GOODS_BUY,
			FINANCE_WITHDRAW_DEPOSIT 
	) )) {
		$data ['type'] = 1;
		$data ['mark'] = '-';
	} else if (in_array ( $minus, array (
			FINANCE_CASH_RECHARGE,
			FINANCE_INCOME_BALANCE,
			FINANCE_PAY_CASH,
			FINANCE_COIN_BUY 
	) )) {
		$data ['type'] = 2;
	...
	if ($data ['type'] == 2) {
		switch ($data ['minus']) {
			...
			/*
			 * case 7:
			 * $data['abstract']="后台会员账户调节,".$data['abstract'];
			 * break;
			 */
			/*
			 * case 7:
	...
	if (! $user_id && ! $minus) {
		$data ['abstract'] = '参数错误';
		$cfinance_mod->add ( $data, false, 0 );
		return false;
	}
	$res = $cfinance_mod->add ( $data, false, 0 );
	if ($res) {
		return true;
	} else {
		return false;
