...
/* 商品数据模型 */
class Goods_giveawayModel extends BaseModel
{
    var $table  = 'goods_giveaway';
    var $prikey = 'goods_id';
    var $alias  = 'g';
    var $_name  = 'goods_giveaway';
    var $temp; // 临时变量
    var $_relation = array(
    		
    	//=====add by liang.li  Begin =====	
    	// 一个商品对应多个相册图片
    	'has_goodsgallery' => array(
    			'model'         => 'goodsgallery',
    			'type'          => HAS_MANY,
    ...
    function get_channel_list($par){
    	...
    	$conditions .= " WHERE 1 AND g.cate_id IN ($par) ";
    	
    	$order = ' ORDER BY g.goods_id DESC';
    	...
    	foreach($goods_list as $key => $val){
    		$_goods[$val["cate_id"]]["cate_id"]   = $val["cate_id"];
    		$_goods[$val["cate_id"]]["cate_name"] = $val["cate_name"];
    		$_goods[$val["cate_id"]]["child"][] = $val;
    	}
    	return $_goods;
    }
    /**
    ...
    function get_list($params = array(), $scate_ids = array(), $desc = false, $no_picture = true)
    {
        ...
        $fields = "g.goods_id, g.store_id, g.type, g.goods_name, g.cate_id, g.cate_name, g.brand, g.spec_qty, g.spec_name_1, g.spec_name_2, g.if_show, g.if_new,g.if_new,g.price_n,g.cost_price, g.closed, g.add_time, g.recommended, g.default_image,g.fabric_number, " .
                "gs.spec_id, gs.spec_1, gs.spec_2, gs.color_rgb, gs.price, gs.stock, " .
                "s.store_name, s.region_id, s.region_name, s.credit_value, s.sgrade, " .
                "gst.views, gst.sales, gst.comments";
        $desc && $fields .= ", g.description";
 ...
 * $num 购买数量
 * */
    function update_goods_count($goods_id,$num)
    {
    	$goods=$this->get(array(
    			'conditions'=>"goods_id='{$goods_id}'",
    	));
    	$counts=$goods['buy_count']+$num;
    	$this->edit($goods_id, array('buy_count' => $counts));
    } 
...
class GoodsB_giveawayModel extends Goods_giveawayModel
{
    var $_store_id = 0;
...
/*     function add($data, $compatible = false)
    ...
    function get_brand_goods($_str){
    	$sql = "SELECT * FROM {$this->table} WHERE closed = 0 AND goods_id IN ($_str) AND recommended=1 ORDER BY add_time desc LIMIT 12";
