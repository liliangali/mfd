    	...
    	if ($this->_nodes_found){
    		...
    		foreach ($nodes_found as $v){
    			...
    			if(in_array($v['top_cate'], Constants::$graphParent)){//工艺
    				if (!in_array($v['parent_cate'], Constants::$graphRuleoutParent)){//工艺下不纳入出图规则计算
 		...
 		return array('cross'=>$cross,'crosskey'=>array_reverse(array_unique($crosskey)),'crosskeyname'=>array_reverse(array_unique($crosskeyname)));
...
//     				var_dump($haystack);exit();
    ...
    protected function _get_history($pams){
    	...
    	$where = '';
    	/* 根绝点击log检索 */
    	if ($pams['uid']) $where = ' AND a.uid='.$uid;
...
//     				$spec = $this->_format_spec_data($item['pt_id']);
					...
					if (in_array($patinfo['cate_id'], Constants::$fabricsParent)){//面料顶级id
						...
						$patinfo['part_small'] = SITE_URL.'/'.$patinfo['part_small'];		
