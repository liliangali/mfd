...
class Tree extends Object
{
    ...
    function getArrayList($root = 0 , $layer = NULL)
    {
        ...
        foreach ($this->child[$root] as $id)
        {
			...
			if ($id == '1261' or $id == '1262' or $id == '1263' or $id == '1316' or $id == '1371')
			{
			    $top_store = $db->getall("select cf_store.store_id,cf_store.store_name from cf_gcategory join cf_category_store on cf_gcategory.cate_id=cf_category_store.cate_id join cf_store on cf_store.store_id=cf_category_store.store_id where cf_gcategory.cate_id = '$id'");
			}
            $data[] = array('id' => $id, 'value' => $this->getValue($id), 'children' => $this->child[$id] ? $this->getArrayList($id , $layer) : array(),'top_store'=>$top_store);		
        }
        return $data;
    }
