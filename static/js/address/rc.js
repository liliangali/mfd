...
; (function($) {
	...
	$.rc.photo_slide = function() {
		...
		if (! (f != 1 && f < 4)) {
			...
			if (!$.rc.util.empty(a) && !$.rc.util.empty(a.listID) && !$.rc.util.empty(a.listBtnID) && !$.rc.util.empty(a.feedClassName) && !$.rc.util.empty(a.feedWidth) && $("#" + a.listID).size() != 0) {
				var e = 0,
				d = 1,
				j = null,
				h = [],
				g = null,
				g = $.rc.util.empty(a.feedBoxClass) ? $("#" + a.listID + " ul") : $("#" + a.listID + " ." + a.feedBoxClass),
				k = function() {
...
(function(c) {
	//wangdy
	c.alert_z = c.dialog.alert_z = function(title,content) {
		return c.dialog({
			title:title,
			id: "alert_z",
			ok: true,
