...
?>
        <tr class="tatr2">
            <td class="firstCell"><input type="checkbox" class="checkitem" value="<?php echo $this->_var['article']['article_id']; ?>"/></td>
            <td><?php echo htmlspecialchars($this->_var['article']['title']); ?></td>
            <td><?php echo htmlspecialchars($this->_var['article']['cate_name']); ?></td>
            <td><?php echo htmlspecialchars($this->_var['article']['code']); ?></td>
            <td><?php echo $this->_var['article']['if_show']; ?></td>
            <td><?php echo local_date("Y-m-d H:i:s",$this->_var['article']['add_time']); ?></td>
            <td><span ectype="inline_edit" fieldname="sort_order" fieldid="<?php echo $this->_var['article']['article_id']; ?>" datatype="pint" maxvalue="255" class="editable"><?php echo $this->_var['article']['sort_order']; ?></span></td>
            <td><a href="index.php?app=article&amp;act=edit&amp;id=<?php echo $this->_var['article']['article_id']; ?>">编辑</a>
                
                <a href="javascript:drop_confirm('您确定要删除它吗？', 'index.php?app=article&amp;act=drop&amp;id=<?php echo $this->_var['article']['article_id']; ?>');">删除</a>
<!-- 				| <a target="_blank" href="<?php echo $this->_var['site_url']; ?>index.php/article-view-<?php echo $this->_var['article']['article_id']; ?>.html">查看</a></td> --><!-- 暂且保留 -->
        </tr>
        <?php endforeach; else: ?>
        <tr class="no_data">
