 ...
 * --------------------------------------------------------
 * @author       小五
 * $Id: order_commit_new_pc.php 14287 2016-03-24 02:09:20Z xiao5 $
 * $Date: 2016-03-24 10:09:20 +0800 (Thu, 24 Mar 2016) $
 * --------------------------------------------------------
 * @example
 */
...
$sql = "select * from cf_order_pc_cron where status in (0,2) ";
/* 测试 5件商品 2件diy套装 2件样衣套装 一件单品  */
// $sql = "SELECT * FROM `cf_order_pc_cron` WHERE `fabric_id` IN (7018,7017,7071,7070,6996) ORDER BY `fabric_id` DESC ";
/* 测试 diy 套装推送 */
//$sql = "SELECT * FROM `cf_order_pc_cron` WHERE `fabric_id` IN (7018,7017) ORDER BY `fabric_id` DESC ";
...
$goods_data = getAlls($sql);
$itmes = array();
foreach ($goods_data as $gk=>$gv){
	/* 套装条件：西服＋西裤 ｜数量1件（多件匹配有问题） */
	if (in_array($gv['cloth'], array('0003','0004')) && $gv['quantity'] == 1){
...
//	var_dump($data);exit();
	...
	if(isset($respon_data['data']['code']) && $respon_data['data']['code'] == 101){
		$up_data['status']  = 1;
		$up_data['err_msg'] = '';
		...
		$up_data['status'] = 2;
		$up_data['err_msg'] = isset($respon_data['data']['rs']) ? $respon_data['data']['rs'] : $respon_data['msg'];
	}

	$sql = update($up_data," fabric_id in ('".implode(array_unique($rid), "','")."') ","cf_order_pc_cron");
...
function getXml2psc($order){
	...
	foreach ($order as $k=>$v){
		...
		$remarker = $xml_arr['OrderInformation']['Remarker'] ;
		$xml_data[$k]  = $xml_arr;
...
// 	$data = getAlls($sql);
	...
	$remarker =  _pre($orderInfo['memo']);//订单备注
	...
	if($orderGoods['size'] == 'diy' && $orderGoods['son_sn']){
		$sql = "SELECT * FROM cf_order_figure WHERE  son_sn = '{$orderGoods['son_sn']}'";
		$data = getAll($sql);
...
// 		"10726:{$orderFigure['part_label_10726']}", //马夹后衣长 add 去掉后衣长 by 曹
			...
			//女西服
			$_cs['95000'] = array('figure' => array(
					"10101:{$orderFigure['lw']}", //领围（衬衣）
					"10102:{$orderFigure['xw']}", //胸围
					"10105:{$orderFigure['zyw']}", //中腰
					"10108:{$orderFigure['tw']}", //臀围
					"10110:{$orderFigure['sbw']}", //上臂围
			...
			$_cs['98000'] = array('figure' => array(
					...
					"10126:{$orderFigure['zkc']}", //左裤长
					"10127:{$orderFigure['ykc']}", //右裤长
					"10128:{$orderFigure['xiw']}", //膝围(成衣)
					"10129:{$orderFigure['jk']}", //脚口(成衣)
			));
			//女大衣
			$_cs['103000'] = array('figure' => array(
					"10101:{$orderFigure['lw']}", //领围（衬衣）
					"10102:{$orderFigure['xw']}", //胸围
					"10105:{$orderFigure['zyw']}", //中腰
					"10108:{$orderFigure['tw']}", //臀围
					...
					"10117:{$orderFigure['syhyc']}", //后衣长
					"10172:{$orderFigure['qyj']}", //前腰节
					"10725:{$orderFigure['dyhyc']}", //大衣后衣长（成衣）
			));
			//女西裙	女西裙目前不支持量体
		/*	$_cs['130000'] = array('figure' => array(
			));*/
			//女马甲 女马甲目前不支持量体
			$_cs['222000'] = array('figure' => array(
					"10101:{$orderFigure['lw']}", //领围
					"10102:{$orderFigure['xw']}", //胸围
					"10105:{$orderFigure['zyw']}", //中腰围
					"10108:{$orderFigure['tw']}", //臀围
	...
	if ($orderGoods['size'] != 'diy'){
		...
		$sizeAreaID = getAreaID($orderGoods['cloth']);
		$figureTmp = array();
		...
		/* 处理 尺码中有字符 如92L0  这样的数据 取92 */
		$sizeArr = array();
		$sizeArr = preg_split("/([a-zA-Z]+)/", $size[1], 0, PREG_SPLIT_NO_EMPTY | PREG_SPLIT_DELIM_CAPTURE);
		$weight = $sizeArr[0];
	...
	//---
	$font = $color = $location = $content = '';
	if ($orderGoods['embs']){
		$embsData = array();
		$embsData = json_decode($orderGoods['embs'],1);
		/*[fdiy]面料下单刺绣内容*/
		if('fdiy' == $orderGoods['type']){
			_format_embs($embsData,$font,$color,$location,$content);
		}else{
		...
		$dfData['4000']['defProcess'] = '4640,4991,70335';
		...
		$dfData['11000']['defProcess'] = '12244,11332,11333';
		$dfData['11000']['defProcessContent'] = '12244:amount,11332:price,11333:composition';
		$dfData['95000']['defProcess'] = '96798,95212,95245';
		$dfData['95000']['defProcessContent'] = '96798:amount,95212:price,95245:composition';
		$dfData['98000']['defProcess'] = '98350,98134,98149';
		$dfData['98000']['defProcessContent'] = '98350:amount,98134:price,98149:composition';

		$dfData['15000']['defProcess'] = '16526,16409,16019';
		$dfData['15000']['defProcessContent'] = '16526:amount,16409:price,16019:composition';
		$dfData['103000']['defProcess'] = '103368,103131,103127';
		$dfData['103000']['defProcessContent'] = '103368:amount,103131:price,103127:composition';

		/*目前女西裙|女马甲 价格\吊牌价格\吊牌面料成分 缺少必要的 code*/
		//女西裙
//		$dfData['130000']['defProcess'] = '130050';
//		$dfData['130000']['defProcessContent'] = ':price,:composition';
//		//女马甲
//		$dfData['222000']['defProcess'] = ',';
//		$dfData['222000']['defProcessContent'] = ':price,:composition';


	/*非固化款推送*/
	if(!isset($orderGoods['design']) || empty($orderGoods['design'])) {
...
// 				$separate_process
				...
				if (!in_array($sylineData[0], explode(',', $dfData[$clothingID]['defProcess']))) {
					$sylineData[0] = str_replace('bty-', '', $sylineData[0]);
					//会有多个 -
					$graph[] = $sylineData[0];
				}
...
// 			echo "\r\t\n".'工艺组合转换耗时'.round($t2-$t1,3).'秒'."\r\n\t";
		...
		/*固化款不包括刺绣信息*/
//		$fixedStyleConf = ['3' => '33572', '2000' => '40940', '4000' => '70530', '3000' => '51554', '6000' => '61214', '95000' => '95237', '98000' => '98139', '11000' => '11355', '15000' => '16093', '103000' => '103173', '130000' => '130017', '222000' => '222090'];
		/*深度设计:款式号 非客户指定*/
		$fixedStyleConf = ['3' => '1374', '2000' => '2618', '4000' => '4639', '3000' => '3713', '6000' => '6602', '95000' => '95254', '98000' => '98023', '11000' => '11316', '15000' => '16101', '103000' => '103008', '130000' => '130011', '222000' => '222087'];
		$graphstr = $fixedStyleConf[$clothingID];
		$pcstr = $fixedStyleConf[$clothingID].':'.$orderGoods['design'];
	}
...
// 正则只保留中文、数字、字母、空格
function _pre($content){
	// 	  " */1A-X=这是a2c123~!@#$%c红领^&*()_+{}:|<>?,./;'┮集团 ┞[]\-=`456" －> "1AX这是a2c123c红领集团456"
	$content = preg_replace("/[^0-9a-z\s\x{4e00}-\x{9fa5}]+/iu",'',$content);
	return $content;
}
